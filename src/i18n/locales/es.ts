import type { Translations } from "./en";

// DRAFT — review with a native Spanish speaker before release.
export const es: Translations = {
  help: {
    docs: "Abrir documentación",
  },

  mobileApp: {
    heading: "DecksMD para iPhone y iPad",
    body: "La misma bóveda, el mismo markdown, repasado en el móvil. Tarjetas, programación e historial siguen en tus archivos.",
    cta: "Descargar en el App Store",
  },
  pro: {
    heading: "Decks Pro",
    body: "Genera tarjetas a partir de tus notas, PDF y diapositivas escaneadas sin configurar una clave de API. Pruébalo gratis: todo lo demás en Decks sigue siendo gratuito.",
    cta: "Más información",
  },

  views: {
    decks: "Mazos",
    review: "Repasar",
    browse: "Explorar",
    reviewOf: "{prefix}: {name}",
    manager: "Gestor de tarjetas",
    managerOf: "Gestionar: {deckName}",
    exam: "Examen",
  },

  ribbon: {
    decks: "Mazos",
  },

  testDeckCanvas: {
    filename: "Decks — Comenzar con canvas.canvas",
    title: "Comenzar con canvas",
    intro:
      "Este canvas también es un mazo de tarjetas. Cada nodo de texto debajo contiene una o más tarjetas, analizadas con los mismos formatos que un mazo de Markdown. La etiqueta del mazo es {tag}.",
    formatsHint:
      "Arrastra nodos, edita el texto, añade más — cada nodo de texto en los archivos .canvas de esta carpeta forma parte de tu mazo.",
    section1: "Tarjetas encabezado-párrafo",
    q1: "¿Qué hace diferente a un mazo de canvas frente a un mazo de Markdown?",
    a1: "Cada nodo de texto se analiza de forma independiente. Mezcla formatos libremente entre nodos.",
    q2: "¿Dónde busca el plugin los mazos de canvas?",
    a2:
      "Dentro de la carpeta configurada en Ajustes → Mazos de canvas. Este archivo vive allí.",
    section2: "Tarjetas de tabla",
    fsrsConceptsHeading: "Conceptos de FSRS",
    colFront: "Anverso",
    colBack: "Reverso",
    colNotes: "Notas",
    row1Front: "¿Qué significa FSRS?",
    row1Back: "Free Spaced Repetition Scheduler",
    row1Notes: "El algoritmo que usa Decks para programar las tarjetas",
    row2Front: "¿Cuáles son las cuatro valoraciones de repaso?",
    row2Back: "Otra vez, Difícil, Bien, Fácil",
    row2Notes:
      "«Otra vez» reinicia la tarjeta; «Fácil» da el intervalo más largo",
    section3: "Tarjetas cloze (huecos)",
    solarHeading: "El sistema solar",
    solarBody:
      "El ==Sol== es la estrella en el centro de nuestro sistema solar. El planeta más cercano es ==Mercurio== y el más grande es ==Júpiter==.",
    section4: "Tarjetas espaciales",
    spatialIntro: "# Tarjetas espaciales\n\nConecta nodos de texto con aristas. Cada arista se convierte en una tarjeta: el nodo de origen es el anverso, el de destino el reverso y la etiqueta de la arista una pista. Los nodos de una cadena (A → B → C) generan tarjetas todos. Los nodos sueltos siguen leyéndose con los cuatro formatos de arriba.",
    spatialNodePhotosynthesis: "Fotosíntesis",
    spatialNodeSunlight: "Luz solar",
    spatialNodeGlucose: "Glucosa",
    spatialEdgeNeeds: "necesita",
    spatialEdgeProduces: "produce",
  },

  testDeck: {
    filename: "Decks — Primeros pasos.md",
    title: "Primeros pasos con Decks",
    intro:
      "¡Te damos la bienvenida a Decks! Para convertir este archivo (o cualquier otro) en un mazo de tarjetas, simplemente añade la etiqueta `{tag}` en el frontmatter o en cualquier parte del texto.",
    formatsHint:
      "Decks admite cinco formatos principales. Elige el que mejor se adapte a tus notas.",
    section1: "1. Formato de encabezado-párrafo",
    section1Body:
      "Por defecto, Decks utiliza **encabezados H2 (`##`)** como el anverso de la tarjeta, y el párrafo directamente debajo como el reverso. *(Puedes cambiar este nivel de encabezado en los ajustes del mazo).*.",
    q1: "¿Qué es la repetición espaciada?",
    a1: "Una técnica de aprendizaje que programa repasos en intervalos crecientes basados en la fuerza de la retención.",
    q2: "¿Qué etiqueta marca un archivo como un mazo de tarjetas?",
    a2: "La etiqueta `{tag}`. Añádela a la lista de `tags` del frontmatter o en cualquier parte del cuerpo del texto.",
    section2: "2. Formato de tabla",
    section2Body:
      "Puedes usar tablas markdown de dos columnas para generar tarjetas en bloque.\n**Regla fundamental:** La tabla *debe* estar directamente debajo de un encabezado. Ese encabezado actúa como contenedor de esas tarjetas.",
    fsrsConceptsHeading: "Conceptos de FSRS",
    colFront: "Anverso",
    colBack: "Reverso",
    colNotes: "Notas",
    row1Front: "¿Qué significan las siglas FSRS?",
    row1Back: "Free Spaced Repetition Scheduler",
    row1Notes: "El algoritmo que Decks usa para programar tarjetas",
    row2Front: "¿Cuáles son las cuatro calificaciones de repaso?",
    row2Back: "Otra vez, Difícil, Bien, Fácil",
    row2Notes:
      "«Otra vez» reinicia la tarjeta; «Fácil» da el intervalo más largo",
    tableNote:
      "*(Nota: La primera columna es el Anverso, la segunda el Reverso. La tercera columna opcional es para Notas/Pistas).*.",
    section3: "3. Huecos (Cloze)",
    section3Body:
      "Usa la sintaxis `==resaltado==` para rellenar los huecos. Cada texto resaltado se convierte en su propia tarjeta.",
    solarHeading: "El sistema solar",
    solarBody:
      "El ==Sol== es la estrella en el centro de nuestro sistema solar. El planeta más cercano es ==Mercurio==, y el más grande es ==Júpiter==.",
    section4: "4. Oclusión de imagen",
    section4Body:
      "Combina una imagen con una lista numerada. La imagen muestra las regiones etiquetadas, y cada elemento de la lista se convierte en una tarjeta separada.",
    bonesHeading: "Huesos del brazo",
    bone1: "Húmero",
    bone2: "Radio",
    bone3: "Cúbito",
    section5: "5. Plantillas (diseños de tarjeta propios)",
    section5Body:
      "Etiqueta el encabezado de una tabla y Decks renderiza cada fila con un archivo de plantilla de tu carpeta de plantillas (Ajustes → Plantillas de tarjetas). El encabezado de esta tabla lleva #{tag}, así que sus filas se renderizan con la plantilla correspondiente: edita una fila en el gestor de tarjetas y tendrás un campo por columna.",
    templatesTableHeading: "Vocabulario kanji",
    templateTag: "vocabulario",
    templateFolderName: "Plantillas de Decks",
    templateFileName: "Tarjeta de vocabulario.md",
    tplColWord: "Palabra",
    tplColReading: "Lectura",
    tplColMeaning: "Significado",
    tplRow1Word: "火",
    tplRow1Reading: "ひ",
    tplRow1Meaning: "fuego",
    tplRow2Word: "水",
    tplRow2Reading: "みず",
    tplRow2Meaning: "agua",
  },
  anki: {
    title: "Importar desde Anki",
    description: "Importa una exportación .apkg de Anki a Decks: tus mazos, tarjetas, archivos multimedia y tu historial de repaso vienen contigo.",
    howItWorks: "Cómo funciona: cada mazo de Anki se convierte en un archivo de Decks (la jerarquía de mazos se conserva como carpetas), las notas se convierten en tarjetas anverso/reverso limpias, los archivos multimedia se copian en tu bóveda y se importa el historial de programación/repaso para que las tarjetas continúen donde Anki las dejó.",
    fileLabel: "Archivo de Anki",
    fileDesc: "Elige un archivo .apkg exportado desde Anki.",
    noFileSelected: "Ningún archivo seleccionado",
    targetFolderName: "Carpeta de destino",
    targetFolderDesc: "Donde se crearán los archivos de Decks importados.",
    profileName: "Perfil",
    profileDesc: "Perfil aplicado a los mazos importados (define el nivel de encabezado y la programación).",
    splitName: "Dividir mazos grandes en varios archivos",
    splitDesc:
      "Divide los mazos con muchas tarjetas o multimedia en archivos numerados para que cada uno se abra rápido en Obsidian. Los submazos siempre se importan como archivos separados.",
    chunkSizeName: "Tarjetas por archivo",
    chunkSizeDesc: "Número máximo de tarjetas en cada archivo cuando se divide un mazo.",
    scanButton: "Analizar",
    scanning: "Analizando…",
    scanSummary: "Se encontraron {decks} mazo(s), {cards} tarjeta(s), {history} con historial de repaso, {media} archivo(s) multimedia.",
    scanFailed: "El análisis falló. Consulta la consola para más detalles.",
    importButton: "Importar",
    importing: "Importando…",
    importFailed: "La importación falló. Consulta la consola para más detalles.",
    selectFileFirst: "Primero selecciona un archivo .apkg.",
    notAnApkg: "Ese no es un archivo .apkg de Anki. Exporta un mazo desde Anki y elígelo.",
    progressReading: "Leyendo la colección…",
    progressWriting: "Escribiendo {deck}…",
    progressSyncing: "Sincronizando mazos…",
    progressImporting: "Importando el historial de repaso…",
    progressCopyingMedia: "Copiando multimedia {done}/{total}…",
    successNotice: "Se importaron {cards} tarjeta(s) en {decks} mazo(s); historial de {history}.",
    doneSummary: "Listo: {decks} mazo(s), {cards} tarjeta(s), {media} multimedia, {history} con historial.",
    cardsMerged:
      "{count} tarjeta(s) compartían el anverso con otra y se combinaron en una.",
    mediaIndexingNotice:
      "Obsidian está indexando {count} archivos multimedia importados en segundo plano; las imágenes y el audio pueden tardar un momento en aparecer.",
    cancel: "Cancelar",
  },

  srMigration: {
    title: "Migrar desde Spaced Repetition",
    description:
      "Convierte notas del plugin Spaced Repetition al formato de Decks, llevándote tus tarjetas y tu historial de repasos.",
    howItWorks:
      "Cómo funciona: las tarjetas de cada nota se extraen a su propio archivo de Decks, mientras que la nota en sí se reescribe como texto limpio y legible (se elimina la sintaxis de tarjetas heredada) conservando el historial de repasos; las tarjetas inversas mantienen ambas direcciones. Los nuevos archivos se crean dentro de la carpeta de destino (se conserva la estructura de carpetas), y la nota migrada enlaza con su archivo de tarjetas — y de vuelta con el original — mediante el frontmatter. Tus notas originales quedan intactas. Volver a ejecutar la migración sobrescribe los archivos generados anteriormente.",
    sourceFolderName: "Carpeta de origen",
    sourceFolderDesc:
      "Carpeta a analizar en busca de tarjetas heredadas. Déjala vacía para analizar todo el almacén.",
    targetFolderName: "Carpeta de destino",
    targetFolderDesc: "Dónde se crearán los nuevos archivos de Decks.",
    sameFolderName: "Crear junto a la nota de origen",
    sameFolderDesc:
      "Crea las notas migradas en la carpeta de cada nota de origen en lugar de una carpeta de destino.",
    srTagName: "Etiqueta de Spaced Repetition",
    srTagDesc:
      "La etiqueta base que usan tus tarjetas heredadas. Las subetiquetas se traducen a tu etiqueta de Decks.",
    profileName: "Perfil",
    profileDesc:
      "Perfil aplicado a los mazos migrados (define el nivel de encabezado y la programación).",
    reviewTagName: "Etiqueta de repaso de nota completa",
    reviewTagDesc:
      "Las notas con esta etiqueta se migran como una única tarjeta de nota completa (nombre de archivo = anverso).",
    hintLabel: "pista",
    flashcardsProperty: "Tarjetas",
    flashcardsSuffix: " (Tarjetas)",
    sourceProperty: "Nota de origen",
    clozeSepName: "Separador de cloze",
    clozeSepDesc:
      "Separador dentro de un cloze para el número/respuesta/pista (por defecto \";;\"). También se acepta \"::\".",
    progressWriting: "Migrando {file}…",
    progressSyncing: "Sincronizando mazos…",
    progressRestoring: "Restaurando el historial de repasos…",
    inlineSepName: "Separador en línea",
    inlineSepDesc:
      "Separador de tarjetas en una sola línea (por defecto \"::\"). La forma invertida repite el último carácter.",
    multiSepName: "Separador multilínea",
    multiSepDesc:
      "Separador de tarjetas multilínea en su propia línea (por defecto \"?\"). La forma invertida repite el último carácter.",
    formatName: "Formato de migración",
    formatDesc:
      "Cómo distribuir las tarjetas migradas. El modo inteligente envía las tarjetas de una línea a tablas y las multilínea a encabezados.",
    formatSmart: "Detección inteligente automática (recomendado)",
    formatHeaders: "Todo en encabezados",
    formatTables: "Todo en tablas",
    formatTablesWarning:
      "Todo en tablas: las tarjetas multilínea se aplanan en celdas de tabla (los saltos de línea se convierten en <br>); puede que el contenido complejo no se muestre.",
    deleteForcesHeaders: "",
    deleteModeName: "Archivar los originales tras migrar",
    deleteModeDesc:
      "Mueve cada nota original a una carpeta _Legacy_SR_Archive después de migrar — se mantiene 100 % a salvo, solo fuera de tu espacio de trabajo activo.",
    deleteWarning:
      "Tus notas originales se mueven a _Legacy_SR_Archive (no se eliminan); elimina esa carpeta tú mismo cuando lo prefieras.",
    scanButton: "Analizar",
    scanning: "Analizando…",
    scanSummary:
      "Se encontraron {files} archivo(s), {cards} tarjeta(s), {history} con historial de repasos.",
    scanFailed: "El análisis falló. Consulta la consola para más detalles.",
    migrateButton: "Migrar",
    migrating: "Migrando…",
    migrateFailed: "La migración falló. Consulta la consola para más detalles.",
    successNotice:
      "Se migraron {cards} tarjeta(s) en {files} archivo(s); se restauró el historial de {history}.",
    doneSummary: "Listo: {files} archivo(s), {cards} tarjeta(s), {history} con historial.",
    cancel: "Cancelar",
  },

  occlusion: {
    studioTitle: "Editar oclusión de imagen",
    drawHint: "Arrastra sobre la imagen para dibujar un cuadro",
    boxes: "Cuadros",
    noBoxes: "Aún no hay cuadros. Arrastra sobre la imagen para dibujar uno.",
    selectBox: "Selecciona un cuadro para añadir su respuesta.",
    answerLabel: "Respuesta (Markdown / LaTeX)",
    answerPlaceholder: "Déjalo vacío para un cuadro solo de ocultación",
    emptyAnswer: "Vacío",
    deleteBox: "Eliminar cuadro",
    resize: "Cambiar tamaño",
    cancel: "Cancelar",
    save: "Guardar",
    edit: "Editar",
    imageNotFound: "Imagen no encontrada: {name}",
    parseError: "Error de oclusión de imagen: {error}",
    pickImage: "Selecciona una imagen para ocluir",
    noImages: "No se encontraron imágenes en el almacén.",
    saveFileNotFound: "No se pudo encontrar la nota para guardar la oclusión.",
    blockMoved: "No se pudo localizar el bloque de oclusión; puede que se haya movido.",
    saveFailed: "No se pudo guardar la oclusión.",
  },

  commands: {
    createExamDemoDeck: "Crear mazo de examen de demostración",
    cleanupOrphanedCards: "Limpiar tarjetas huérfanas (sin mazo)",
    migrateFromSr: "Migrar desde el plugin Spaced Repetition",
    importFromAnki: "Importar desde Anki (.apkg)",
    insertImageOcclusion: "Crear oclusión de imagen en el cursor",
    showPanel: "Mostrar panel de tarjetas",
    showReleaseNotes: "Mostrar notas de la versión",
    openManager: "Abrir gestor de tarjetas",
    createTestDeck: "Crear mazo de prueba",
    createCanvasTestDeck: "Crear mazo de prueba en canvas",
    fullResync:
      "Forzar resincronización completa (volver a analizar todos los mazos)",
    openReview: "Abrir ventana de repaso",
    openAiGenerator: "Generar tarjetas con IA",
  },

  notices: {
    orphansCleaned: "Se eliminaron {count} tarjeta(s) huérfana(s).",
    reparsing: "Reanalizando cada mazo. Esto puede tardar un momento…",
    resyncComplete: "Resincronización completa.",
    resyncFailed:
      "Resincronización fallida. Consulta la consola para más detalles.",
    loadFailed:
      "No se pudo cargar el plugin. Consulta la consola para más detalles.",
    languageChanged:
      "Idioma cambiado. Recarga el plugin para que tenga efecto.",
    noCardsInDeck: "No se encontraron tarjetas en {deckName}",
    customDeckCreated:
      'Mazo personalizado "{name}" creado con {count} tarjetas',
    customDeckUpdated: 'Mazo personalizado "{name}" actualizado',
    customDeckDeleted: 'Mazo personalizado "{name}" eliminado',
    customDeckRenamed: 'Mazo personalizado renombrado a "{name}"',
    deckResetSuccess: "Progreso del mazo reiniciado con éxito",
    deckResetFailed: "Error al reiniciar el progreso del mazo",
    reviewStartFailed: "Error al iniciar el repaso. Consulta la consola.",
    sessionComplete: "Sesión de repaso completada",
    sessionCompleteCount: "Se repasaron {count} tarjetas",
    duplicatesFound:
      "Se encontraron {count} tarjeta(s) duplicada(s) en {deckName}",
    trainedParamsCleared:
      "Parámetros entrenados borrados, valores por defecto restaurados.",
    creatingBackup: "Creando copia de seguridad…",
    backupCreated: "✅ Copia de seguridad creada: {filename}",
    backupFailed: "❌ Error al crear la copia de seguridad: {message}",
    selectBackupPrompt: "Selecciona una copia de seguridad para restaurar",
    restoringBackup: "Restaurando copia de seguridad…",
    restoringFromFile: "Restaurando desde {filename}…",
    restoringFromFileProgress: "Restaurando desde {filename}: {progress}%",
    restoreProgress:
      "Restaurando copia de seguridad: {progress}% ({current}/{total})",
    restoreFailed: "❌ Restauración fallida: {message}",
    cardsRebuilt: "Programación restaurada para {count} tarjetas desde el historial.",
    backupRestoreFailed: "❌ Restauración de copia fallida: {message}",
    dropdownNotFound: "Desplegable no encontrado",
    noBackupsFound: "No se encontraron copias de seguridad",
    backupsFound: "Se encontraron {count} copia(s) de seguridad",
    backupListFailed: "Error al cargar la lista de copias: {message}",
    purgingDatabase: "Purgando base de datos…",
    databasePurged: "✅ Base de datos purgada y reconstruida con éxito",
    purgeFailed: "Error al purgar la base de datos. Consulta la consola.",
    purgeConfirmMismatch:
      "El texto de confirmación no coincide. Purga cancelada.",
    errorLoadingDecks: "Error al cargar los mazos. Consulta la consola.",
    errorStartingReview: "Error al iniciar el repaso. Consulta la consola.",
    errorStartingBrowse:
      "Error al iniciar la exploración. Consulta la consola.",
    errorStartingCram: "Error al iniciar el repaso intensivo. Consulta la consola.",
    noCardsFoundInDeck: "No se encontraron tarjetas en {deckName}",
    noCardsDueForReview: "No hay tarjetas pendientes de repaso en {deckName}",
    noCardsFoundInGroup: 'No se encontraron tarjetas en "{name}"',
    noCardsDueInGroup: 'Sin tarjetas pendientes en "{name}" ({count} archivos)',
    noCardsDueInCustomDeck:
      'Sin tarjetas pendientes en "{name}" ({count} tarjetas)',
    fileNotFound: "Archivo no encontrado: {path}",
    reviewSessionCompleteFor: "¡Sesión de repaso completada para {deckName}!",
    filterDeckUpdated: 'Mazo de filtro "{name}" actualizado',
    customDeckUpdatedCounts:
      '"{name}" actualizado — {added} añadidas, {removed} eliminadas',
    failedToSave: "Error al guardar: {message}",
    failedToCreateDeck: "Error al crear el mazo: {message}",
    failedToCreateFilterDeck: "Error al crear el mazo de filtro: {message}",
    failedToAddCards: "Error al añadir tarjetas: {message}",
    filterDeckCreated: 'Mazo de filtro "{name}" creado',
    cardsAddedToCustomDeck: "{count} tarjetas añadidas al mazo personalizado",
    dailyLimitsReached: "\n\nLímites diarios alcanzados:",
    dailyNewLimit: "\nTarjetas nuevas: {used}/{max}",
    dailyReviewLimit: "\nTarjetas de repaso: {used}/{max}",
    dailyNewOnlyLimit:
      "\n\nLímite diario de tarjetas nuevas alcanzado: {used}/{max}",
    dailyReviewOnlyLimit:
      "\n\nLímite diario de tarjetas de repaso alcanzado: {used}/{max}",
    dailyProgressHeader: "Progreso diario de {deckName}:\n",
    dailyProgressNewDisabled:
      "Tarjetas nuevas: DESACTIVADAS (0 permitidas por día)\n",
    dailyProgressNewExceeded:
      "Tarjetas nuevas: {used}/{max} (LÍMITE SUPERADO)\n",
    dailyProgressNewRemaining:
      "Tarjetas nuevas: {used}/{max} ({remaining} restantes)\n",
    dailyProgressReviewDisabled:
      "Tarjetas de repaso: DESACTIVADAS (0 permitidas por día)\n",
    dailyProgressReviewExceeded:
      "Tarjetas de repaso: {used}/{max} (LÍMITE SUPERADO)\n",
    dailyProgressReviewRemaining:
      "Tarjetas de repaso: {used}/{max} ({remaining} restantes)\n",
    dailyProgressOnlyLearning:
      "\n\nNota: Solo se mostrarán tarjetas en aprendizaje (límites superados)",
    noFlashcardsToExport: "No hay tarjetas en este mazo para exportar",
    ankiExportSuccess:
      "{count} tarjetas exportadas desde {type} al formato Anki",
    ankiExportFailed: "Error al exportar al formato Anki",
    ankiExportTypeDeck: "mazo",
    ankiExportTypeTagGroup: "grupo de etiquetas",
  },

  examDeck: {
    filename: "Decks — Examen de demostración.md",
    title: "Examen de demostración",
    intro:
      "Esta nota es un mazo de examen: lleva la etiqueta {tag}, así que cada encabezado con una lista de tareas debajo se convierte en una pregunta de opción múltiple — marca las respuestas correctas con `[x]`. Haz clic en la fila del mazo (o usa su menú) y pulsa «Iniciar examen» para probarlo.",
    formatsHint:
      "A continuación están todas las formas de pregunta en orden: respuesta única, varias respuestas (calificadas a todo o nada), verdadero o falso, una respuesta corta escrita, filas de tabla y huecos (cloze). Abre este archivo en modo de código fuente para ver cómo se escribe cada una — un `%%comment%%` debajo de una pregunta se convierte en una explicación que se muestra en la revisión de resultados.",
    mcq1Front: "¿Qué elemento es un gas noble?",
    mcq1OptA: "Oxígeno",
    mcq1OptB: "Argón",
    mcq1OptC: "Nitrógeno",
    mcq1OptD: "Cloro",
    mcq1Note: "Los elementos del grupo 18 tienen la capa de valencia completa, lo que los hace inertes.",
    mcq2Front: "¿Cuáles de estos son gases nobles?",
    mcq2OptA: "Helio",
    mcq2OptB: "Oxígeno",
    mcq2OptC: "Argón",
    mcq2OptD: "Neón",
    tfFront: "La mitocondria es el lugar donde ocurre la fosforilación oxidativa.",
    tfTrue: "Verdadero",
    tfFalse: "Falso",
    saFront: "¿Cuál es la central energética de la célula?",
    saBack: "La mitocondria.",
    tableHeading: "Símbolos de los elementos",
    tColFront: "Anverso",
    tColBack: "Reverso",
    tColNotes: "Notas",
    tRow1Front: "¿Símbolo del oro?",
    tRow1Back: "Au",
    tRow1Notes: "Del latín aurum.",
    tRow2Front: "¿Símbolo del hierro?",
    tRow2Back: "Fe",
    tRow2Notes: "Del latín ferrum.",
    clozeHeading: "El agua",
    clozeBody: "El agua está compuesta de ==hidrógeno== y ==oxígeno==.",
    outro:
      "El diálogo «Configuración del examen» te permite elegir el número de preguntas, un límite de tiempo, la nota para aprobar y más. Los repasos y los exámenes registran sus resultados por separado, así que puedes usar ambos con las mismas tarjetas.",
  },
  exam: {
    startExam: "Iniciar examen",
    startReview: "Iniciar repaso",
    setupTitle: "Configuración del examen",
    eligibleCount: "{count} preguntas disponibles",
    skippedCount: "{count} omitidas (no válidas o no calificables)",
    skippedHint: "Revisa las tarjetas marcadas en el gestor de tarjetas.",
    noQuestions: "No hay preguntas de examen disponibles en esta selección.",
    questionCountSetting: "Número de preguntas",
    questionCountAll: "0 = todas",
    timeLimitSetting: "Límite de tiempo (minutos)",
    timeLimitOff: "0 = sin límite",
    passScoreSetting: "Nota mínima para aprobar (%)",
    shuffleQuestionsSetting: "Mezclar preguntas",
    shuffleOptionsSetting: "Mezclar opciones",
    feedbackTimingSetting: "Retroalimentación",
    feedbackEnd: "Al final",
    feedbackImmediate: "Después de cada pregunta",
    selectionModeSetting: "Selección de preguntas",
    selectionRandom: "Aleatoria",
    selectionSequential: "En el orden del archivo",
    typedGradingSetting: "Calificación de respuestas escritas",
    gradingExact: "Coincidencia exacta",
    gradingTolerant: "Tolerante con pequeñas erratas",
    gradingSelf: "Autoevaluación",
    optionLabelsSetting: "Etiquetas de las opciones",
    optionLabelsLetters: "Letras (a, b, c)",
    optionLabelsNumbers: "Números (1, 2, 3)",
    start: "Comenzar",
    cancel: "Cancelar",
    questionOf: "Pregunta {current} de {total}",
    typeAnswerPlaceholder: "Escribe tu respuesta",
    submitAnswer: "Enviar",
    selfPromptQuestion: "¿Acertaste?",
    selfYes: "Acerté",
    selfNo: "Me equivoqué",
    next: "Siguiente",
    previous: "Anterior",
    submitExam: "Entregar examen",
    quit: "Salir",
    quitConfirmTitle: "Abandonar examen",
    quitConfirmBody: "¿Abandonar este examen? No se guardará nada.",
    submitConfirmTitle: "Entregar examen",
    submitConfirmUnanswered: "{count} preguntas sin responder se calificarán como incorrectas.",
    confirm: "Confirmar",
    correct: "Correcta",
    incorrect: "Incorrecta",
    unanswered: "Sin responder",
    yourAnswer: "Tu respuesta",
    correctAnswer: "Respuesta correcta",
    resultsTitle: "Resultados",
    scoreLabel: "Puntuación",
    passed: "Aprobado",
    failed: "Suspendido",
    timeUsed: "Tiempo empleado",
    previousAttempts: "Intentos anteriores",
    retake: "Repetir",
    close: "Cerrar",
    notes: "Notas",
    invalidQuestionBadge: "Pregunta no válida",
    answerTooLongBadge: "No calificable",
    invalidReasonNoCorrect: "Ninguna respuesta correcta marcada",
    invalidReasonSingleOption: "Se necesitan al menos dos opciones",
    invalidReasonMixedList: "Lista mixta bajo el encabezado",
    invalidReasonNested: "Las listas de tareas anidadas no son compatibles",
    invalidReasonEmptyOption: "Texto de opción vacío",
    answerTooLongTooltip: "Respuesta demasiado larga para calificarla como texto — acórtala o cambia el modo de calificación del mazo",
    examEnabledSetting: "Preguntas de examen",
    examEnabledDesc: "Una lista de tareas bajo un encabezado crea una tarjeta de opción múltiple",
    examSettingsHeading: "Valores por defecto del examen",
    typeFlipWarningTitle: "Activar preguntas de examen",
    typeFlipWarningBody: "{count} tarjetas ya repasadas se convertirán en preguntas de examen. Su historial de repaso se aparca en un token inactivo y se restaura automáticamente si desactivas esta opción.",
    checkAnswer: "Comprobar la respuesta",
    selfGradePrompt: "¿Acertaste?",
    nextQuestion: "Siguiente pregunta",
    previousQuestion: "Pregunta anterior",
    question: "Pregunta",
    multipleCorrect: "Esta pregunta tiene más de una respuesta correcta.",
    timeRemaining: "Tiempo restante:",
    loadingExam: "Cargando el examen…",
    loadingAnswers: "Cargando las respuestas…",
    noStoredAnswers: "No se guardaron respuestas de este intento.",
    closeExam: "Cerrar el examen",
    everyQuestion: "Cada pregunta",
    settingCount: "Preguntas de este examen. 0 = todas las elegibles.",
    settingTimeLimit: "Cuenta atrás para todo el examen. 0 = sin límite.",
    settingPassMark: "Porcentaje necesario para aprobar (0–100).",
    settingSelection: "Cómo se eligen las preguntas del mazo.",
    settingFeedback: "Cuándo se muestran las respuestas.",
    settingGrading: "Cómo se comprueban las respuestas escritas. La autoevaluación admite respuestas más largas.",
    settingShuffleQuestions: "Aleatorizar el orden de las preguntas.",
    settingShuffleOptions: "Aleatorizar el orden de las opciones.",
    settingOptionLabels: "Cómo se etiquetan las opciones.",
    flagQuestion: "Marcar para volver",
    unflagQuestion: "Quitar la marca de esta pregunta",
    goToQuestion: "Ir a la pregunta {number}",
  },
  deckList: {
    srMigration: "Migrar desde Spaced Repetition",
    ankiImport: "Importar desde Anki",
    moreActions: "Más acciones",
    exportToAnki: "Exportar a Anki",
    title: "Mazos",
    emptyNoDecks: "No se encontraron mazos.",
    emptyNoFilterMatch: "Ningún mazo coincide con el filtro.",
    emptyFilterHint: "Prueba con otros términos de búsqueda.",
    filterPlaceholder: "Filtrar...",
    clearFilter: "Borrar filtro",
    search: "Buscar",
    refresh: "Actualizar",
    syncing: "Sincronizando en segundo plano…",
    aiGenerate: "Generar tarjetas con IA",
    settings: "Ajustes",
    statistics: "Ver estadísticas",
    profiles: "Gestionar perfiles",
    openManager: "Abrir gestor de tarjetas",
    configureDeck: "Configurar mazo",
    pinToTop: "Fijar arriba",
    unpinFromTop: "Desfijar",
    pinned: "Fijado",
    review: "Repasar",
    browse: "Explorar",
    browseAllCards: "Explorar todas las tarjetas",
    openSourceFile: "Abrir archivo de origen",
    cram: "Repaso intensivo",
    resumeCram: "Reanudar repaso intensivo",
    configureProfile: "Configurar perfil",
    rename: "Renombrar",
    delete: "Eliminar",
    editFilter: "Editar filtro",
    editCards: "Editar tarjetas",
    sortByName: "Ordenar por nombre",
    sortByNewCardCount: "Ordenar por tarjetas nuevas",
    sortByDueCardCount: "Ordenar por tarjetas pendientes",
    columnDeck: "Mazo",
    columnTagGroup: "Grupo de etiquetas",
    columnCustomDeck: "Mazo personalizado",
    columnNew: "Nuev.",
    columnDue: "Pend.",
    statTotal: "Total",
    statPastMonth: "Mes anterior",
    statPastWeek: "Semana anterior",
    optionsTagGroup: "Opciones de grupo de etiquetas",
    optionsCustomDeck: "Opciones de mazo personalizado",
    optionsDeck: "Opciones de mazo",
    optionsForItem: "Opciones para {name}",
    clickToReview: "Haz clic para repasar {name}",
    treeView: "Vista de árbol",
    flatView: "Vista plana",
    viewToggle: "Vista de lista de mazos",
    reviewAllUnder: "Repasar todas las tarjetas en {name}",
    studyAll: "Estudiar todo",
    collapseAll: "Contraer todo",
    collapse: "Contraer",
    expand: "Expandir",
    resetProgress: "Reiniciar progreso",
    deleteCustomDeckTitle: "Eliminar mazo personalizado",
    deleteCustomDeckMessage:
      '¿Eliminar el mazo personalizado "{name}"? Las tarjetas en sí no se eliminarán.',
    resetCustomDeckProgress:
      '¿Reiniciar todo el progreso de "{name}"? Todas las tarjetas volverán al estado nuevo y se borrará el historial de repasos. Esta acción no se puede deshacer.',
    progressResetFor: 'Progreso reiniciado para "{name}"',
    resetCustomDeckTitle: "Reiniciar progreso del mazo personalizado",
    resetCustomDeckMessage:
      '¿Reiniciar progreso de las tarjetas en "{name}"? Se borrará el historial de estas tarjetas y volverán al estado nuevo.',
    resetCustomDeckFailed: "Error al reiniciar el mazo personalizado",
    confirmDelete: "Eliminar",
    newCardsTooltip: "Tarjetas nuevas restantes hoy: {remaining} / {limit}",
    reviewCardsTooltip:
      "Tarjetas de repaso restantes hoy: {remaining} / {limit}",
    tabFiles: "Archivos",
    tabTags: "Etiquetas",
    tabCustom: "Personalizado",
    tagYourNotes: "Etiqueta tus notas con {tag} para crear mazos.",
    filesCount: "({count} archivos)",
    cardsCount: "({count} tarjetas)",
    newCardsGroupTooltip:
      "{count} tarjetas nuevas disponibles hoy (límite: {limit} por mazo)",
    newCardsLimitTooltip:
      "{count} tarjetas nuevas disponibles hoy (límite: {limit})",
    newCardsDueTooltip: "{count} tarjetas nuevas pendientes",
    reviewCardsGroupTooltip:
      "{count} tarjetas de repaso disponibles hoy (límite: {limit} por mazo)",
    reviewCardsLimitTooltip:
      "{count} tarjetas de repaso disponibles hoy (límite: {limit})",
    reviewCardsDueTooltip: "{count} tarjetas de repaso pendientes",
    studiedSummary: "Hoy estudiaste {cards} tarjetas en {hours} ({pace})",
    statCardsToday: "Tarjetas de hoy",
  },

  review: {
    showAnswer: "Mostrar respuesta",
    showAnswerButton: "Mostrar respuesta",
    again: "Otra vez",
    hard: "Difícil",
    good: "Bien",
    easy: "Fácil",
    edit: "Editar tarjeta",
    delete: "Eliminar tarjeta",
    skip: "Saltar",
    undo: "Deshacer",
    undoTooltip: "Deshacer último repaso (Cmd/Ctrl+Z)",
    flag: "Marcar tarjeta",
    unflag: "Quitar marca",
    suspend: "Suspender",
    bury: "Enterrar",
    reset: "Restablecer tarjeta",
    unsuspend: "Reanudar",
    unbury: "Desenterrar",
    cardActions: "Acciones de tarjeta",
    cardSuspended: "Tarjeta suspendida",
    cardUnsuspended: "Tarjeta reanudada",
    cardBuried: "Tarjeta enterrada hasta mañana",
    cardReset: "Progreso de la tarjeta restablecido",
    resetCardConfirmTitle: "¿Restablecer tarjeta?",
    resetCardConfirmMessage:
      "¿Restablecer el progreso de esta tarjeta? Se eliminará el historial de repasos y la tarjeta volverá al estado nuevo. Esto no se puede deshacer.",
    close: "Cerrar",
    front: "Anverso",
    back: "Reverso",
    progress: "Progreso",
    sessionEnded: "Sesión finalizada",
    sessionStats: "{reviewed} repasadas • {remaining} restantes",
    noMoreCards: "No hay tarjetas para repasar",
    keyboardHint: "Pulsa 1-4 para calificar, Espacio para voltear",
    timeUp: "Tiempo de sesión agotado",
    revealCloze: "Revelar hueco",
    nextCloze: "Siguiente hueco",
    openSource: "Abrir nota de origen",
    openSourceFile: "Abrir archivo de origen",
    playAudio: "Leer en voz alta",
    stopAudio: "Detener lectura",
    cardLeechAndDense:
      "Tarjeta olvidada repetidamente y compleja — considera revisarla",
    cardLeech: "Tarjeta olvidada repetidamente — considera reescribirla",
    cardDense: "La tarjeta es compleja. Considera dividirla.",
    copyContent: "Copiar contenido",
    toggleNotes: "Alternar notas (N)",
    scratchpad: "Bloc de notas",
    scratchpadPen: "Lápiz",
    scratchpadEraser: "Borrador",
    scratchpadText: "Texto",
    scratchpadClear: "Borrar",
    scratchpadTextPlaceholder: "Escribe notas…",
    searchCardsPlaceholder: "Buscar tarjetas...",
    noCardsMatch: "Ninguna tarjeta coincide",
    closeSearch: "Cerrar búsqueda",
    searchCards: "Buscar tarjetas",
    cardOfCount: "Tarjeta {current} / {total}",
    cardOfCountAlt: "Tarjeta {current} de {total}",
    reviewedLabel: "Repasadas: {count}",
    remainingCount: "{count} restantes",
    moreCardsAvailable: "Más tarjetas disponibles",
    sessionComplete: "Sesión completa",
    timeRemaining: "Tiempo restante:",
    finish: "Terminar",
    next: "Siguiente",
    previous: "Anterior",
    spaceShortcut: "Espacio",
    copyAnswer: "Copiar el reverso",
    editMasks: "Editar las máscaras",
    refactorInstruction: "Instrucción de reescritura",
    refactorPlaceholder: "¿Cómo debe cambiar esta tarjeta? p. ej. acortar el reverso, corregir el LaTeX",
    refactorRewritten: "Reescrito arriba: revísalo y luego guarda.",
    refactorSaved: "Guardado en la nota de origen y vuelto a sincronizar.",
    clozeHint: "Envuelve cada respuesta en ==dobles iguales== para que siga siendo un cloze.",
    typeEditedElsewhere: "Este tipo de tarjeta se edita donde se creó, no durante el repaso.",
    loading: "Cargando el repaso…",
    backToDecks: "Volver a los mazos",
    closeReview: "Cerrar el repaso",
    closeBrowse: "Cerrar la exploración",
    closeCram: "Cerrar el repaso intensivo",
    loadingCram: "Cargando el repaso intensivo…",
    nothingToBrowse: "Nada que explorar",
    deckHasNoCards: "Este mazo aún no tiene tarjetas.",
    nextCard: "Tarjeta siguiente",
    previousCard: "Tarjeta anterior",
    card: "Tarjeta",
    timeRemainingLabel: "Tiempo restante:",
    youHeld: "Has aguantado",
    ttsRateSaved: "Guardado en el perfil del mazo, así la próxima sesión empieza aquí.",
    examDrawDesc: "Los exámenes usan tarjetas de opción múltiple y de escritura de mazos cuyo perfil tiene activadas las preguntas de examen.",
    deckDetail: "Detalle del mazo",
    selectDeckPrompt: "Elige un mazo para ver sus cifras y opciones de estudio.",
    dueNext14: "Tarjetas pendientes en los próximos 14 días",
    next14Days: "Próximos 14 días",
    youHold: "Mantienes",
    hideAnswer: "Ocultar respuesta",
    deleteCardConfirm: "¿Eliminar esta tarjeta de la base de datos de tarjetas?",
    fileOrder: "Orden del archivo",
    clozeLabel: "Hueco",
    maskLabel: "Máscara",
  },

  manager: {
    title: "Gestor de tarjetas",
    searchPlaceholder: "Buscar tarjetas…",
    filter: "Filtro",
    sort: "Ordenar",
    edit: "Editar",
    delete: "Eliminar",
    selectAll: "Seleccionar todo",
    deselectAll: "Deseleccionar todo",
    bulkActions: "Acciones en lote",
    exportSelection: "Exportar selección",
    suspendSelected: "Suspender seleccionadas",
    unsuspendSelected: "Reanudar seleccionadas",
    burySelected: "Enterrar seleccionadas",
    unburySelected: "Desenterrar seleccionadas",
    resetSelected: "Restablecer seleccionadas…",
    suspendedBadge: "Suspendida",
    buriedBadge: "Enterrada",
    suspendedTooltip: "Suspendida (solo repaso manual)",
    buriedUntilTooltip: "Enterrada hasta {date}",
    resetSelectedConfirmTitle: "¿Restablecer las tarjetas seleccionadas?",
    resetSelectedConfirmMessage:
      "¿Restablecer el progreso de {count} tarjetas? Se eliminará todo el historial de repasos de estas tarjetas. Esto no se puede deshacer.",
    cardActions: "Acciones de tarjeta",
    deleteSelected: "Eliminar seleccionadas",
    cardsSelected: "{count} tarjetas seleccionadas",
    noResults: "Ninguna tarjeta coincide con el filtro actual",
    columnFront: "Anverso",
    columnBack: "Reverso",
    columnDeck: "Mazo",
    columnDue: "Pendiente",
    columnState: "Estado",
    columnStability: "Estabilidad",
    columnDifficulty: "Dificultad",
    columnLapses: "Lapsus",
    columnReviewed: "Repasada",
    stateNew: "Nueva",
    stateLearning: "Aprendiendo",
    stateReview: "Repaso",
    stateRelearning: "Reaprendiendo",
    rowsPerPage: "Filas por página",
    of: "de",
    page: "Página",
    first: "Primera",
    previous: "Anterior",
    next: "Siguiente",
    last: "Última",
    openInVault: "Abrir en el vault",
    deckTagDescription: "Mazo",
    createCustomDeck: "Crear mazo personalizado",
    customDeckName: "Nombre del mazo",
    customDeckNamePlaceholder: "Introduce el nombre",
    saveCustomDeck: "Guardar mazo",
    addFilter: "Añadir filtro",
    removeFilter: "Quitar filtro",
    clearFilters: "Borrar filtros",
    loadingFlashcards: "Cargando tarjetas...",
    editLabel: "Editar:",
    browseAllOption: "— Ver todas las tarjetas —",
    saveTooltip: "Guardar cambios en {name}",
    discardTooltip: "Descartar edición y volver al modo de exploración",
    saveButton: "Guardar",
    cancelButton: "Cancelar",
    searchPlaceholderFull: "Buscar archivo, ruta, anverso, reverso...",
    clearSearch: "Borrar búsqueda",
    filterButton: "+ Filtro",
    filterButtonCount: "+ Filtro ({count})",
    removeFilterAria: "Quitar filtro",
    searchOnlyView: "Vista de solo búsqueda (guardar deshabilitado)",
    filterDeckNamePlaceholder: "Nombre del mazo de filtro",
    saveAsFilterDeck: "💾 Guardar como mazo de filtro",
    saveAsTitleHasFilter: "Guardar el filtro actual como un mazo de filtro",
    saveAsTitleNoFilter: "Añade una regla de filtro para guardar",
    selectedCount: "{count} seleccionadas",
    addToCustomDeck: "Añadir a mazo personalizado",
    createNewDeckOption: "+ Crear nuevo mazo...",
    newDeckPlaceholder: "Nombre del mazo",
    createButton: "Crear",
    colFront: "Anverso",
    colBack: "Reverso",
    colHint: "Pista",
    colNotes: "Notas",
    colFile: "Archivo",
    colBreadcrumb: "Ruta",
    colDeckTag: "Etiqueta del mazo",
    colCardTags: "Etiquetas de tarjeta",
    colState: "Estado",
    colHealth: "Salud",
    colDue: "Vencimiento",
    colReviewed: "Repasada",
    badgeNew: "Nueva",
    badgeReview: "Repaso",
    badgeLeech: "Sanguijuela",
    badgeDense: "Densa",
    badgeHealthy: "Saludable",
    leechTooltip:
      "Olvidada repetidamente ({count} lapsus) — considera reescribirla",
    denseTooltip: "El reverso tiene {count} caracteres — considera dividirla",
    healthyTooltip: "Sin problemas de sanguijuela o densidad detectados",
    footerShowingOfInDeck: 'Mostrando {shown} de {total} tarjetas en "{name}"',
    footerInDeck: '{total} tarjetas en "{name}"',
    footerShowingOf: "Mostrando {shown} de {total} tarjetas",
    footerTotal: "{total} tarjetas",
    loadMore: "Cargar más ({remaining} restantes)",
    addToDeck: "Añadir al mazo",
    createAndAdd: "Crear y añadir",
    createDeck: "Crear un mazo",
    deckScope: "Ámbito del mazo",
    sortOrder: "Orden",
    sortDueSoonest: "Vencen antes",
    sortFrontAZ: "Anverso A–Z",
    sortRecent: "Repasadas hace poco",
    filters: "Filtros",
    selectNone: "No seleccionar nada",
    source: "Origen",
    loadingCards: "Cargando tarjetas…",
    loadingDecks: "Cargando mazos…",
    noCardsMatch: "Ninguna tarjeta coincide con tus filtros.",
    noCardsYet: "Todavía no hay tarjetas.",
    tagToCreate: "Etiqueta tus notas para convertirlas en mazos y tarjetas.",
    noCustomDecks: "Todavía no hay mazos personalizados: crea uno abajo.",
    filterDeckHint: "Parte de los filtros que muestra Explorar. Un mazo de filtro se mantiene al día solo.",
    noRulesMatchAll: "Sin reglas, este mazo incluye todas las tarjetas.",
    typeEditedElsewhereList: "Este tipo de tarjeta se edita donde se creó, no desde la lista.",
    reset: "Restablecer",
    deckType: "Tipo de mazo",
    manual: "Manual",
    manualDesc: "Elige tú las tarjetas",
    rulesDesc: "Tarjetas que cumplen las reglas, siempre al día",
    rules: "Reglas",
    matchingCards: "Tarjetas coincidentes",
    selectAllShown: "Seleccionar todo lo mostrado",
    selectedOnly: "Solo lo seleccionado",
    noCardsSearch: "Ninguna tarjeta coincide con tu búsqueda.",
    noCardsSyncFirst: "Aún no hay tarjetas: sincroniza primero un mazo.",
    namePlaceholder: "p. ej. Repaso de examen",
    cardDetail: "Detalle de la tarjeta",
    backToCards: "Volver a las tarjetas",
    selectCardHere: "Elige una tarjeta para verla aquí.",
  },

  config: {
    title: "Configuración del mazo",
    profile: "Perfil FSRS",
    profileLabel: "Perfil",
    profileSelectDesc: "Selecciona un perfil para esta etiqueta",
    profileNameLabel: "Nombre del perfil",
    profileStandard: "Estándar",
    profileIntensive: "Intensivo",
    newCardsLimitLabel: "Límite de tarjetas nuevas",
    reviewCardsLimitLabel: "Límite de tarjetas de repaso",
    perDay: "{count} por día",
    unlimited: "Sin límite",
    headerLevelLabel: "Nivel de encabezado",
    headerTitle: "Título",
    headerH: "H{level}",
    reviewOrderLabel: "Orden de repaso",
    reviewOrderOldestDue: "Más antiguas primero",
    reviewOrderRandomLabel: "Aleatorio",
    clozeDeletions: "Huecos",
    clozeEnabled: "Activado (contexto: {mode})",
    clozeShowOthers: "mostrar otros huecos",
    clozeHideAll: "ocultar todos los huecos",
    clozeDisabled: "Desactivado",
    fsrsSettings: "Ajustes FSRS",
    fsrsSettingsDesc: "Retención: {retention}, Perfil: {profile}",
    decksUsingProfile: "Mazos que usan el perfil",
    deckCount: "{count} mazo(s)",
    editNoteIntro: "Nota:",
    editNoteBody:
      " Para editar los ajustes del perfil, usa el botón «Gestionar perfiles» en el panel principal.",
    sectionTagSelection: "Selección de etiqueta",
    sectionProfileSelection: "Selección de perfil",
    sectionCurrentSettings: "Ajustes del perfil actual",
    saving: "Guardando…",
    applyProfileToTag: "Aplicar perfil a la etiqueta",
    applyProfileDesc:
      "Selecciona una etiqueta para aplicar el perfil a todos sus mazos",
    errorSavingConfig: "Error al guardar la configuración",
    requestRetention: "Retención solicitada",
    requestRetentionDesc: "Probabilidad objetivo de recuerdo (0,7-0,99)",
    maxIntervalDays: "Intervalo máximo (días)",
    maxIntervalDaysDesc: "Límite del intervalo programado",
    learningSteps: "Pasos de aprendizaje",
    relearningSteps: "Pasos de reaprendizaje",
    newCardsPerDay: "Tarjetas nuevas por día",
    newCardsPerDayDesc:
      "Número máximo de tarjetas nuevas al día (0 desactiva el límite)",
    reviewsPerDay: "Repasos por día",
    reviewsPerDayDesc:
      "Número máximo de tarjetas de repaso al día (0 desactiva el límite)",
    reviewOrder: "Orden de repaso",
    reviewOrderDueDate: "Fecha de vencimiento",
    reviewOrderRandom: "Aleatorio",
    headerLevel: "Nivel de encabezado para tarjetas",
    headerLevelDesc:
      "Nivel de encabezado tratado como anverso en mazos de encabezado-párrafo",
    useTrainedWeights: "Usar pesos entrenados",
    useTrainedWeightsDesc:
      "Aplica los pesos FSRS entrenados globalmente a este mazo (solo perfil estándar).",
    save: "Guardar",
    cancel: "Cancelar",
    reset: "Restablecer valores predeterminados",
    enableDailyLimits: "Activar límites diarios",
    enableDailyLimitsDesc:
      "Limita el número de tarjetas nuevas y de repaso al día",
  },

  profiles: {
    refactorPromptLabel: "Prompt de sistema de IA",
    refactorPromptDesc: "Instrucciones que sigue la IA al refactorizar tarjetas en mazos que usan este perfil. Déjalo vacío para usar el valor predeterminado.",
    refactorPromptPlaceholder: "p. ej. Haz las respuestas concisas y corrige la gramática.",
    sectionAiRefactor: "Prompt de sistema de IA",
    title: "Perfiles FSRS",
    addProfile: "Añadir perfil",
    editProfile: "Editar perfil",
    deleteProfile: "Eliminar perfil",
    duplicateProfile: "Duplicar perfil",
    profileName: "Nombre del perfil",
    profileNamePlaceholder: "Introduce el nombre",
    save: "Guardar perfil",
    cancel: "Cancelar",
    tagMappings: "Asignaciones de etiquetas",
    addTagMapping: "Añadir asignación",
    removeTagMapping: "Quitar asignación",
    tagPlaceholder: "Etiqueta (p. ej. #decks/math)",
    noProfiles: "Aún no hay perfiles personalizados",
    deleteConfirm:
      "¿Eliminar este perfil? Los mazos que lo usan volverán al perfil predeterminado.",
    cannotDeleteDefault: "Los perfiles predeterminados no se pueden eliminar.",
    selectProfile: "Seleccionar perfil",
    chooseProfileDesc: "Elige un perfil para ver y editar",
    createNewProfile: "Crear nuevo perfil",
    createTooltip: "Crear un nuevo perfil",
    profileSettings: "Ajustes del perfil",
    profileNameDesc: "Nombre de este perfil",
    sectionDailyLimits: "Límites diarios",
    sectionNewCards: "Tarjetas nuevas",
    sectionLapses: "Lapsus",
    sectionCardParsing: "Análisis de tarjetas",
    sectionReviewSettings: "Ajustes de repaso",
    sectionFsrsAlgorithm: "Algoritmo FSRS",
    sectionProfileInfo: "Info del perfil",
    sectionReadAloud: "Lectura en voz alta",
    ttsVoiceLabel: "Voz",
    ttsVoiceDesc: "Voz utilizada por el botón de lectura en voz alta durante el repaso",
    ttsVoiceDefault: "Predeterminada del sistema",
    ttsRateLabel: "Velocidad",
    ttsRateDesc: "Velocidad de reproducción de la lectura en voz alta",
    ttsUnavailable: "La lectura en voz alta no está disponible en este dispositivo",
    limitNewCardsLabel: "Limitar tarjetas nuevas por día",
    limitNewCardsDesc: "Activar límite diario de tarjetas nuevas",
    newCardsPerDayLabel: "Tarjetas nuevas por día",
    newCardsPerDayDesc:
      "Máximo de tarjetas nuevas a introducir por día (0-9999)",
    limitReviewCardsLabel: "Limitar tarjetas de repaso por día",
    limitReviewCardsDesc: "Activar límite diario de tarjetas de repaso",
    reviewCardsPerDayLabel: "Tarjetas de repaso por día",
    reviewCardsPerDayDesc: "Máximo de tarjetas de repaso por día (1-9999)",
    againIntervalNew: "Intervalo de Otra vez",
    againIntervalNewDesc:
      "Intervalo al pulsar Otra vez en una tarjeta nueva (p. ej. 1m)",
    againIntervalReview: "Intervalo de Otra vez",
    againIntervalReviewDesc:
      "Intervalo al pulsar Otra vez en una tarjeta de repaso (p. ej. 10m)",
    headerLevelLabel: "Nivel de encabezado",
    headerLevelDescParsing: "Nivel de encabezado para el análisis",
    extraHeaderLevelsLabel: "Niveles de encabezado adicionales",
    extraHeaderLevelsDesc: "Analiza también estos niveles de encabezado como tarjetas (las tarjetas nuevas e importadas siguen usando el nivel principal de arriba)",
    clozeDeletionsLabel: "Huecos",
    clozeDeletionsDesc: "Generar tarjetas con huecos desde texto ==resaltado==",
    clozeContextLabel: "Contexto de huecos",
    clozeContextDesc: "Cómo aparecen los huecos no evaluados durante el repaso",
    clozeShowOption: "Mostrar otros huecos",
    clozeHideOption: "Ocultar todos los huecos",
    reviewOrderLabel: "Orden de repaso",
    reviewOrderDesc: "Orden en que se repasan las tarjetas",
    requestRetentionLabel: "Retención solicitada",
    requestRetentionDesc: "Tasa de retención objetivo (0,5 - 0,995)",
    fsrsProfileLabel: "Perfil FSRS",
    fsrsTrainedDesc:
      "Estándar e intensivo usan pesos de fábrica. Entrenado aplica tus pesos optimizados (intervalos estándar).",
    fsrsUntrainedDesc:
      "Perfil de intensidad de aprendizaje. Entrena los pesos en Ajuste del algoritmo para habilitar la opción Entrenado.",
    fsrsStandardOption: "Estándar",
    fsrsIntensiveOption: "Intensivo",
    fsrsTrainedOption: "Entrenado",
    fsrsTrainedUnavailable: "Entrenado (no disponible)",
    decksUsingProfile: "Mazos que usan el perfil",
    deckCount: "{count} mazo(s)",
    tagAssignmentsHeading: "Asignaciones de etiquetas",
    saveChanges: "Guardar cambios",
    savingChanges: "Guardando...",
    deleteProfileButton: "Eliminar perfil",
    close: "Cerrar",
    noticeProfileCreated: "Nuevo perfil creado",
    noticeProfileNameEmpty: "El nombre del perfil no puede estar vacío",
    noticeNewCardsRange:
      "Las tarjetas nuevas por día deben estar entre 0 y 9999",
    noticeReviewCardsRange:
      "Las tarjetas de repaso por día deben estar entre 1 y 9999",
    noticeRequestRetentionRange:
      "La retención solicitada debe estar entre 0,5 y 0,995",
    noticeInvalidAgainInterval: "Intervalo de Otra vez no válido",
    noticeReadAloudRange: 'La velocidad de lectura debe estar entre {min} y {max}',
    noticeInvalidLapseInterval: 'Intervalo de recaída no válido',
    noticeQuestionCountRange: 'El número de preguntas debe ser 0 o más',
    noticeTimeLimitRange: 'El límite de tiempo debe ser 0 o más',
    noticePassScoreRange: 'La puntuación mínima para aprobar debe estar entre 0 y 100',
    noticeProfileSaved: "Perfil guardado con éxito",
    noticeProfileSaveError: "Error al guardar el perfil",
    noticeCannotDeleteDefault: "No se puede eliminar el perfil PREDETERMINADO",
    noticeProfileDeleted: "Perfil eliminado",
    confirmDeletePrompt:
      '¿Seguro que quieres eliminar el perfil "{name}"?\n\nTodos los mazos que usan este perfil se restablecerán al perfil PREDETERMINADO.',
    modalTitle: "Perfiles",
    modalSubtitle: "Configura un perfil y elige qué mazos lo usan, todo en un solo lugar.",
    tabSettings: "Ajustes",
    tabAssignments: "Asignaciones",
    activeProfileLabel: "Perfil activo",
    newProfileButton: "Nuevo",
    newProfileName: "Nuevo perfil",
    assignmentsHeading: "Mazos y etiquetas que usan {name}",
    assignmentsExplainer: "Aplica este perfil a una etiqueta y todos los mazos bajo ella heredan estos ajustes. Un mazo solo puede usar un perfil.",
    assignTagPlaceholder: "Elige una etiqueta para aplicar…",
    applyButton: "Aplicar",
    assignmentsEmpty: "Ningún mazo usa este perfil todavía. Aplícalo a una etiqueta arriba.",
    assignmentMeta: "{decks} mazos · {cards} tarjetas",
    effectiveSettingsHeading: "Ajustes efectivos",
    removeAssignmentConfirm: "¿Quitar este perfil de «{tag}»? Sus mazos volverán al perfil heredado.",
    noticeProfileDuplicated: "Perfil duplicado",
    copySuffix: " copia",
    defaultSuffix: "(PREDETERMINADO)",
    nameDesc: "Nombre de este perfil.",
    headerLevelDesc: "Nivel de encabezado que se lee como anverso.",
    clozeDesc: "Generar tarjetas cloze del texto ==resaltado==.",
    examDesc: "Una lista de tareas bajo un encabezado se convierte en una tarjeta de opción múltiple.",
    newLimitDesc: "Activar un límite diario de tarjetas nuevas.",
    reviewLimitDesc: "Activar un límite diario de repasos.",
    newLimitMaxDesc: "Máximo de tarjetas nuevas al día (0–9999).",
    reviewLimitMaxDesc: "Máximo de repasos al día (1–9999).",
    retentionDesc: "Tasa de retención objetivo (0,5–0,995).",
    newAgainInterval: "Intervalo de «Otra vez» en tarjetas nuevas",
    newAgainIntervalDesc: "Intervalo al pulsar «Otra vez» en una tarjeta nueva (p. ej. 1m).",
    lapseAgainInterval: "Intervalo de «Otra vez» tras un olvido",
    lapseAgainIntervalDesc: "Intervalo al pulsar «Otra vez» en un repaso (p. ej. 10m).",
    intervals: "Intervalos",
    clozeMaskDesc: "Cómo aparecen los cloze no evaluados durante el repaso.",
    ttsLanguageDesc: "Idioma en que lee la voz del sistema. Automático sigue al dispositivo.",
    ttsPreview: "Escuchar la voz y la velocidad actuales.",
    ttsVoiceMissing: "La voz guardada no está instalada en este dispositivo, así que se usa el idioma de abajo.",
    automatic: "Automático",
    random: "Aleatorio",
    sections: "Secciones del perfil",
    loading: "Cargando perfiles…",
    noneYet: "Todavía no hay perfiles.",
    availableWhenDb: "Los perfiles aparecen cuando la base de datos esté disponible.",
    tagToApply: "Etiqueta a aplicar",
    tagDesc: "Aplica este perfil a una etiqueta y cada mazo bajo ella hereda estos ajustes. Un mazo solo puede usar un perfil.",
    duplicate: "Duplicar",
    examsHeading: "Exámenes",
  },

  statistics: {
    title: "Estadísticas",
    overall: "General",
    deck: "Mazo",
    cardCounts: "Recuento de tarjetas",
    cardCountsNew: "Nuevas",
    cardCountsLearning: "Aprendiendo",
    cardCountsReview: "Repaso",
    cardCountsRelearning: "Reaprendiendo",
    cardCountsTotal: "Total",
    cardCountsYoung: "Jóvenes",
    cardCountsMature: "Maduras",
    cardsAddedSeries: "Tarjetas añadidas",
    againSeries: "Otra vez",
    hardSeries: "Difícil",
    goodSeries: "Bien",
    easySeries: "Fácil",
    reviewsOnDate: "{count} repasos el {date}",
    totalReviewsCount: "{count} repasos",
    cardDifficulty: "Dificultad de tarjetas",
    cardStability: "Estabilidad de tarjetas",
    cardRetrievability: "Recuperabilidad",
    cardsAdded: "Tarjetas añadidas",
    futureDue: "Tarjetas próximas a vencer",
    hourlyBreakdown: "Desglose por hora",
    maturityProgression: "Progresión de madurez",
    reviewHeatmap: "Mapa de calor de repasos",
    reviewIntervals: "Intervalos de repaso",
    reviewsOverTime: "Repasos en el tiempo",
    trueRetention: "Retención real",
    legendDays: "días",
    legendCount: "recuento",
    legendCards: "tarjetas",
    legendReviews: "repasos",
    legendAccuracy: "precisión",
    daysAgo: "hace {count} días",
    today: "Hoy",
    yesterday: "Ayer",
    tomorrow: "Mañana",
    inDays: "En {count} días",
    rangeWeek: "Semana",
    rangeMonth: "Mes",
    rangeYear: "Año",
    rangeAll: "Todo",
    matureCards: "Maduras ({count}+ días)",
    youngCards: "Jóvenes (<{count} días)",
    againRate: "Tasa de Otra vez",
    hardRate: "Tasa de Difícil",
    goodRate: "Tasa de Bien",
    easyRate: "Tasa de Fácil",
    retention: "Retención",
    retentionYoung: "Retención (jóvenes)",
    retentionMature: "Retención (maduras)",
    noData: "No hay datos disponibles",
    elapsedDays: "Días transcurridos",
    intervalDays: "Intervalo (días)",
    countLabel: "Recuento",
    averageDifficulty: "Dificultad media",
    averageStability: "Estabilidad media",
    averageRetrievability: "Recuperabilidad media",
    totalReviews: "Repasos totales",
    totalCards: "Tarjetas totales",
    studyTime: "Tiempo de estudio",
    streak: "Racha",
    currentStreak: "Racha actual",
    longestStreak: "Racha más larga",
    daysStudied: "Días estudiados",
    minutesPerDay: "Min/día",

    // Chart strings — DRAFT translations
    cardDistribution: "Distribución de tarjetas",
    cardDistributionByState: "Distribución de tarjetas por estado",
    cardDistributionByStateDoughnut:
      "Distribución de tarjetas por estado (donut)",
    cardCountsTooltip: "{label}: {count} tarjetas ({percent}%)",
    selectDeckCardDistribution: "Selecciona un mazo para ver la distribución.",
    cardDifficultyDistribution: "Distribución de dificultad",
    cardDifficultyRange: "Rango de dificultad",
    cardDifficultySubtitle:
      "Los valores de dificultad de FSRS indican cuán difíciles son las tarjetas",
    higherDifficultyTip: "Mayor dificultad = más difícil de recordar",
    numberOfCards: "Número de tarjetas",
    numberOfReviews: "Número de repasos",
    selectDeckCardDifficulty:
      "Selecciona un mazo para ver la distribución de dificultad.",
    cardStabilityDistribution: "Distribución de estabilidad",
    cardStabilityRange: "Rango de estabilidad",
    cardStabilitySubtitle:
      "Los valores de estabilidad de FSRS muestran qué tan bien se retienen las tarjetas en memoria",
    selectDeckCardStability:
      "Selecciona un mazo para ver la distribución de estabilidad.",
    cardRetrievabilityDistribution: "Distribución de recuperabilidad",
    retrievabilityRange: "Rango de recuperabilidad",
    reviewsLabel: "Repasos",
    cardRetrievabilitySubtitle:
      "Los valores de recuperabilidad de FSRS muestran la probabilidad de recuerdo hoy (0-100%)",
    higherRetrievabilityTip: "Mayor recuperabilidad = más fácil de recordar",
    reviewsTooltip: "{label}: {count} repasos ({percent}%)",
    selectDeckCardRetrievability:
      "Selecciona un mazo para ver la distribución de recuperabilidad.",
    cardsAddedOverTime: "Tarjetas añadidas en el tiempo",
    dateAdded: "Fecha de adición",
    selectDeckCardsAdded:
      "Selecciona un mazo para ver las tarjetas añadidas en el tiempo.",
    cardsAddedTooltip: "{label}: {count} {plural}",
    cardSingular: "tarjeta",
    cardPlural: "tarjetas",
    basedOnFirstReview: "Basado en la fecha del primer repaso",
    timeframeLabel: "Periodo:",
    timeframe1Month: "1 mes",
    timeframe3Months: "3 meses",
    timeframe1Year: "1 año",
    allTime: "Todo",
    futureDueTitle: "Próximas a vencer",
    selectDeckFutureDue: "Selecciona un mazo para ver los próximos repasos.",
    dueCards: "Tarjetas pendientes",
    cumulativeBacklog: "Acumulado pendiente",
    totalReviewsLabel: "Repasos totales:",
    avgPerDay: "Promedio/día:",
    dueTomorrow: "Vencen mañana:",
    futureDueSubtitle: "Número de repasos pendientes en el futuro.",
    backlogControl: "Acumulado",
    onemonth: "1 mes",
    threeMonths: "3 meses",
    oneYear: "1 año",
    all: "todo",
    cumulativeSeries: "Acumulado",
    dayLabel: "Día {label}",
    totalBacklogTooltip: "Acumulado total: {value} repasos",
    dueTooltip: "Pendientes: {value} tarjetas",
    noUpcomingReviews:
      "No hay repasos programados. Añade tarjetas a tus mazos para ver el pronóstico.",
    hourlyBreakdownTitle: "Actividad por hora",
    hourlyBreakdownChart: "Actividad de repaso por hora del día",
    reviewCountLabel: "Cantidad de repasos",
    hourOfDay: "Hora del día",
    successRatePercent: "Tasa de éxito (%)",
    selectDeckHourly: "Selecciona un mazo para ver la actividad por hora.",
    hourlySuccessRateTooltip: "{label}: {value}%",
    hourlyReviewsTooltip: "{label}: {value} repasos",
    timePeriod: "Periodo del día: {period}",
    morning: "Mañana",
    afternoon: "Tarde",
    evening: "Noche",
    night: "Madrugada",
    maturityForecast: "Pronóstico de madurez",
    cardMaturityProgression: "Progresión de madurez",
    daysFromNow: "Días desde ahora",
    newCardsLabel: "Tarjetas nuevas",
    matureCardsLabel: "Tarjetas maduras",
    learningCardsLabel: "Tarjetas en aprendizaje",
    selectDeckMaturity: "Selecciona un mazo para ver el pronóstico de madurez.",
    calculating: "Calculando…",
    maturityRetentionNote:
      "Según tu retención histórica de {percent}%, así madurarán tus tarjetas:",
    maturityAllMatureIn:
      "Todas las tarjetas estarán maduras en aproximadamente {days}.",
    maturitySubtitle:
      "Proyección de cómo madurarán las tarjetas con el tiempo según el ritmo actual de repaso.",
    maturityRetentionDetail:
      "Según tu retención histórica de {retention}%, aproximadamente {maintenance}% de las tarjetas (~{count} tarjetas) estarán siempre en la fase de aprendizaje debido a olvidos naturales.",
    maturityEquilibrium: "Equilibrio alcanzado en {days}.",
    maturityAllMatureAtPace:
      "Todas las tarjetas estarán maduras en aproximadamente {days} al ritmo actual.",
    maturityTotalCards: "Total: {count} tarjetas",
    standardMaintenanceLine: "Nivel de mantenimiento estándar ({percent}%)",
    yourMaintenanceLine: "Tu nivel de mantenimiento ({percent}%)",
    overallTitle: "Estadísticas generales",
    currentStatus: "Estado actual",
    learningLabel: "Aprendiendo",
    matureLabel: "Maduras",
    dueToday: "Vencen hoy",
    reviewPace: "Ritmo de repaso",
    averagePerCard: "Promedio por tarjeta",
    totalTime: "Tiempo total",
    activitySummary: "Resumen de actividad",
    thisWeek: "Esta semana",
    thisMonth: "Este mes",
    thisYear: "Este año",
    answerButtons: "Botones de respuesta",
    noAnswerButtonData: "Aún no hay datos de botones de respuesta.",
    reviewActivityHeading: "Actividad de repaso",
    legendLess: "Menos",
    legendMore: "Más",
    reviewIntervalDistribution: "Distribución de intervalos",
    intervalRange: "Rango de intervalo",
    reviewIntervalsTitle: "Intervalos de repaso",
    selectDeckIntervals:
      "Selecciona un mazo para ver la distribución de intervalos.",
    reviewIntervalsSubtitle:
      "Distribución de los intervalos de repaso actuales",
    reviewsOverTimeTitle: "Repasos en el tiempo",
    selectDeckReviewsOverTime:
      "Selecciona un mazo para ver el historial de repasos.",
    dateAxisLabel: "Fecha",
    selectDeckLabel: "Selecciona mazo(s):",
    deckDropdownPrompt: "-- Selecciona un mazo --",
    allDecks: "Todos los mazos",
    tagPrefix: "Etiqueta: {tag}",
    statisticsTitle: "Estadísticas generales",
    loadingStatistics: "Cargando estadísticas…",
    failedToLoad: "Error al cargar estadísticas",
    failedToLoadIntro:
      "Hubo un error al cargar tus estadísticas. Posibles causas:",
    failedReasonDb: "Problemas de conexión con la base de datos",
    failedReasonData: "Datos corruptos",
    failedReasonSize: "Conjunto de datos demasiado grande para procesar",
    failedConsoleHint:
      "Revisa la consola del navegador (F12) para más detalles.",
    retryLoading: "Reintentar",
    timeframeLast12Months: "Últimos 12 meses",
    timeframeAllHistory: "Todo el historial",
    labelReviews: "repasos",
    labelTime: "tiempo",
    labelCorrect: "correcto",
    completeReviewsHint:
      "Completa algunos repasos para ver las estadísticas de los botones de respuesta.",
    selectDeckPrompt:
      "Selecciona un mazo del desplegable para ver estadísticas.",
    reviewHeatmapTitle: "Mapa de calor de repasos",
    dailyReviewActivity: "Actividad diaria de repaso en el tiempo",
    trueRetentionTitle: "Retención real",
    selectDeckRetention:
      "Selecciona un mazo para ver las estadísticas de retención.",
    retentionDescription:
      "Tasas de acierto para tarjetas de repaso (calificación ≥ Bien). Solo tarjetas con intervalos > 1 día.",
    noReviewDataYet:
      "Aún no hay datos de repaso. Completa algunos repasos para ver las estadísticas de retención.",
    columnCardType: "Tipo de tarjeta",
    columnPassed: "Acertadas",
    columnTotalLabel: "Total",
    columnPassRate: "Tasa de acierto",
    youngWithDays: "Jóvenes (< {count} días)",
    matureWithDays: "Maduras (≥ {count} días)",
    allCards: "Todas las tarjetas",
    loading: "Cargando…",
    scope: "Ámbito de las estadísticas",
    timeframe: "Periodo",
    updating: "Actualizando…",
    waitingForDb: "Esperando la base de datos de tarjetas…",
    tryAgain: "Reintentar",
    noneYet: "Todavía no hay estadísticas.",
    noneYetHint: "Repasa algunas tarjetas y tu progreso aparecerá aquí.",
    noReviewsInPeriod: "No hay repasos registrados en este periodo.",
    notEnoughHistory: "Aún no hay suficiente historial.",
    cards: "Tarjetas",
    cardStateDistribution: "Distribución de estados de tarjeta",
    dueNext30: "Tarjetas pendientes en los próximos 30 días",
    reviewsAllTime: "Repasos en total",
    studyTimeAllTime: "Tiempo de estudio total",
    timeInPeriod: "Tiempo en el periodo",
    passRateDesc: "Tasa de acierto en tarjetas repasadas con un intervalo de un día o más.",
  },

  ankiExport: {
    title: "Exportar a Anki",
    titleWithName: "Exportar a Anki: {name}",
    description:
      "Exporta las tarjetas seleccionadas como CSV compatible con Anki.",
    ankiDeckNameLabel: "Nombre del mazo en Anki",
    ankiDeckNameDesc:
      "Nombre del mazo en Anki al que se importarán las tarjetas",
    ankiDeckNamePlaceholder: "Introduce el nombre del mazo...",
    exportButtonLabel: "Exportar a Anki",
    sectionInfo: "Información del export",
    sectionHowToImport: "Cómo importar",
    sectionSettings: "Ajustes de export",
    infoSourceDeck: "Mazo origen:",
    infoSourceTag: "Etiqueta origen:",
    infoSourceFile: "Archivo origen:",
    importStep1: "Se descargará un archivo de texto compatible con Anki",
    importStep2Prefix: "Abre Anki y ve a",
    importStep2Item: "Archivo → Importar",
    importStep3: "Selecciona el archivo descargado",
    importStep4: "Elige tu mazo de destino y los ajustes de importación",
    importStep5Prefix: "Haz clic en",
    importStep5Item: "Importar",
    importStep5Suffix: "para añadir las tarjetas",
    selectAll: "Seleccionar todo",
    deselectAll: "Deseleccionar todo",
    selectedCount: "{count} seleccionadas",
    deckName: "Nombre del mazo",
    deckNamePlaceholder: "Mi Mazo",
    includeTags: "Incluir etiquetas",
    includeTagsDesc: "Añade etiquetas de mazo y tarjeta al export",
    includeScheduling: "Incluir programación",
    includeSchedulingDesc: "Conserva el estado FSRS en el export",
    export: "Exportar",
    cancel: "Cancelar",
    exporting: "Exportando…",
    exportSuccess: "{count} tarjetas exportadas a {filename}",
    exportFailed: "Exportación fallida: {message}",
    noSelection: "Selecciona al menos una tarjeta para exportar.",
  },

  filterBuilder: {
    title: "Filtro",
    add: "Añadir condición",
    addRule: "+ Añadir regla",
    remove: "Quitar condición",
    removeRule: "Quitar regla",
    clear: "Borrar todo",
    apply: "Aplicar",
    save: "Guardar filtro",
    matchLabel: "Coincidir",
    matchAll: "Coincidir todas",
    matchAny: "Coincidir cualquiera",
    matchAllOption: "Todas",
    matchAnyOption: "Cualquiera",
    matchSuffix: "de las siguientes reglas",
    fieldDeck: "Mazo",
    fieldCardState: "Estado de la tarjeta",
    fieldDueDate: "Fecha de vencimiento",
    fieldDeckTag: "Etiqueta del mazo",
    fieldCardTag: "Etiqueta de tarjeta",
    fieldSourceFile: "Archivo origen",
    fieldCardType: "Tipo de tarjeta",
    fieldState: "Estado",
    fieldTag: "Etiqueta",
    fieldDue: "Vencimiento",
    fieldDifficulty: "Dificultad",
    fieldStability: "Estabilidad",
    fieldLapses: "Lapsus",
    fieldFront: "Texto anverso",
    fieldBack: "Texto reverso",
    fieldFlagged: "Marcada",
    fieldSuspended: "Suspendida",
    fieldCreated: "Creada",
    fieldReviewed: "Último repaso",
    fieldIsLeech: "Es sanguijuela",
    fieldIsDense: "Es densa",
    fieldIsSuspended: "Está suspendida",
    fieldIsBuried: "Está enterrada",
    operatorIs: "Es",
    operatorIsNot: "No es",
    operatorIsNew: "Es nueva",
    operatorIsDue: "Vence",
    operatorHasTag: "Tiene etiqueta",
    operatorEquals: "igual",
    operatorNotEquals: "Distinto",
    operatorContains: "contiene",
    operatorNotContains: "no contiene",
    operatorBefore: "antes de",
    operatorAfter: "después de",
    operatorGreaterThan: "Mayor que",
    operatorLessThan: "Menor que",
    operatorBetween: "entre",
    operatorIsLeech: "es sanguijuela",
    operatorIsDense: "es densa",
    selectTagPlaceholder: "Selecciona etiqueta...",
    stateNew: "Nueva",
    stateReview: "Repaso",
    typeHeaderParagraph: "Encabezado-párrafo",
    typeTable: "Tabla",
    typeCloze: "Hueco",
    typeImageOcclusion: "Oclusión de imagen",
    typeSpatial: "Espacial",
    boolTrue: "Verdadero",
    boolFalse: "Falso",
    valuePlaceholder: "Valor...",
    secondValuePlaceholder: "Hasta",
    cardsMatch: "{count} tarjetas coinciden",
    cardMatchSingular: "{count} tarjeta coincide",
    fieldBreadcrumb: "Ruta",
    fieldInterval: "Intervalo (min)",
    fieldRepetitions: "Repeticiones",
    operatorEqualsCap: "Igual",
    operatorIsOneOf: "Es uno de",
    operatorContainsCap: "Contiene",
    operatorNotContainsCap: "No contiene",
    operatorHasAnyOf: "Tiene alguno de",
    operatorDoesNotHaveTag: "No tiene etiqueta",
    operatorBeforeCap: "Antes de",
    operatorAfterCap: "Después de",
    allRules: "Todas las reglas",
    anyRule: "Cualquier regla",
    noRulesYet: "Aún no hay reglas: un filtro sin reglas incluye todas las tarjetas.",
    ruleField: "Campo de la regla",
    ruleOperator: "Operador de la regla",
    ruleValue: "Valor de la regla",
    ruleMatching: "Coincidencia de reglas",
    selectDeck: "Elige un mazo",
    selectTag: "Elige una etiqueta",
    selectType: "Elige un tipo",
    value: "Valor",
    badgeType: "Tipo",
    badgeInterval: "Intervalo",
    badgeReps: "Reps.",
    badgeNot: "Sin {label}",
    badgeNotText: "no \"{value}\"",
  },

  modals: {
    confirm: {
      cancel: "Cancelar",
      confirm: "Confirmar",
      ok: "OK",
    },
    renameCustomDeck: {
      title: "Renombrar mazo personalizado",
      label: "Nombre del mazo",
      placeholder: "Introduce el nuevo nombre",
      rename: "Renombrar",
      cancel: "Cancelar",
    },
    deckReset: {
      title: "Reiniciar progreso del mazo",
      intro: 'Esto reiniciará todo el progreso de "{name}":',
      listCardsReset: "Todas las tarjetas vuelven al estado nuevo",
      listHistoryDeleted: "Historial de repasos eliminado",
      listSessionsDeleted: "Sesiones de repaso eliminadas",
      cannotUndo: "Esta acción no se puede deshacer.",
      cancel: "Cancelar",
      resetButton: "Reiniciar progreso",
      resetting: "Reiniciando progreso…",
      resetSuccess: "Progreso del mazo reiniciado con éxito",
      resetFailed: "Error al reiniciar el progreso del mazo",
    },
    purgeDatabase: {
      title: "Purgar base de datos",
      warningStrong:
        "Esto eliminará permanentemente todos los datos de tarjetas, incluyendo:",
      listFlashcards: "Todas las tarjetas y su contenido",
      listReviews: "El historial completo de repasos y el progreso",
      listDecks: "Toda la información de los mazos",
      listStatistics: "Datos estadísticos",
      cannotUndo: "¡Esta acción no se puede deshacer!",
      rebuildNote:
        "La base de datos se reconstruirá desde los archivos actuales del vault, pero se perderá todo el progreso.",
      confirmPrompt:
        "Para confirmar esta acción, escribe el texto mostrado en el placeholder",
      confirmPlaceholder: "BORRAR TODOS LOS DATOS",
      purgeButton: "Purgar base de datos",
      cancel: "Cancelar",
    },
    releaseNotes: {
      title: "Novedades",
      close: "Cerrar",
      versionLabel: "Versión {version}",
    },
    statistics: {
      title: "Estadísticas",
      close: "Cerrar",
    },
    optimizeFsrs: {
      title: "Optimizar parámetros",
      loadingHistory: "Cargando historial de repasos…",
      trainingStep: "Paso de entrenamiento {step} / {total}",
      notEnough:
        "Aún no hay suficiente historial de repasos para entrenar. Sigue repasando e inténtalo más tarde.",
      reviewsAvailable:
        "Repasos disponibles en mazos de perfil estándar: {count}.",
      trainedSummary:
        "Entrenado con {reviews} repasos en {cards} tarjetas en {seconds} s.",
      logLossBefore: "Log-loss antes",
      logLossAfter: "Log-loss después",
      improvement: "Mejora",
      noImprovement:
        "El entrenamiento no mejoró el log-loss de forma significativa — los pesos actuales ya son un buen ajuste. Puedes descartar con seguridad.",
      discard: "Descartar",
      apply: "Aplicar",
      close: "Cerrar",
      failedHeader: "Optimización fallida:",
      applied: "Parámetros entrenados aplicados.",
    },
    flashcardManager: {
      title: "Gestor de tarjetas",
    },
    decksView: {
      title: "Mazos",
    },
    aiBatch: {
      title: "Refactorización por lotes con IA",
      intro: "Revisando {count} tarjetas seleccionadas. Acepta o rechaza cada campo sugerido.",
      start: "Iniciar",
      cancel: "Cancelar",
      close: "Cerrar",
      retry: "Reintentar",
      accept: "Aceptar",
      decline: "Rechazar",
      acceptAll: "Aceptar todo",
      discardAll: "Descartar todo",
      stop: "Detener",
      skipCard: "Omitir tarjeta",
      processing: "Procesando {current} de {total}…",
      waiting: "Esperando…",
      done: "Listo",
      noProposals: "No se sugieren cambios",
      applied: "Aplicado",
      skipped: "Omitido",
      failed: "Fallido",
      summary: "{applied} aplicadas · {skipped} omitidas · {failed} fallidas",
      bulkButton: "Refactorizar con IA",
      suggestionFor: "Sugerencia para {field}",
      statusReady: "Listo",
      statusAccepted: "Aceptado",
      apply: "Aplicar {count}",
      backToList: "Atrás",
      selectPrompt: "Selecciona una tarjeta para revisar",
    },
    aiGenerator: {
      destination: "Destino",
      deckSearchPlaceholder: "Buscar mazos existentes en la bóveda…",
      title: "Generar tarjetas con IA",
      intro: "Describe qué generar. Las tarjetas aparecen abajo: conserva las que quieras y guárdalas.",
      promptPlaceholder: "Describe las tarjetas que se generarán…",
      generate: "Generar",
      generating: "Generando…",
      continueGenerating: "Seguir generando",
      stop: "Detener",
      streaming: "{count} generadas hasta ahora…",
      noCards: "Aún no hay tarjetas. Describe abajo qué generar.",
      backToList: "Atrás",
      selectPrompt: "Selecciona una tarjeta para verla",
      fieldFront: "Anverso",
      fieldBack: "Reverso",
      fieldNotes: "Notas",
      keep: "Conservar",
      discard: "Descartar",
      addNote: "Adjuntar una nota",
      addImage: "Adjuntar una imagen",
      modeNew: "Crear archivo nuevo",
      modeAppend: "Añadir al mazo",
      format: "Formato",
      formatHeader: "Encabezado + párrafo",
      formatTable: "Tabla",
      formatCanvas: "Lienzo",
      name: "Nombre del archivo",
      namePlaceholder: "Mazo nuevo",
      folder: "Carpeta",
      folderPlaceholder: "Raíz del cofre",
      tag: "Etiqueta",
      profile: "Perfil",
      deck: "Mazo",
      save: "Guardar {count}",
      saving: "Guardando…",
      saved: "{count} tarjetas guardadas",
      saveFailed: "No se pudieron guardar las tarjetas",
      nameRequired: "Introduce un nombre de archivo",
      deckRequired: "Elige un mazo al que añadir",
      noKept: "Conserva al menos una tarjeta para guardar",
      close: "Cerrar",
      cancel: "Cancelar",
      includeGenerated: "Incluir las tarjetas generadas como contexto",
      existingCardsHeading: "Tarjetas ya generadas en esta sesión:",
      avoidDuplicatesInstruction: "No dupliques ninguna de las tarjetas anteriores.",
      savedBadge: "Guardada",
      savedNotice: "{count} tarjetas guardadas hasta ahora",
      generateFailed: "No se pudieron generar las tarjetas",
      debugTitle: "Depuración",
      debugSystem: "Sistema",
      debugUser: "Usuario",
      debugPriorAssistant: "Asistente (tarjetas hasta ahora)",
      debugFollowup: "Seguimiento",
      debugImages: "Imágenes",
      debugResponse: "Respuesta",
      debugEmpty: "Ejecuta una generación para ver la solicitud y la respuesta.",
      debugToggle: "Mostrar/ocultar panel de depuración",
      debugOcr: "Transcripciones de OCR",
      debugOcrPage: "Página {page}",
      pdfChapters: "Capítulos",
      pdfClosePanel: "Cerrar el panel de capítulos",
      pdfCloseTab: "Quitar este PDF",
      pdfParseMode: "Modo de análisis",
      pdfModeAuto: "Auto (texto, OCR si escaneado)",
      pdfModeText: "Solo texto",
      pdfModeOcr: "OCR en todas las páginas",
      pdfSelectAll: "Seleccionar todo",
      sourceCovered: "El modelo no encontró nada más que añadir de esta fuente.",
      clearCards: "Vaciar",
      clearConfirmTitle: "¿Descartar las tarjetas generadas?",
      clearConfirmMessage:
        "{count} tarjetas marcadas aún no se han guardado y se descartarán. Las tarjetas ya guardadas permanecen en tu bóveda.",
      pdfClear: "Limpiar",
      pdfOcrProgress: "Leyendo páginas {done}/{total}…",
      pdfCached: "en caché",
      pdfNoChapters: "No se detectaron capítulos.",
      addPdf: "Adjuntar un PDF",
      pdfFromVault: "Desde el vault",
      pdfFromComputer: "Desde el ordenador",
      pdfTooLarge: "El PDF es demasiado grande (máx. {max} MB).",
      tooManyNotes: "Puedes adjuntar como máximo {max} notas.",
      tooManyImages: "Puedes adjuntar como máximo {max} imágenes.",
    },
    editFlashcard: {
      aiRefactor: "Refactorizar con IA",
      aiRefactoring: "Refactorizando…",
      aiSuggestion: "Sugerencia de IA",
      aiAccept: "Aceptar",
      aiDecline: "Rechazar",
      aiNoChanges: "La IA no sugirió cambios",
      aiDrawerTitle: "Refactorización con IA",
      aiIncludeSourceContext: "Incluir contexto de origen ({file})",
      aiPresetFixLatex: "Corregir LaTeX",
      aiPresetSimplify: "Simplificar la explicación",
      aiPresetTranslate: "Traducir al inglés",
      aiCustomInstructionsPlaceholder: "Instrucciones personalizadas para esta tarjeta…",
      aiTargetScope: "Campos objetivo",
      aiGenerate: "Generar",
      aiPromptPlaceholder: "Describe cómo refactorizar esta tarjeta…",
      aiAddNote: "Adjuntar una nota",
      aiAddImage: "Adjuntar una imagen",
      aiAddPdf: "Adjuntar un PDF",
      aiSend: "Generar",
      aiModel: "Modelo",
      aiAcceptVersion: "Aceptar esta versión",
      aiDismiss: "Descartar",
      aiShow: "Mostrar asistente de IA",
      aiHide: "Ocultar asistente de IA",
      aiSplit: "Dividir",
      aiSplitToggle: "Dividir en varias tarjetas",
      aiAcceptSplit: "Aceptar esta división",
      aiSplitTitle: "Dividir en {count} tarjetas",
      aiRemoveContext: "Quitar",
      aiDebugTitle: "Última petición / respuesta",
      aiDebugSystem: "Sistema",
      aiDebugPrompt: "Indicación",
      aiDebugResponse: "Respuesta",
      cancel: "Cancelar",
      save: "Guardar",
      saving: "Guardando…",
      title: "Editar tarjeta",
      fieldHeader: "Encabezado",
      fieldBody: "Contenido",
      fieldFront: "Anverso",
      fieldBack: "Reverso",
      fieldNotes: "Notas",
      fieldHint: "Pista",
      fieldImage: "Imagen",
      fieldQuestion: "Pregunta",
      fieldOptions: "Opciones",
      fieldSentence: "Frase",
      templateColumnsHint: "Esta tarjeta usa una plantilla: edita abajo sus columnas.",
      templateColumnFallback: "Columna {n}",
    },
  },

  settings: {
    ai: {
      heading: "Funciones de IA",
      enabled: "Activar las funciones de IA",
      enabledDesc: "Activa la reescritura de tarjetas con IA. Requiere una clave API o un modelo local.",
      provider: "Proveedor",
      providerDesc: "Qué proveedor de IA usar para la reescritura.",
      model: "Modelo",
      modelDesc: "Nombre del modelo para el proveedor elegido.",
      tier: "Nivel de calidad",
      tierDesc: "Decks Pro elige el modelo según el nivel.",
      modelCustom: "Personalizado…",
      modelCustomLabel: "Modelo personalizado",
      localBaseUrl: "URL del servidor local",
      localBaseUrlDesc: "URL base de un servidor local compatible con OpenAI como Ollama o LM Studio.",
      apiKey: "Clave API",
      apiKeyDesc: "Se guarda en un archivo local que no se sincroniza entre dispositivos.",
      apiKeyPlaceholder: "Pega la clave API",
      providerGemini: "Google Gemini",
      providerOpenai: "OpenAI",
      providerClaude: "Anthropic Claude",
      providerLocal: "Endpoint local o personalizado",
      providerDecksPro: "Decks Pro",
      serverUrl: "URL del servidor",
      serverUrlDesc: "URL del backend de Decks Pro. Déjalo vacío para usar el predeterminado.",
      siteUrl: "URL del sitio web",
      siteUrlDesc: "URL del sitio de Decks Pro. Déjalo vacío para usar el predeterminado.",
      accountLoading: "Cargando cuenta\u2026",
      accountError: "No se pudieron cargar los datos de la cuenta. Comprueba tu conexi\u00f3n.",
      accountRefresh: "Actualizar",
      trialActive: "Prueba gratuita: unas 100 tarjetas o 10 p\u00e1ginas de PDF.",
      trialExhausted: "Prueba gratuita agotada. Suscr\u00edbete para seguir generando.",
      dailyLimitReached: "L\u00edmite diario alcanzado. Se restablece ma\u00f1ana.",
      account: "Cuenta de Decks Pro",
      accountSignedOut: "Inicia sesión para usar Decks Pro.",
      accountSignedInAs: "Sesión iniciada como {email}",
      accountSignedIn: "Sesión iniciada",
      signIn: "Iniciar sesión en Decks Pro",
      signInPending: "Termina de iniciar sesión en el navegador y vuelve aquí.",
      signInSuccess: "Sesión iniciada en Decks Pro.",
      signInFailed: "Error al iniciar sesión. Inténtalo de nuevo desde los ajustes.",
      signOut: "Cerrar sesión",
      signOutDone: "Sesión cerrada en Decks Pro.",
      subscribe: "Suscribirse",
      subscribeDesc: "9,99 \u20ac al mes: unas 20.000 tarjetas o 5.000 p\u00e1ginas de PDF.",
      manageSubscription: "Gestionar suscripción",
      openAccount: "Abrir página de la cuenta",
      subscriptionStatus: "Suscripción",
      subscriptionNone: "Sin suscripción activa. Suscríbete para empezar a generar.",
      subscriptionActive: "Activa",
      subscriptionTrial: "En prueba",
      subscriptionPastDue: "Pago pendiente",
      subscriptionCancelled: "Cancelada: activa hasta el final del periodo",
      subscriptionExpired: "Caducada",
      subscriptionPaused: "En pausa",
      enableDesc: "Activa la generación de tarjetas y las acciones de IA en el editor.",
      baseUrl: "URL base",
      baseUrlDesc: "Un endpoint de modelo local o autoalojado accesible desde este dispositivo.",
      modelId: "Id del modelo",
      modelIdDesc: "El id de modelo que expone tu servidor.",
      proAccount: "Cuenta de DecksMD Pro",
      proDesc: "Usa tu cuenta de DecksMD para generar tarjetas sin una clave API propia.",
      noKeyNeeded: "DecksMD Pro no necesita clave propia. Los demás proveedores usan tu cuenta directamente.",
      email: "Correo electrónico",
      password: "Contraseña",
      noAccountYet: "¿Aún no tienes cuenta?",
      createOne: "Crea una en decksmd.app",
      manageAccount: "Gestionar la cuenta",
      billingDesc: "La facturación y los dispositivos vinculados están en el sitio web.",
      seePlans: "Ver los planes",
      open: "Abrir",
      clearKey: "Borrar",
      deviceSignedIn: "Este dispositivo tiene la sesión iniciada, así que no hace falta clave.",
    },
    language: {
      heading: "Idioma",
      name: "Idioma del plugin",
      desc: 'Elige el idioma. "Auto" sigue la configuración de Obsidian. Cambiarlo requiere recargar el plugin para que tenga efecto.',
      auto: "Auto",
    },

    review: {
      heading: "Sesiones de repaso",
      showProgress: "Mostrar progreso",
      showProgressDesc:
        "Mostrar barra de progreso durante las sesiones de repaso",
      keyboardShortcuts: "Atajos de teclado",
      enableShortcuts: "Activar atajos de teclado",
      keyboardShortcutsDesc:
        "Activar atajos de teclado en la ventana de repaso (1-4 para dificultad)",
      shortcutsHeading: "Atajos de repaso",
      shortcutsHeadingDesc: "Haz clic en una tecla de abajo y luego pulsa la que quieras usar. Pulsa Esc para cancelar.",
      shortcutReveal: "Tecla de mostrar / siguiente",
      shortcutPressKey: "Pulsa una tecla… (Esc para cancelar)",
      shortcutResetDefaults: "Restablecer valores predeterminados",
      shortcutDuplicate: "Esa tecla ya la usa otra acción de repaso.",
      sessionDuration: "Duración de la sesión",
      sessionDurationDesc:
        "Duración máxima de las sesiones de repaso en minutos (1-60)",
      sessionDurationPlaceholder: "25",
      studyDayStartsAt: "El día de estudio empieza a",
      studyDayStartsAtDesc:
        "Hora a la que cambia el día de estudio (0-23, por defecto 4). Los repasos antes de esta hora cuentan para las estadísticas y límites diarios del día anterior.",
      enableGlobalReviewCap: "Límite diario global de tarjetas",
      enableGlobalReviewCapDesc: "Limita el total de tarjetas (nuevas + repaso) por día en todos los mazos combinados (además de los límites por mazo).",
      globalReviewCap: "Tarjetas por día (todos los mazos)",
      globalReviewCapDesc: "Número máximo de tarjetas (nuevas + repaso) mostradas por día en todos los mazos combinados.",
      globalReviewCapPlaceholder: "100",
      leechThreshold: "Umbral de sanguijuela",
      leechThresholdDesc:
        "Una tarjeta se marca como sanguijuela cuando su número de lapsus llega a esta cifra (por defecto 8). Las sanguijuelas se olvidan repetidamente y suelen necesitar reescribirse.",
      leechThresholdPlaceholder: "8",
      denseCardCharThreshold: "Umbral de caracteres para tarjetas densas",
      denseCardCharThresholdDesc:
        "Una tarjeta se marca como densa cuando su reverso alcanza este número de caracteres (por defecto 500). Las tarjetas densas suelen tener demasiada información y conviene dividirlas.",
      denseCardCharThresholdPlaceholder: "500",
    },

    parsing: {
      heading: "Análisis",
      deckTag: "Etiqueta de mazo",
      deckTagDesc:
        "Etiqueta base para identificar mazos de tarjetas. Los archivos con esta etiqueta (o sub-etiquetas como {tag}/math) se tratan como mazos. La etiqueta por defecto cambió de #flashcards a #decks. Si usabas #flashcards, seguirá funcionando hasta que lo cambies aquí.",
      deckTagPlaceholder: "#decks",
      folderSearchPath: "Carpeta de búsqueda",
      folderSearchPathDesc:
        "Limita el análisis a una carpeta concreta. Selecciona 'escanear todo el vault' para analizar todos los archivos.",
      folderSearchPathDefault: "Escanear todo el vault (por defecto)",
    },

    templates: {
      heading: "Plantillas de tarjetas",
      folder: "Carpeta de plantillas",
      folderDesc: "Carpeta de archivos de plantilla. Los decks-tags de cada archivo lo vinculan a filas de tabla o notas.",
      folderDefault: "Desactivado (sin carpeta de plantillas)",
      previewHeading: "Vista previa",
      previewHint: "La vista previa usa datos de ejemplo.",
      noTemplates: "Todavía no se han encontrado plantillas en la carpeta elegida.",
      notesLabel: "Notas",
    },

    canvasDecks: {
      heading: "Mazos de canvas",
      folderPath: "Carpeta de mazos de canvas",
      folderPathDesc:
        "Carpeta donde buscar archivos canvas (.canvas) de Obsidian. Cada canvas se convierte en un mazo. Cada nodo de texto se analiza con los mismos cuatro formatos de tarjeta que los archivos Markdown. Déjalo vacío para desactivar el escaneo de canvas.",
      folderPathDefault: "Desactivado — sin escaneo de canvas",
      tagName: "Etiqueta de mazo de canvas",
      tagNameDesc:
        "Etiqueta aplicada a cada mazo de canvas detectado. Se usa para agrupar en la vista de Etiquetas y para asignar perfiles. Por defecto: #decks/canvas.",
      tagNamePlaceholder: "#decks/canvas",
    },

    ui: {
      heading: "Interfaz",
      backgroundRefreshInterval: "Intervalo de actualización en segundo plano",
      backgroundRefreshIntervalDesc:
        "Con qué frecuencia se actualizan las estadísticas en el panel lateral (en segundos)",
      backgroundRefreshIntervalPlaceholder: "5",
      enableBackgroundRefresh: "Activar actualización en segundo plano",
      enableBackgroundRefreshDesc:
        "Actualiza automáticamente las estadísticas en el panel lateral",
      enableNotices: "Activar notificaciones",
      enableNoticesDesc:
        "Muestra mensajes para sesiones completadas y operaciones de sincronización",
      hideAnchorTokens: "Ocultar marcadores de ID de tarjeta",
      hideAnchorTokensDesc:
        "Oculta los marcadores %%dk:…%% que Decks escribe en tus notas mientras editas en vista previa en vivo. Reaparecen en la línea donde está el cursor y nunca se eliminan del archivo.",
      reviewDisplayMode: "Modo de visualización del repaso",
      reviewDisplayModeDesc:
        "Dónde se abren las sesiones de repaso y exploración. Las pestañas pueden arrastrarse a barras laterales, paneles inferiores o ventanas separadas.",
      flashcardManagerDisplayMode: "Modo del gestor de tarjetas",
      flashcardManagerDisplayModeDesc:
        "Dónde se abre el gestor de tarjetas. Las pestañas pueden arrastrarse a barras laterales, paneles inferiores o ventanas separadas.",
      aiGeneratorDisplayMode: "Modo de visualización del generador con IA",
      aiGeneratorDisplayModeDesc:
        "Dónde se abre el generador de tarjetas con IA. Las pestañas pueden arrastrarse a barras laterales, paneles inferiores o ventanas separadas.",
      displayModeModal: "Modal superpuesto",
      displayModeTab: "Pestaña nueva",
      minDeckCardCount: "Ocultar mazos con menos de",
      minDeckCardCountDesc:
        "Los mazos (y grupos) con menos tarjetas en total se ocultan de la lista. Los mazos fijados siempre se muestran. 0 desactiva.",
      minDeckCardCountPlaceholder: "0",
    },

    backup: {
      heading: "Copia de seguridad",
      enableAutoBackup: "Activar copia automática",
      enableAutoBackupDesc:
        "Hacer una copia de seguridad de los datos después de cada sesión",
      maxBackups: "Máx. copias",
      maxBackupsDesc: "Número máximo de copias a conservar (3-10)",
      createBackupNow: "Crear copia ahora",
      createBackupNowDesc: "Crear una copia manual de la base de datos",
      createBackupButton: "Crear copia",
      restoreHeading: "Restaurar copia",
      availableBackups: "Copias disponibles",
      availableBackupsDesc: "Selecciona una copia para restaurar",
      selectBackupOption: "Selecciona copia…",
      noBackupsOption: "No se encontraron copias",
      refreshButton: "Actualizar",
      refreshTooltip: "Actualizar lista de copias",
      restoreButton: "Restaurar",
      restoreFromFile: "Restaurar desde archivo",
      restoreFromFileTooltip:
        "Selecciona un archivo .db de cualquier ubicación",
      rebuildState: "Reconstruir la programación",
      rebuildStateDesc: "Restaura la programación de tarjetas que aparecen como nuevas pero conservan historial.",
      rebuildStateButton: "Reconstruir desde el historial",
    },

    fsrs: {
      heading: "Ajuste del algoritmo",
      optimize: "Optimizar parámetros",
      optimizeButton: "Optimizar",
      resetButton: "Restablecer predeterminados",
      descUntrained:
        "Entrena los 21 pesos del algoritmo con tu historial de repasos del perfil estándar. Requiere al menos 100 repasos. Los mazos intensivos no se ven afectados.",
      descTrained:
        "Usando actualmente pesos entrenados desde {when} ({count} repasos, log-loss {before} → {after}).",
      descTrainedUnknownWhen: "desconocido",
      descTrainedMissingMetric: "—",
    },

    debug: {
      heading: "Depuración",
      paragraph:
        "Ajustes de depuración para resolución de problemas y desarrollo.",
      enableLogging: "Activar registro de depuración",
      enableLoggingDesc:
        "Muestra registros detallados en la consola para sincronización y procesamiento de tarjetas",
      performanceLogs: "Activar registros de rendimiento",
      performanceLogsDesc:
        "Muestra métricas de tiempo de rendimiento en la consola (sincronización, análisis, etc.)",
    },

    paths: {
      heading: "Ubicaciones de archivos",
      paragraph:
        "Todas las rutas son relativas al vault. Deja vacío para usar la ubicación por defecto. La mayoría de usuarios no necesita cambiarlas — existen para que puedas mover la BD fuera de la carpeta oculta {configDir}/ (que iCloud y otros sincronizadores tratan con menor prioridad).",
      dbFolder: "Carpeta de base de datos",
      dbFolderDesc:
        "Carpeta que contiene flashcards.db. Por defecto: {pluginFolder}/. Reinicia Obsidian después de cambiar. El archivo no se mueve automáticamente — haz copia primero, luego muévelo manualmente o usa 'Restaurar desde archivo' para importarlo a la nueva ubicación.",
      backupFolder: "Carpeta de copias",
      backupFolderDesc:
        "Carpeta para copias automáticas y manuales. Por defecto: {pluginFolder}/backups/. Los cambios surten efecto inmediatamente — no se requiere reinicio.",
      backupFolderPlaceholder: "{pluginFolder}/backups",
      syncLogFolder: "Carpeta del log de sincronización",
      syncLogFolderDesc:
        "Carpeta de los archivos .deckssynclog por dispositivo usados por la sincronización multi-dispositivo. Por defecto: raíz del vault (la que iCloud sincroniza más rápido). Reinicia Obsidian después de cambiar.",
      syncLogFolderPlaceholder: "(raíz del vault)",
      pdfCacheFolder: "Carpeta de caché de OCR de PDF",
      pdfCacheFolderDesc: "Carpeta con el texto OCR de cada página en caché, para no leer dos veces un PDF.",
      pdfCacheFolderPlaceholder: "{pluginFolder}/pdf-ocr",
    },

    database: {
      heading: "Gestión de la base de datos",
      paragraph:
        "Gestiona tu base de datos de tarjetas. Usar con precaución — estas acciones no se pueden deshacer.",
      purgeDatabase: "Purgar base de datos",
      purgeDatabaseDesc:
        "Elimina permanentemente todas las tarjetas, el historial de repasos y los datos de los mazos. Esto forzará una reconstrucción limpia desde los archivos del vault. ¡Se perderá todo el progreso!",
      purgeDatabaseButton: "Purgar base de datos",
    },
    hideIdentityMarkers: "Ocultar los marcadores de identidad",
    scanFolderDesc: "Limitar el escaneo a una carpeta. Déjalo vacío para escanear todo.",
    foldersRelativeDesc: "Las carpetas son relativas al vault. Déjalo vacío para la ubicación predeterminada.",
    syncLogFolderDesc: "Carpeta de los registros de sincronización por dispositivo. Por defecto: la raíz del vault.",
    syncLogIdDesc: "Identifica el registro de sincronización de este dispositivo. Un cambio de carpeta se aplica al abrir el vault de nuevo.",
    canvasTagRule: "Una etiqueta de canvas debe empezar por # y usar minúsculas, números, guiones, guiones bajos o barras.",
    fallbackExamDesc: "Lo usan los mazos sin perfil propio, como los personalizados. Los mazos con perfil usan los ajustes de examen de ese perfil.",
  },
  cardEdit: {
    typeMismatch: "El tipo de edición {edit} no coincide con el tipo de tarjeta {card}",
    vaultReadOnly: "Este vault está abierto en solo lectura.",
    canvasNotHere: "Las tarjetas de Canvas no se pueden editar aquí.",
    couldNotRead: "No se pudo leer {file}: {reason}",
    cardNotFound: "No se pudo localizar esta tarjeta en la nota de origen.",
    noteChanged: "La nota cambió desde que se cargó esta tarjeta. Actualiza e inténtalo de nuevo.",
    tableRowUnparseable: "La fila de la tabla ya no se puede analizar.",
    tableRowChanged: "La fila de la tabla cambió desde que se cargó esta tarjeta.",
    clozeChanged: "El contenido del hueco cambió desde que se cargó esta tarjeta.",
    clozeRowUnparseable: "La fila que aloja el hueco ya no se puede analizar.",
    clozeRowInvalid: "La fila que aloja el hueco no es una fila de tabla válida.",
    emptyHeaderSegment: "Segmento de encabezado vacío.",
    anchorNotHeader: "La línea de anclaje no es un encabezado.",
    notATableRow: "No es una fila de tabla válida.",
    noNotesColumn: "Esta tabla no tiene columna de notas.",
    separatorNotFound: "No se encontró la fila separadora de la tabla para añadir una columna de notas.",
    frontEmpty: "El texto del anverso no puede estar vacío.",
    headerEmpty: "El texto del encabezado no puede estar vacío.",
    clozeNeedsSpan: "Un hueco necesita al menos un ==fragmento==.",
  },
};
