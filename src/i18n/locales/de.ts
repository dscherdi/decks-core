import type { Translations } from "./en";

// DRAFT — review with a native German speaker before release.
export const de: Translations = {
  help: {
    docs: "Dokumentation öffnen",
  },

  mobileApp: {
    heading: "DecksMD für iPhone und iPad",
    body: "Derselbe Vault, dasselbe Markdown, auf dem Handy wiederholt. Karten, Planung und Verlauf bleiben in deinen Dateien.",
    cta: "Laden im App Store",
  },
  pro: {
    heading: "Decks Pro",
    body: "Erstelle Karteikarten aus deinen Notizen, PDFs und gescannten Folien, ohne einen API-Schlüssel einzurichten. Kostenlos testen — alles andere in Decks bleibt kostenlos.",
    cta: "Mehr erfahren",
  },

  views: {
    decks: "Stapel",
    review: "Wiederholen",
    browse: "Durchsuchen",
    reviewOf: "{prefix}: {name}",
    manager: "Karteikarten-Manager",
    managerOf: "Verwalten: {deckName}",
    exam: "Prüfung",
  },

  ribbon: {
    decks: "Stapel",
  },

  testDeckCanvas: {
    filename: "Decks — Canvas Erste Schritte.canvas",
    title: "Canvas — Erste Schritte",
    intro:
      "Dieses Canvas ist ebenfalls ein Karteikarten-Stapel. Jeder Textknoten unten enthält eine oder mehrere Karten, die mit den gleichen Formaten wie in einem Markdown-Stapel geparst werden. Das Stapel-Tag ist {tag}.",
    formatsHint:
      "Verschiebe die Knoten, bearbeite Text, füge neue hinzu — jeder Textknoten in den .canvas-Dateien dieses Ordners wird Teil deines Stapels.",
    section1: "Überschrift-Absatz-Karten",
    q1: "Was unterscheidet einen Canvas-Stapel von einem Markdown-Stapel?",
    a1: "Jeder Textknoten wird unabhängig geparst. Du kannst Formate frei über mehrere Knoten hinweg mischen.",
    q2: "Wo sucht das Plugin nach Canvas-Stapeln?",
    a2:
      "Im Ordner, der unter Einstellungen → Canvas-Stapel konfiguriert ist. Diese Datei liegt dort.",
    section2: "Tabellen-Karten",
    fsrsConceptsHeading: "FSRS-Konzepte",
    colFront: "Vorderseite",
    colBack: "Rückseite",
    colNotes: "Notizen",
    row1Front: "Wofür steht FSRS?",
    row1Back: "Free Spaced Repetition Scheduler",
    row1Notes: "Der Algorithmus, den Decks zum Planen von Karten verwendet",
    row2Front: "Was sind die vier Bewertungsstufen?",
    row2Back: "Noch einmal, Schwer, Gut, Einfach",
    row2Notes:
      "„Noch einmal“ setzt die Karte zurück; „Einfach“ gibt den längsten Intervall",
    section3: "Lückentext-Karten (Cloze)",
    solarHeading: "Das Sonnensystem",
    solarBody:
      "Die ==Sonne== ist der Stern im Zentrum unseres Sonnensystems. Der nächste Planet ist der ==Merkur== und der größte Planet ist der ==Jupiter==.",
    section4: "Räumliche Karten",
    spatialIntro: "# Räumliche Karten\n\nVerbinde Textknoten mit Kanten. Jede Kante wird zu einer Karte — der Ausgangsknoten ist die Vorderseite, der Zielknoten die Rückseite, die Kantenbeschriftung ein Hinweis. Karten in einer Kette (A → B → C) erzeugen alle Karten. Einzelne (nicht verbundene) Knoten werden weiterhin mit den vier Formaten oben gelesen.",
    spatialNodePhotosynthesis: "Photosynthese",
    spatialNodeSunlight: "Sonnenlicht",
    spatialNodeGlucose: "Glukose",
    spatialEdgeNeeds: "braucht",
    spatialEdgeProduces: "erzeugt",
  },

  testDeck: {
    filename: "Decks — Erste Schritte.md",
    title: "Erste Schritte mit Decks",
    intro:
      "Willkommen bei Decks! Um diese Datei (oder eine beliebige andere) in einen Karteikarten-Stapel zu verwandeln, füge einfach das Tag `{tag}` zu den Frontmatter-Eigenschaften oder irgendwo im Text hinzu.",
    formatsHint:
      "Decks unterstützt fünf Hauptformate. Wähle dasjenige, das am besten zu deinen Notizen passt.",
    section1: "1. Überschrift-Absatz-Format",
    section1Body:
      "Standardmäßig verwendet Decks **H2-Überschriften (`##`)** als Vorderseite der Karte und den direkt darunter liegenden Absatz als Rückseite. *(Du kannst diese Überschriftenebene in den Deck-Einstellungen ändern).*",
    q1: "Was ist Spaced Repetition?",
    a1: "Eine Lerntechnik, die Wiederholungen in immer größeren Abständen plant, basierend darauf, wie gut man sich erinnert.",
    q2: "Welches Tag markiert eine Datei als Karteikarten-Stapel?",
    a2: "Das Tag `{tag}`. Füge es zur `tags`-Liste im Frontmatter oder irgendwo im Text hinzu.",
    section2: "2. Tabellen-Format",
    section2Body:
      "Du kannst zweispaltige Markdown-Tabellen verwenden, um Karten in großen Mengen zu erstellen.\n**Wichtige Regel:** Die Tabelle *muss* direkt unter einer Überschrift stehen. Diese Überschrift fungiert als Container für die Karten.",
    fsrsConceptsHeading: "FSRS-Konzepte",
    colFront: "Vorderseite",
    colBack: "Rückseite",
    colNotes: "Notizen",
    row1Front: "Wofür steht FSRS?",
    row1Back: "Free Spaced Repetition Scheduler",
    row1Notes: "Der Algorithmus, den Decks zum Planen von Karten verwendet",
    row2Front: "Was sind die vier Bewertungsstufen?",
    row2Back: "Noch einmal, Schwer, Gut, Einfach",
    row2Notes:
      "„Noch einmal“ setzt die Karte zurück; „Einfach“ gibt den längsten Intervall",
    tableNote:
      "*(Hinweis: Die erste Spalte ist die Vorderseite, die zweite die Rückseite. Die optionale dritte Spalte ist für Notizen/Hinweise gedacht).*",
    section3: "3. Lückentexte (Cloze)",
    section3Body:
      "Verwende die Syntax `==hervorgehoben==`, um Lückentext-Karten zu erstellen. Jede Hervorhebung wird zu einer eigenen Karte.",
    solarHeading: "Das Sonnensystem",
    solarBody:
      "Die ==Sonne== ist der Stern im Zentrum unseres Sonnensystems. Der nächste Planet ist der ==Merkur== und der größte Planet ist der ==Jupiter==.",
    section4: "4. Bildverdeckung (Image occlusion)",
    section4Body:
      "Kombiniere ein Bild mit einer nummerierten Liste. Das Bild zeigt die markierten Bereiche, und jeder Listenpunkt wird zu einer separaten Karte.",
    bonesHeading: "Knochen des Arms",
    bone1: "Humerus (Oberarmknochen)",
    bone2: "Radius (Speiche)",
    bone3: "Ulna (Elle)",
    section5: "5. Vorlagen (eigene Kartenlayouts)",
    section5Body:
      "Markiere die Überschrift einer Tabelle, und Decks rendert jede Zeile über eine Vorlagendatei aus deinem Vorlagenordner (Einstellungen → Kartenvorlagen). Die Überschrift dieser Tabelle trägt #{tag}, also werden ihre Zeilen mit der passenden Vorlage gerendert — bearbeite eine Zeile im Kartenmanager und du bekommst ein Eingabefeld pro Spalte.",
    templatesTableHeading: "Kanji-Wortschatz",
    templateTag: "vokabeln",
    templateFolderName: "Decks-Vorlagen",
    templateFileName: "Vokabelkarte.md",
    tplColWord: "Wort",
    tplColReading: "Lesung",
    tplColMeaning: "Bedeutung",
    tplRow1Word: "火",
    tplRow1Reading: "ひ",
    tplRow1Meaning: "Feuer",
    tplRow2Word: "水",
    tplRow2Reading: "みず",
    tplRow2Meaning: "Wasser",
  },
  anki: {
    title: "Aus Anki importieren",
    description: "Importiere einen Anki-.apkg-Export in Decks — deine Stapel, Karten, Medien und dein Lernverlauf kommen mit.",
    howItWorks: "So funktioniert's: Jeder Anki-Stapel wird zu einer Decks-Datei (die Stapelhierarchie bleibt als Ordner erhalten), Notizen werden in saubere Vorder-/Rückseiten-Karten umgewandelt, Medien werden in deinen Tresor kopiert, und der Planungs-/Lernverlauf wird importiert, sodass die Karten dort weiterlaufen, wo Anki aufgehört hat.",
    fileLabel: "Anki-Datei",
    fileDesc: "Wähle eine aus Anki exportierte .apkg-Datei.",
    noFileSelected: "Keine Datei ausgewählt",
    targetFolderName: "Zielordner",
    targetFolderDesc: "Wo die importierten Decks-Dateien erstellt werden.",
    profileName: "Profil",
    profileDesc: "Profil, das auf die importierten Stapel angewendet wird (legt Überschriftenebene und Planung fest).",
    splitName: "Große Stapel in mehrere Dateien aufteilen",
    splitDesc:
      "Teilt Stapel mit vielen Karten oder Medien in nummerierte Teildateien auf, damit jede in Obsidian schnell zu öffnen bleibt. Unterstapel werden immer als separate Dateien importiert.",
    chunkSizeName: "Karten pro Datei",
    chunkSizeDesc: "Maximale Anzahl Karten pro Teildatei, wenn ein Stapel aufgeteilt wird.",
    scanButton: "Scannen",
    scanning: "Wird gescannt…",
    scanSummary: "{decks} Stapel, {cards} Karten, {history} mit Lernverlauf, {media} Mediendateien gefunden.",
    scanFailed: "Scan fehlgeschlagen. Details in der Konsole.",
    importButton: "Importieren",
    importing: "Wird importiert…",
    importFailed: "Import fehlgeschlagen. Details in der Konsole.",
    selectFileFirst: "Wähle zuerst eine .apkg-Datei aus.",
    notAnApkg: "Das ist keine Anki-.apkg-Datei. Exportiere ein Deck aus Anki und wähle dieses aus.",
    progressReading: "Sammlung wird gelesen…",
    progressWriting: "{deck} wird geschrieben…",
    progressSyncing: "Stapel werden synchronisiert…",
    progressImporting: "Lernverlauf wird importiert…",
    progressCopyingMedia: "Medien werden kopiert {done}/{total}…",
    successNotice: "{cards} Karten in {decks} Stapel importiert; Verlauf für {history}.",
    doneSummary: "Fertig: {decks} Stapel, {cards} Karten, {media} Medien, {history} mit Verlauf.",
    cardsMerged:
      "{count} Karte(n) hatten dieselbe Vorderseite wie eine andere und wurden zu einer zusammengeführt.",
    mediaIndexingNotice:
      "Obsidian indiziert {count} importierte Mediendateien im Hintergrund – Bilder und Audio können einen Moment brauchen, bis sie erscheinen.",
    cancel: "Abbrechen",
  },

  srMigration: {
    title: "Von Spaced Repetition migrieren",
    description:
      "Wandle Notizen des Spaced-Repetition-Plugins in das Decks-Format um und übernimm dabei deine Karten und deinen Wiederholungsverlauf.",
    howItWorks:
      "So funktioniert es: Die Lernkarten jeder Notiz werden in eine eigene Decks-Datei extrahiert, während die Notiz selbst als sauberer, lesbarer Text neu geschrieben wird (alte Karten-Syntax entfernt) und der Wiederholungsverlauf erhalten bleibt; umgekehrte Karten behalten beide Richtungen. Die neuen Dateien werden im Zielordner erstellt (deine Ordnerstruktur bleibt erhalten), und die migrierte Notiz verweist über das Frontmatter auf ihre Kartendatei – und zurück auf das Original. Deine ursprünglichen Notizen bleiben unangetastet. Ein erneutes Ausführen der Migration überschreibt zuvor erzeugte Dateien.",
    sourceFolderName: "Quellordner",
    sourceFolderDesc:
      "Ordner, der nach alten Karten durchsucht wird. Leer lassen, um den gesamten Tresor zu durchsuchen.",
    targetFolderName: "Zielordner",
    targetFolderDesc: "Wo die neuen Decks-Dateien erstellt werden.",
    sameFolderName: "Neben der Quellnotiz ausgeben",
    sameFolderDesc:
      "Migrierte Notiz(en) im Ordner der jeweiligen Quellnotiz statt in einem Zielordner erstellen.",
    srTagName: "Spaced-Repetition-Tag",
    srTagDesc:
      "Der Basis-Tag, den deine alten Karten verwenden. Untertags werden in deinen Decks-Tag übersetzt.",
    profileName: "Profil",
    profileDesc:
      "Profil, das auf die migrierten Stapel angewendet wird (legt Überschriftenebene und Planung fest).",
    reviewTagName: "Ganzseiten-Wiederholungstag",
    reviewTagDesc:
      "Notizen mit diesem Tag werden als einzelne Ganzseiten-Karte migriert (Dateiname = Vorderseite).",
    hintLabel: "Hinweis",
    flashcardsProperty: "Lernkarten",
    flashcardsSuffix: " (Lernkarten)",
    sourceProperty: "Ursprungsnotiz",
    clozeSepName: "Cloze-Trennzeichen",
    clozeSepDesc:
      "Trennzeichen innerhalb eines Cloze für Nummer/Antwort/Hinweis (Standard \";;\"). \"::\" wird ebenfalls akzeptiert.",
    progressWriting: "Migriere {file}…",
    progressSyncing: "Stapel werden synchronisiert…",
    progressRestoring: "Wiederholungsverlauf wird wiederhergestellt…",
    inlineSepName: "Inline-Trennzeichen",
    inlineSepDesc:
      "Trennzeichen für einzeilige Karten (Standard \"::\"). Die umgekehrte Form wiederholt das letzte Zeichen.",
    multiSepName: "Mehrzeiliges Trennzeichen",
    multiSepDesc:
      "Trennzeichen für mehrzeilige Karten in einer eigenen Zeile (Standard \"?\"). Die umgekehrte Form wiederholt das letzte Zeichen.",
    formatName: "Migrationsformat",
    formatDesc:
      "Wie die migrierten Karten angeordnet werden. Smart leitet einzeilige Karten in Tabellen und mehrzeilige Karten in Überschriften.",
    formatSmart: "Smart-Erkennung (empfohlen)",
    formatHeaders: "Alle Überschriften",
    formatTables: "Alle Tabellen",
    formatTablesWarning:
      "Alle Tabellen: Mehrzeilige Karten werden in Tabellenzellen zusammengefasst (Zeilenumbrüche werden zu <br>); komplexe Inhalte werden möglicherweise nicht dargestellt.",
    deleteForcesHeaders: "",
    deleteModeName: "Originale nach der Migration archivieren",
    deleteModeDesc:
      "Verschiebt jede ursprüngliche Notiz nach der Migration in einen _Legacy_SR_Archive-Ordner – zu 100 % sicher aufbewahrt, nur außerhalb deines aktiven Arbeitsbereichs.",
    deleteWarning:
      "Deine ursprünglichen Notizen werden nach _Legacy_SR_Archive verschoben (nicht gelöscht); entferne diesen Ordner selbst, wann immer du bereit bist.",
    scanButton: "Scannen",
    scanning: "Wird gescannt…",
    scanSummary:
      "{files} Datei(en), {cards} Karte(n), {history} mit Wiederholungsverlauf gefunden.",
    scanFailed: "Scan fehlgeschlagen. Siehe Konsole für Details.",
    migrateButton: "Migrieren",
    migrating: "Wird migriert…",
    migrateFailed: "Migration fehlgeschlagen. Siehe Konsole für Details.",
    successNotice:
      "{cards} Karte(n) in {files} Datei(en) migriert; Verlauf für {history} wiederhergestellt.",
    doneSummary:
      "Fertig: {files} Datei(en), {cards} Karte(n), {history} mit Verlauf.",
    cancel: "Abbrechen",
  },

  occlusion: {
    studioTitle: "Bildverdeckung bearbeiten",
    drawHint: "Zum Zeichnen eines Felds auf dem Bild ziehen",
    boxes: "Felder",
    noBoxes: "Noch keine Felder. Zum Zeichnen auf dem Bild ziehen.",
    selectBox: "Wähle ein Feld, um seine Antwort hinzuzufügen.",
    answerLabel: "Antwort (Markdown / LaTeX)",
    answerPlaceholder: "Leer lassen für ein reines Verdeckungsfeld",
    emptyAnswer: "Leer",
    deleteBox: "Feld löschen",
    resize: "Größe ändern",
    cancel: "Abbrechen",
    save: "Speichern",
    edit: "Bearbeiten",
    imageNotFound: "Bild nicht gefunden: {name}",
    parseError: "Fehler bei der Bildverdeckung: {error}",
    pickImage: "Bild zum Verdecken auswählen",
    noImages: "Keine Bilder im Tresor gefunden.",
    saveFileNotFound: "Notiz zum Speichern der Verdeckung nicht gefunden.",
    blockMoved: "Verdeckungsblock nicht gefunden – er wurde möglicherweise verschoben.",
    saveFailed: "Speichern der Verdeckung fehlgeschlagen.",
  },

  commands: {
    createExamDemoDeck: "Demo-Prüfungsstapel erstellen",
    cleanupOrphanedCards: "Verwaiste Karten bereinigen (ohne Stapel)",
    migrateFromSr: "Von Spaced-Repetition-Plugin migrieren",
    importFromAnki: "Aus Anki importieren (.apkg)",
    insertImageOcclusion: "Bildverdeckung an Cursorposition erstellen",
    showPanel: "Karteikarten-Panel anzeigen",
    showReleaseNotes: "Versionshinweise anzeigen",
    openManager: "Karteikarten-Manager öffnen",
    createTestDeck: "Teststapel erstellen",
    createCanvasTestDeck: "Canvas-Teststapel erstellen",
    fullResync:
      "Vollständige Neusynchronisierung erzwingen (jeden Stapel neu parsen)",
    openReview: "Wiederholungsfenster öffnen",
    openAiGenerator: "Karteikarten mit KI generieren",
  },

  notices: {
    orphansCleaned: "{count} verwaiste Karte(n) entfernt.",
    reparsing:
      "Jeder Stapel wird neu eingelesen. Dies kann einen Moment dauern…",
    resyncComplete: "Vollständige Neusynchronisierung abgeschlossen.",
    resyncFailed:
      "Neusynchronisierung fehlgeschlagen. Siehe Konsole für Details.",
    loadFailed:
      "Plugin konnte nicht geladen werden. Siehe Konsole für Details.",
    languageChanged:
      "Sprache geändert. Plugin neu laden, damit die Änderung wirksam wird.",
    noCardsInDeck: "Keine Karten gefunden in {deckName}",
    customDeckCreated:
      'Benutzerdefinierten Stapel "{name}" mit {count} Karten erstellt',
    customDeckUpdated: 'Benutzerdefinierter Stapel "{name}" aktualisiert',
    customDeckDeleted: 'Benutzerdefinierter Stapel "{name}" gelöscht',
    customDeckRenamed: 'Benutzerdefinierter Stapel umbenannt in "{name}"',
    deckResetSuccess: "Stapel-Fortschritt erfolgreich zurückgesetzt",
    deckResetFailed: "Zurücksetzen des Stapel-Fortschritts fehlgeschlagen",
    reviewStartFailed: "Fehler beim Starten der Wiederholung. Siehe Konsole.",
    sessionComplete: "Wiederholungssitzung abgeschlossen",
    sessionCompleteCount: "{count} Karten wiederholt",
    duplicatesFound: "{count} doppelte Karte(n) in {deckName} gefunden",
    trainedParamsCleared:
      "Trainierte Parameter gelöscht, Standardwerte wiederhergestellt.",
    creatingBackup: "Sicherung wird erstellt…",
    backupCreated: "✅ Sicherung erstellt: {filename}",
    backupFailed: "❌ Sicherung fehlgeschlagen: {message}",
    selectBackupPrompt: "Bitte eine Sicherung zum Wiederherstellen auswählen",
    restoringBackup: "Sicherung wird wiederhergestellt…",
    restoringFromFile: "Wiederherstellung aus {filename}…",
    restoringFromFileProgress: "Wiederherstellung aus {filename}: {progress}%",
    restoreProgress:
      "Sicherung wird wiederhergestellt: {progress}% ({current}/{total})",
    restoreFailed: "❌ Wiederherstellung fehlgeschlagen: {message}",
    cardsRebuilt: "Planung für {count} Karten aus dem Lernverlauf wiederhergestellt.",
    backupRestoreFailed:
      "❌ Wiederherstellung der Sicherung fehlgeschlagen: {message}",
    dropdownNotFound: "Dropdown nicht gefunden",
    noBackupsFound: "Keine Sicherungen gefunden",
    backupsFound: "{count} Sicherung(en) gefunden",
    backupListFailed: "Sicherungsliste konnte nicht geladen werden: {message}",
    purgingDatabase: "Datenbank wird geleert…",
    databasePurged: "✅ Datenbank geleert und neu aufgebaut",
    purgeFailed:
      "Datenbank konnte nicht geleert werden. Siehe Konsole für Details.",
    purgeConfirmMismatch:
      "Bestätigungstext stimmt nicht überein. Datenbankleerung abgebrochen.",
    errorLoadingDecks:
      "Fehler beim Laden der Stapel. Siehe Konsole für Details.",
    errorStartingReview:
      "Fehler beim Starten der Wiederholung. Siehe Konsole für Details.",
    errorStartingBrowse:
      "Fehler beim Starten des Durchsuchens. Siehe Konsole für Details.",
    errorStartingCram: "Fehler beim Starten des Paukens. Siehe Konsole für Details.",
    noCardsFoundInDeck: "Keine Karten gefunden in {deckName}",
    noCardsDueForReview: "Keine Karten zur Wiederholung fällig in {deckName}",
    noCardsFoundInGroup: 'Keine Karten gefunden in "{name}"',
    noCardsDueInGroup: 'Keine Karten fällig in "{name}" ({count} Dateien)',
    noCardsDueInCustomDeck: 'Keine Karten fällig in "{name}" ({count} Karten)',
    fileNotFound: "Datei nicht gefunden: {path}",
    reviewSessionCompleteFor:
      "Wiederholungssitzung abgeschlossen für {deckName}!",
    filterDeckUpdated: 'Filterstapel "{name}" aktualisiert',
    customDeckUpdatedCounts:
      '"{name}" aktualisiert — {added} hinzugefügt, {removed} entfernt',
    failedToSave: "Speichern fehlgeschlagen: {message}",
    failedToCreateDeck: "Stapel konnte nicht erstellt werden: {message}",
    failedToCreateFilterDeck:
      "Filterstapel konnte nicht erstellt werden: {message}",
    failedToAddCards: "Karten konnten nicht hinzugefügt werden: {message}",
    filterDeckCreated: 'Filterstapel "{name}" erstellt',
    cardsAddedToCustomDeck:
      "{count} Karten zum benutzerdefinierten Stapel hinzugefügt",
    dailyLimitsReached: "\n\nTägliche Limits erreicht:",
    dailyNewLimit: "\nNeue Karten: {used}/{max}",
    dailyReviewLimit: "\nWiederholungskarten: {used}/{max}",
    dailyNewOnlyLimit:
      "\n\nTägliches Limit für neue Karten erreicht: {used}/{max}",
    dailyReviewOnlyLimit:
      "\n\nTägliches Limit für Wiederholungskarten erreicht: {used}/{max}",
    dailyProgressHeader: "Tagesfortschritt für {deckName}:\n",
    dailyProgressNewDisabled: "Neue Karten: DEAKTIVIERT (0 pro Tag erlaubt)\n",
    dailyProgressNewExceeded:
      "Neue Karten: {used}/{max} (LIMIT ÜBERSCHRITTEN)\n",
    dailyProgressNewRemaining:
      "Neue Karten: {used}/{max} ({remaining} verbleibend)\n",
    dailyProgressReviewDisabled:
      "Wiederholungskarten: DEAKTIVIERT (0 pro Tag erlaubt)\n",
    dailyProgressReviewExceeded:
      "Wiederholungskarten: {used}/{max} (LIMIT ÜBERSCHRITTEN)\n",
    dailyProgressReviewRemaining:
      "Wiederholungskarten: {used}/{max} ({remaining} verbleibend)\n",
    dailyProgressOnlyLearning:
      "\n\nHinweis: Nur Lernkarten werden angezeigt (Limits überschritten)",
    noFlashcardsToExport:
      "Keine Karten in diesem Stapel zum Exportieren gefunden",
    ankiExportSuccess:
      "{count} Karten erfolgreich vom {type} ins Anki-Format exportiert",
    ankiExportFailed: "Export in das Anki-Format fehlgeschlagen",
    ankiExportTypeDeck: "Stapel",
    ankiExportTypeTagGroup: "Tag-Gruppe",
  },

  examDeck: {
    filename: "Decks — Demo-Prüfung.md",
    title: "Demo-Prüfung",
    intro:
      "Diese Notiz ist ein Prüfungsstapel: Durch das Tag {tag} wird jede Überschrift mit einer Aufgabenliste darunter zu einer Multiple-Choice-Frage — markiere die richtigen Antworten mit `[x]`. Klicke auf die Zeile des Stapels (oder nutze das Stapel-Menü) und wähle „Prüfung starten“, um es auszuprobieren.",
    formatsHint:
      "Unten folgen alle Frageformen der Reihe nach: eine richtige Antwort, mehrere richtige Antworten (Alles-oder-nichts-Bewertung), Wahr oder Falsch, eine getippte Kurzantwort, Tabellenzeilen und Lückentexte. Öffne diese Datei im Quelltextmodus, um zu sehen, wie sie jeweils geschrieben werden — ein `%%comment%%` unter einer Frage wird zu einer Erklärung, die in der Ergebnisübersicht angezeigt wird.",
    mcq1Front: "Welches Element ist ein Edelgas?",
    mcq1OptA: "Sauerstoff",
    mcq1OptB: "Argon",
    mcq1OptC: "Stickstoff",
    mcq1OptD: "Chlor",
    mcq1Note: "Elemente der Gruppe 18 haben eine volle Valenzschale, was sie reaktionsträge macht.",
    mcq2Front: "Welche davon sind Edelgase?",
    mcq2OptA: "Helium",
    mcq2OptB: "Sauerstoff",
    mcq2OptC: "Argon",
    mcq2OptD: "Neon",
    tfFront: "Das Mitochondrium ist der Ort der oxidativen Phosphorylierung.",
    tfTrue: "Wahr",
    tfFalse: "Falsch",
    saFront: "Was ist das Kraftwerk der Zelle?",
    saBack: "Das Mitochondrium.",
    tableHeading: "Elementsymbole",
    tColFront: "Vorderseite",
    tColBack: "Rückseite",
    tColNotes: "Notizen",
    tRow1Front: "Symbol für Gold?",
    tRow1Back: "Au",
    tRow1Notes: "Vom lateinischen aurum.",
    tRow2Front: "Symbol für Eisen?",
    tRow2Back: "Fe",
    tRow2Notes: "Vom lateinischen ferrum.",
    clozeHeading: "Wasser",
    clozeBody: "Wasser besteht aus ==Wasserstoff== und ==Sauerstoff==.",
    outro:
      "Im Dialog „Prüfung einrichten“ kannst du die Anzahl der Fragen, ein Zeitlimit, die Bestehensgrenze und mehr festlegen. Wiederholungen und Prüfungen erfassen ihre Ergebnisse getrennt, du kannst also beides mit denselben Karten nutzen.",
  },
  exam: {
    startExam: "Prüfung starten",
    startReview: "Wiederholung starten",
    setupTitle: "Prüfung einrichten",
    eligibleCount: "{count} Fragen verfügbar",
    skippedCount: "{count} übersprungen (ungültig oder nicht bewertbar)",
    skippedHint: "Überprüfe markierte Karten im Karteikarten-Manager.",
    noQuestions: "Keine Prüfungsfragen in dieser Auswahl verfügbar.",
    questionCountSetting: "Anzahl der Fragen",
    questionCountAll: "0 = alle",
    timeLimitSetting: "Zeitlimit (Minuten)",
    timeLimitOff: "0 = kein Limit",
    passScoreSetting: "Mindestpunktzahl zum Bestehen (%)",
    shuffleQuestionsSetting: "Fragen mischen",
    shuffleOptionsSetting: "Antwortoptionen mischen",
    feedbackTimingSetting: "Feedback",
    feedbackEnd: "Am Ende",
    feedbackImmediate: "Nach jeder Frage",
    selectionModeSetting: "Fragenauswahl",
    selectionRandom: "Zufällig",
    selectionSequential: "In Dateireihenfolge",
    typedGradingSetting: "Bewertung getippter Antworten",
    gradingExact: "Exakte Übereinstimmung",
    gradingTolerant: "Tolerant bei kleinen Tippfehlern",
    gradingSelf: "Selbstbewertung",
    optionLabelsSetting: "Optionsbeschriftungen",
    optionLabelsLetters: "Buchstaben (a, b, c)",
    optionLabelsNumbers: "Zahlen (1, 2, 3)",
    start: "Starten",
    cancel: "Abbrechen",
    questionOf: "Frage {current} von {total}",
    typeAnswerPlaceholder: "Antwort eingeben",
    submitAnswer: "Absenden",
    selfPromptQuestion: "Lagst du richtig?",
    selfYes: "Ich lag richtig",
    selfNo: "Ich lag falsch",
    next: "Weiter",
    previous: "Zurück",
    submitExam: "Prüfung abgeben",
    quit: "Beenden",
    quitConfirmTitle: "Prüfung abbrechen",
    quitConfirmBody: "Diese Prüfung abbrechen? Es wird nichts gespeichert.",
    submitConfirmTitle: "Prüfung abgeben",
    submitConfirmUnanswered: "{count} unbeantwortete Fragen werden als falsch gewertet.",
    confirm: "Bestätigen",
    correct: "Richtig",
    incorrect: "Falsch",
    unanswered: "Unbeantwortet",
    yourAnswer: "Deine Antwort",
    correctAnswer: "Richtige Antwort",
    resultsTitle: "Ergebnisse",
    scoreLabel: "Punktzahl",
    passed: "Bestanden",
    failed: "Nicht bestanden",
    timeUsed: "Benötigte Zeit",
    previousAttempts: "Frühere Versuche",
    retake: "Erneut versuchen",
    close: "Schließen",
    notes: "Notizen",
    invalidQuestionBadge: "Ungültige Frage",
    answerTooLongBadge: "Nicht bewertbar",
    invalidReasonNoCorrect: "Keine richtige Antwort markiert",
    invalidReasonSingleOption: "Mindestens zwei Optionen erforderlich",
    invalidReasonMixedList: "Gemischte Liste unter der Überschrift",
    invalidReasonNested: "Verschachtelte Aufgabenlisten werden nicht unterstützt",
    invalidReasonEmptyOption: "Leerer Optionstext",
    answerTooLongTooltip: "Antwort zu lang für die Textbewertung — kürze sie oder ändere den Bewertungsmodus des Stapels",
    examEnabledSetting: "Prüfungsfragen",
    examEnabledDesc: "Eine Aufgabenliste unter einer Überschrift erzeugt eine Multiple-Choice-Karte",
    examSettingsHeading: "Prüfungsvorgaben",
    typeFlipWarningTitle: "Prüfungsfragen aktivieren",
    typeFlipWarningBody: "{count} bereits wiederholte Karten werden zu Prüfungsfragen. Ihr Wiederholungsverlauf wird auf einem ruhenden Token geparkt und stellt sich automatisch wieder her, wenn du die Option wieder deaktivierst.",
    checkAnswer: "Antwort prüfen",
    selfGradePrompt: "Hattest du recht?",
    nextQuestion: "Nächste Frage",
    previousQuestion: "Vorherige Frage",
    question: "Frage",
    multipleCorrect: "Diese Frage hat mehr als eine richtige Antwort.",
    timeRemaining: "Verbleibende Zeit:",
    loadingExam: "Prüfung wird geladen…",
    loadingAnswers: "Antworten werden geladen…",
    noStoredAnswers: "Für diesen Versuch wurden keine Antworten gespeichert.",
    closeExam: "Prüfung schließen",
    everyQuestion: "Jede Frage",
    settingCount: "Fragen in dieser Prüfung. 0 = alle infrage kommenden.",
    settingTimeLimit: "Countdown für die gesamte Prüfung. 0 = kein Limit.",
    settingPassMark: "Prozentsatz zum Bestehen (0–100).",
    settingSelection: "Wie Fragen aus dem Stapel gewählt werden.",
    settingFeedback: "Wann Antworten gezeigt werden.",
    settingGrading: "Wie getippte Antworten geprüft werden. Selbst bewertet lässt auch längere Antworten zu.",
    settingShuffleQuestions: "Reihenfolge der Fragen zufällig anordnen.",
    settingShuffleOptions: "Reihenfolge der Antwortoptionen zufällig anordnen.",
    settingOptionLabels: "Wie Antwortoptionen beschriftet werden.",
    flagQuestion: "Zum Wiederkommen markieren",
    unflagQuestion: "Markierung dieser Frage entfernen",
    goToQuestion: "Zu Frage {number} springen",
  },
  deckList: {
    srMigration: "Von Spaced Repetition migrieren",
    ankiImport: "Aus Anki importieren",
    moreActions: "Weitere Aktionen",
    exportToAnki: "Nach Anki exportieren",
    title: "Stapel",
    emptyNoDecks: "Keine Stapel gefunden.",
    emptyNoFilterMatch: "Keine Stapel entsprechen dem Filter.",
    emptyFilterHint: "Versuche andere Suchbegriffe.",
    filterPlaceholder: "Filter...",
    clearFilter: "Filter löschen",
    search: "Suchen",
    refresh: "Aktualisieren",
    syncing: "Synchronisierung im Hintergrund…",
    aiGenerate: "Karteikarten mit KI generieren",
    settings: "Einstellungen",
    statistics: "Statistik anzeigen",
    profiles: "Profile verwalten",
    openManager: "Karteikarten-Manager öffnen",
    configureDeck: "Stapel konfigurieren",
    pinToTop: "Oben anheften",
    unpinFromTop: "Anheftung lösen",
    pinned: "Angeheftet",
    review: "Wiederholen",
    browse: "Durchsuchen",
    browseAllCards: "Alle Karten durchsuchen",
    openSourceFile: "Quelldatei öffnen",
    cram: "Pauken",
    resumeCram: "Pauken fortsetzen",
    configureProfile: "Profil konfigurieren",
    rename: "Umbenennen",
    delete: "Löschen",
    editFilter: "Filter bearbeiten",
    editCards: "Karten bearbeiten",
    sortByName: "Nach Name sortieren",
    sortByNewCardCount: "Nach Anzahl neuer Karten sortieren",
    sortByDueCardCount: "Nach fälligen Karten sortieren",
    columnDeck: "Stapel",
    columnTagGroup: "Tag-Gruppe",
    columnCustomDeck: "Benutzerdefinierter Stapel",
    columnNew: "Neu",
    columnDue: "Fäll.",
    statTotal: "Gesamt",
    statPastMonth: "Letzter Monat",
    statPastWeek: "Letzte Woche",
    optionsTagGroup: "Optionen Tag-Gruppe",
    optionsCustomDeck: "Optionen benutzerdefinierter Stapel",
    optionsDeck: "Stapel-Optionen",
    optionsForItem: "Optionen für {name}",
    clickToReview: "Klicken, um {name} zu wiederholen",
    treeView: "Baumansicht",
    flatView: "Flache Ansicht",
    viewToggle: "Stapellisten-Ansicht",
    reviewAllUnder: "Alle Karten unter {name} wiederholen",
    studyAll: "Alle lernen",
    collapseAll: "Alle einklappen",
    collapse: "Einklappen",
    expand: "Ausklappen",
    resetProgress: "Fortschritt zurücksetzen",
    deleteCustomDeckTitle: "Benutzerdefinierten Stapel löschen",
    deleteCustomDeckMessage:
      'Benutzerdefinierten Stapel "{name}" löschen? Die Karten selbst werden nicht gelöscht.',
    resetCustomDeckProgress:
      'Allen Fortschritt für "{name}" zurücksetzen? Alle Karten dieses Stapels werden auf neu zurückgesetzt und der Wiederholungsverlauf wird gelöscht. Diese Aktion kann nicht rückgängig gemacht werden.',
    progressResetFor: 'Fortschritt zurückgesetzt für "{name}"',
    resetCustomDeckTitle:
      "Fortschritt des benutzerdefinierten Stapels zurücksetzen",
    resetCustomDeckMessage:
      'Fortschritt der Karten in "{name}" zurücksetzen? Der Wiederholungsverlauf für diese Karten wird gelöscht und sie kehren in den neuen Zustand zurück.',
    resetCustomDeckFailed:
      "Zurücksetzen des benutzerdefinierten Stapels fehlgeschlagen",
    confirmDelete: "Löschen",
    newCardsTooltip: "Heute verbleibende neue Karten: {remaining} / {limit}",
    reviewCardsTooltip:
      "Heute verbleibende Wiederholungskarten: {remaining} / {limit}",
    tabFiles: "Dateien",
    tabTags: "Tags",
    tabCustom: "Benutzerdefiniert",
    tagYourNotes: "Versehe deine Notizen mit {tag}, um Stapel zu erstellen.",
    filesCount: "({count} Dateien)",
    cardsCount: "({count} Karten)",
    newCardsGroupTooltip:
      "{count} neue Karten heute verfügbar (Limit: {limit} pro Stapel)",
    newCardsLimitTooltip:
      "{count} neue Karten heute verfügbar (Limit: {limit})",
    newCardsDueTooltip: "{count} neue Karten fällig",
    reviewCardsGroupTooltip:
      "{count} Wiederholungskarten heute verfügbar (Limit: {limit} pro Stapel)",
    reviewCardsLimitTooltip:
      "{count} Wiederholungskarten heute verfügbar (Limit: {limit})",
    reviewCardsDueTooltip: "{count} Wiederholungskarten fällig",
    studiedSummary: "{cards} Karten in {hours} heute gelernt ({pace})",
    statCardsToday: "Karten heute",
  },

  review: {
    showAnswer: "Antwort anzeigen",
    showAnswerButton: "Antwort anzeigen",
    again: "Nochmal",
    hard: "Schwer",
    good: "Gut",
    easy: "Leicht",
    edit: "Karte bearbeiten",
    delete: "Karte löschen",
    skip: "Überspringen",
    undo: "Rückgängig",
    undoTooltip: "Letzte Wiederholung rückgängig machen (Cmd/Strg+Z)",
    flag: "Karte markieren",
    unflag: "Markierung entfernen",
    suspend: "Aussetzen",
    bury: "Vergraben",
    reset: "Karte zurücksetzen",
    unsuspend: "Aussetzung aufheben",
    unbury: "Wieder aufdecken",
    cardActions: "Kartenaktionen",
    cardSuspended: "Karte ausgesetzt",
    cardUnsuspended: "Aussetzung der Karte aufgehoben",
    cardBuried: "Karte bis morgen vergraben",
    cardReset: "Kartenfortschritt zurückgesetzt",
    resetCardConfirmTitle: "Karte zurücksetzen?",
    resetCardConfirmMessage:
      "Fortschritt für diese Karte zurücksetzen? Der Lernverlauf wird gelöscht und die Karte kehrt in den Neu-Zustand zurück. Dies kann nicht rückgängig gemacht werden.",
    close: "Schließen",
    front: "Vorderseite",
    back: "Rückseite",
    progress: "Fortschritt",
    sessionEnded: "Sitzung beendet",
    sessionStats: "{reviewed} wiederholt • {remaining} verbleibend",
    noMoreCards: "Keine Karten zum Wiederholen",
    keyboardHint: "1-4 drücken zum Bewerten, Leertaste zum Umdrehen",
    timeUp: "Sitzungszeit ist abgelaufen",
    revealCloze: "Lücke aufdecken",
    nextCloze: "Nächste Lücke",
    openSource: "Quellnotiz öffnen",
    openSourceFile: "Quelldatei öffnen",
    playAudio: "Vorlesen",
    stopAudio: "Vorlesen stoppen",
    cardLeechAndDense:
      "Karte wird wiederholt vergessen und ist komplex — überarbeiten erwägen",
    cardLeech: "Karte wird wiederholt vergessen — neu schreiben erwägen",
    cardDense: "Karte ist komplex. Aufteilen erwägen.",
    copyContent: "Inhalt kopieren",
    toggleNotes: "Notizen umschalten (N)",
    scratchpad: "Notizblock",
    scratchpadPen: "Stift",
    scratchpadEraser: "Radierer",
    scratchpadText: "Text",
    scratchpadClear: "Löschen",
    scratchpadTextPlaceholder: "Notizen eingeben…",
    searchCardsPlaceholder: "Karten suchen...",
    noCardsMatch: "Keine Karten passen",
    closeSearch: "Suche schließen",
    searchCards: "Karten suchen",
    cardOfCount: "Karte {current} / {total}",
    cardOfCountAlt: "Karte {current} von {total}",
    reviewedLabel: "Wiederholt: {count}",
    remainingCount: "{count} verbleibend",
    moreCardsAvailable: "Weitere Karten verfügbar",
    sessionComplete: "Sitzung abgeschlossen",
    timeRemaining: "Verbleibende Zeit:",
    finish: "Fertig",
    next: "Weiter",
    previous: "Zurück",
    spaceShortcut: "Leerzeichen",
    copyAnswer: "Antwort kopieren",
    editMasks: "Masken bearbeiten",
    refactorInstruction: "Anweisung zur Überarbeitung",
    refactorPlaceholder: "Wie soll sich diese Karte ändern? z. B. Antwort kürzen, LaTeX korrigieren",
    refactorRewritten: "Oben neu geschrieben — prüfe es und speichere dann.",
    refactorSaved: "In der Quellnotiz gespeichert und neu abgeglichen.",
    clozeHint: "Setze jede Antwort in ==doppelte Gleichheitszeichen==, damit sie ein Lückentext bleibt.",
    typeEditedElsewhere: "Dieser Kartentyp wird dort bearbeitet, wo er erstellt wurde, nicht während der Wiederholung.",
    loading: "Wiederholung wird geladen…",
    backToDecks: "Zurück zu den Stapeln",
    closeReview: "Wiederholung schließen",
    closeBrowse: "Durchsuchen schließen",
    closeCram: "Pauken schließen",
    loadingCram: "Pauken wird geladen…",
    nothingToBrowse: "Nichts zum Durchsuchen",
    deckHasNoCards: "Dieser Stapel hat noch keine Karten.",
    nextCard: "Nächste Karte",
    previousCard: "Vorherige Karte",
    card: "Karte",
    timeRemainingLabel: "Verbleibende Zeit:",
    youHeld: "Du hast gehalten",
    ttsRateSaved: "Im Stapelprofil gespeichert, damit die nächste Sitzung hier beginnt.",
    examDrawDesc: "Prüfungen nutzen Multiple-Choice- und Eingabekarten aus Stapeln, deren Profil Prüfungsfragen aktiviert hat.",
    deckDetail: "Stapeldetails",
    selectDeckPrompt: "Wähle einen Stapel, um Zahlen und Lernoptionen zu sehen.",
    dueNext14: "Fällige Karten der nächsten 14 Tage",
    next14Days: "Nächste 14 Tage",
    youHold: "Du hältst",
    hideAnswer: "Antwort verbergen",
    deleteCardConfirm: "Diese Karte aus der Kartendatenbank löschen?",
    fileOrder: "Dateireihenfolge",
    clozeLabel: "Lückentext",
    maskLabel: "Maske",
  },

  manager: {
    title: "Karteikarten-Manager",
    searchPlaceholder: "Karten suchen…",
    filter: "Filter",
    sort: "Sortieren",
    edit: "Bearbeiten",
    delete: "Löschen",
    selectAll: "Alle auswählen",
    deselectAll: "Alle abwählen",
    bulkActions: "Massenaktionen",
    exportSelection: "Auswahl exportieren",
    suspendSelected: "Ausgewählte aussetzen",
    unsuspendSelected: "Aussetzung aufheben",
    burySelected: "Ausgewählte vergraben",
    unburySelected: "Ausgewählte freigeben",
    resetSelected: "Ausgewählte zurücksetzen…",
    suspendedBadge: "Ausgesetzt",
    buriedBadge: "Vergraben",
    suspendedTooltip: "Ausgesetzt (nur manuelle Wiederholung)",
    buriedUntilTooltip: "Vergraben bis {date}",
    resetSelectedConfirmTitle: "Ausgewählte Karten zurücksetzen?",
    resetSelectedConfirmMessage:
      "Fortschritt für {count} Karten zurücksetzen? Der gesamte Lernverlauf dieser Karten wird gelöscht. Dies kann nicht rückgängig gemacht werden.",
    cardActions: "Kartenaktionen",
    deleteSelected: "Ausgewählte löschen",
    cardsSelected: "{count} Karten ausgewählt",
    noResults: "Keine Karten entsprechen dem aktuellen Filter",
    columnFront: "Vorderseite",
    columnBack: "Rückseite",
    columnDeck: "Stapel",
    columnDue: "Fällig",
    columnState: "Status",
    columnStability: "Stabilität",
    columnDifficulty: "Schwierigkeit",
    columnLapses: "Vergessen",
    columnReviewed: "Wiederholt",
    stateNew: "Neu",
    stateLearning: "Lernen",
    stateReview: "Wiederholen",
    stateRelearning: "Nachlernen",
    rowsPerPage: "Zeilen pro Seite",
    of: "von",
    page: "Seite",
    first: "Erste",
    previous: "Vorherige",
    next: "Nächste",
    last: "Letzte",
    openInVault: "Im Vault öffnen",
    deckTagDescription: "Stapel",
    createCustomDeck: "Benutzerdefinierten Stapel erstellen",
    customDeckName: "Name des Stapels",
    customDeckNamePlaceholder: "Name eingeben",
    saveCustomDeck: "Stapel speichern",
    addFilter: "Filter hinzufügen",
    removeFilter: "Filter entfernen",
    clearFilters: "Filter zurücksetzen",
    loadingFlashcards: "Karteikarten werden geladen...",
    editLabel: "Bearbeiten:",
    browseAllOption: "— Alle Karteikarten durchsuchen —",
    saveTooltip: "Änderungen an {name} speichern",
    discardTooltip: "Bearbeitung verwerfen und zurück zur Übersicht",
    saveButton: "Speichern",
    cancelButton: "Abbrechen",
    searchPlaceholderFull: "Dateinamen, Pfad, Vorderseite, Rückseite suchen...",
    clearSearch: "Suche löschen",
    filterButton: "+ Filter",
    filterButtonCount: "+ Filter ({count})",
    removeFilterAria: "Filter entfernen",
    searchOnlyView: "Nur-Such-Ansicht (Speichern ist deaktiviert)",
    filterDeckNamePlaceholder: "Name des Filter-Stapels",
    saveAsFilterDeck: "💾 Als Filter-Stapel speichern",
    saveAsTitleHasFilter: "Aktuellen Filter als Filter-Stapel speichern",
    saveAsTitleNoFilter: "Filter-Regel hinzufügen, um zu speichern",
    selectedCount: "{count} ausgewählt",
    addToCustomDeck: "Zu Stapel hinzufügen",
    createNewDeckOption: "+ Neuen Stapel erstellen...",
    newDeckPlaceholder: "Stapelname",
    createButton: "Erstellen",
    colFront: "Vorderseite",
    colBack: "Rückseite",
    colHint: "Hinweis",
    colNotes: "Notizen",
    colFile: "Datei",
    colBreadcrumb: "Pfad",
    colDeckTag: "Stapel-Tag",
    colCardTags: "Karten-Tags",
    colState: "Status",
    colHealth: "Gesundheit",
    colDue: "Fällig",
    colReviewed: "Wiederholt",
    badgeNew: "Neu",
    badgeReview: "Wiederholen",
    badgeLeech: "Egel",
    badgeDense: "Dicht",
    badgeHealthy: "Gesund",
    leechTooltip:
      "Wiederholt vergessen ({count} mal) — neu schreiben in Erwägung ziehen",
    denseTooltip:
      "Rückseite hat {count} Zeichen — Aufteilen in Erwägung ziehen",
    healthyTooltip: "Keine Egel- oder Dichte-Probleme erkannt",
    footerShowingOfInDeck: '{shown} von {total} Karten in „{name}" angezeigt',
    footerInDeck: '{total} Karten in „{name}"',
    footerShowingOf: "{shown} von {total} Karteikarten angezeigt",
    footerTotal: "{total} Karteikarten",
    loadMore: "Mehr laden ({remaining} verbleibend)",
    addToDeck: "Zum Stapel hinzufügen",
    createAndAdd: "Erstellen und hinzufügen",
    createDeck: "Stapel erstellen",
    deckScope: "Stapelbereich",
    sortOrder: "Sortierung",
    sortDueSoonest: "Bald fällig",
    sortFrontAZ: "Vorderseite A–Z",
    sortRecent: "Zuletzt wiederholt",
    filters: "Filter",
    selectNone: "Auswahl aufheben",
    source: "Quelle",
    loadingCards: "Karten werden geladen…",
    loadingDecks: "Stapel werden geladen…",
    noCardsMatch: "Keine Karte passt zu deinen Filtern.",
    noCardsYet: "Noch keine Karten.",
    tagToCreate: "Markiere deine Notizen, damit daraus Stapel und Karten werden.",
    noCustomDecks: "Noch keine eigenen Stapel — erstelle unten einen.",
    filterDeckHint: "Beginnt mit den Filtern, die Durchsuchen gerade zeigt. Ein Filterstapel hält sich selbst aktuell.",
    noRulesMatchAll: "Ohne Regeln passt dieser Stapel auf jede Karte.",
    typeEditedElsewhereList: "Dieser Kartentyp wird dort bearbeitet, wo er erstellt wurde, nicht in der Kartenliste.",
    reset: "Zurücksetzen",
    deckType: "Stapeltyp",
    manual: "Manuell",
    manualDesc: "Die Karten selbst auswählen",
    rulesDesc: "Karten nach Regeln, automatisch aktuell",
    rules: "Regeln",
    matchingCards: "Passende Karten",
    selectAllShown: "Alle angezeigten auswählen",
    selectedOnly: "Nur ausgewählte",
    noCardsSearch: "Keine Karte passt zu deiner Suche.",
    noCardsSyncFirst: "Noch keine Karten — gleiche zuerst einen Stapel ab.",
    namePlaceholder: "z. B. Prüfungsvorbereitung",
    cardDetail: "Kartendetails",
    backToCards: "Zurück zu den Karten",
    selectCardHere: "Wähle eine Karte, um sie hier zu sehen.",
  },

  config: {
    title: "Stapel-Konfiguration",
    profile: "FSRS-Profil",
    profileLabel: "Profil",
    profileSelectDesc: "Wähle ein Profil für dieses Tag",
    profileNameLabel: "Profilname",
    profileStandard: "Standard",
    profileIntensive: "Intensiv",
    newCardsLimitLabel: "Limit für neue Karten",
    reviewCardsLimitLabel: "Limit für Wiederholungskarten",
    perDay: "{count} pro Tag",
    unlimited: "Unbegrenzt",
    headerLevelLabel: "Überschriftenebene",
    headerTitle: "Titel",
    headerH: "H{level}",
    reviewOrderLabel: "Wiederholungsreihenfolge",
    reviewOrderOldestDue: "Älteste zuerst",
    reviewOrderRandomLabel: "Zufällig",
    clozeDeletions: "Lückentexte",
    clozeEnabled: "Aktiviert (Kontext: {mode})",
    clozeShowOthers: "andere Lücken anzeigen",
    clozeHideAll: "alle Lücken verbergen",
    clozeDisabled: "Deaktiviert",
    fsrsSettings: "FSRS-Einstellungen",
    fsrsSettingsDesc: "Retention: {retention}, Profil: {profile}",
    decksUsingProfile: "Stapel, die das Profil verwenden",
    deckCount: "{count} Stapel",
    editNoteIntro: "Hinweis:",
    editNoteBody:
      ' Um Profileinstellungen zu bearbeiten, nutze die Schaltfläche „Profile verwalten" im Hauptpanel.',
    sectionTagSelection: "Tag-Auswahl",
    sectionProfileSelection: "Profil-Auswahl",
    sectionCurrentSettings: "Aktuelle Profil-Einstellungen",
    saving: "Speichere …",
    applyProfileToTag: "Profil auf Tag anwenden",
    applyProfileDesc:
      "Wähle ein Tag, um das Profil auf alle seine Stapel anzuwenden",
    errorSavingConfig: "Fehler beim Speichern der Konfiguration",
    requestRetention: "Gewünschte Retention",
    requestRetentionDesc: "Zielwahrscheinlichkeit für Erinnerung (0,7-0,99)",
    maxIntervalDays: "Maximales Intervall (Tage)",
    maxIntervalDaysDesc: "Obergrenze des geplanten Intervalls",
    learningSteps: "Lernschritte",
    relearningSteps: "Nachlernschritte",
    newCardsPerDay: "Neue Karten pro Tag",
    newCardsPerDayDesc:
      "Maximale Anzahl neuer Karten pro Tag (0 deaktiviert das Limit)",
    reviewsPerDay: "Wiederholungen pro Tag",
    reviewsPerDayDesc:
      "Maximale Anzahl Wiederholungskarten pro Tag (0 deaktiviert das Limit)",
    reviewOrder: "Wiederholungsreihenfolge",
    reviewOrderDueDate: "Fälligkeitsdatum",
    reviewOrderRandom: "Zufällig",
    headerLevel: "Überschriftenebene für Karten",
    headerLevelDesc:
      "Überschriftenebene, die als Vorderseite in Überschrift-Absatz-Stapeln behandelt wird",
    useTrainedWeights: "Trainierte Gewichte verwenden",
    useTrainedWeightsDesc:
      "Wendet die global trainierten FSRS-Gewichte auf diesen Stapel an (nur Standard-Profil).",
    save: "Speichern",
    cancel: "Abbrechen",
    reset: "Auf Standard zurücksetzen",
    enableDailyLimits: "Tageslimits aktivieren",
    enableDailyLimitsDesc:
      "Anzahl neuer und Wiederholungskarten pro Tag begrenzen",
  },

  profiles: {
    refactorPromptLabel: "KI-System-Prompt",
    refactorPromptDesc: "Anweisungen, die die KI beim Überarbeiten von Karten in Stapeln mit diesem Profil befolgt. Leer lassen für den Standard.",
    refactorPromptPlaceholder: "z. B. Antworten kürzen und Grammatik korrigieren.",
    sectionAiRefactor: "KI-System-Prompt",
    title: "FSRS-Profile",
    addProfile: "Profil hinzufügen",
    editProfile: "Profil bearbeiten",
    deleteProfile: "Profil löschen",
    duplicateProfile: "Profil duplizieren",
    profileName: "Profilname",
    profileNamePlaceholder: "Profilname eingeben",
    save: "Profil speichern",
    cancel: "Abbrechen",
    tagMappings: "Tag-Zuordnungen",
    addTagMapping: "Tag-Zuordnung hinzufügen",
    removeTagMapping: "Tag-Zuordnung entfernen",
    tagPlaceholder: "Tag eingeben (z. B. #decks/math)",
    noProfiles: "Noch keine benutzerdefinierten Profile",
    deleteConfirm:
      "Dieses Profil löschen? Stapel, die es verwenden, fallen auf das Standardprofil zurück.",
    cannotDeleteDefault: "Die Standardprofile können nicht gelöscht werden.",
    selectProfile: "Profil auswählen",
    chooseProfileDesc: "Wähle ein Profil zum Anzeigen und Bearbeiten",
    createNewProfile: "Neues Profil erstellen",
    createTooltip: "Ein neues Profil erstellen",
    profileSettings: "Profileinstellungen",
    profileNameDesc: "Name für dieses Profil",
    sectionDailyLimits: "Tageslimits",
    sectionNewCards: "Neue Karten",
    sectionLapses: "Vergessen",
    sectionCardParsing: "Karten-Parsen",
    sectionReviewSettings: "Wiederholungs-Einstellungen",
    sectionFsrsAlgorithm: "FSRS-Algorithmus",
    sectionProfileInfo: "Profil-Info",
    sectionReadAloud: "Vorlesen",
    ttsVoiceLabel: "Stimme",
    ttsVoiceDesc: "Stimme für die Vorlesen-Schaltfläche während der Wiederholung",
    ttsVoiceDefault: "Systemstandard",
    ttsRateLabel: "Geschwindigkeit",
    ttsRateDesc: "Wiedergabegeschwindigkeit beim Vorlesen",
    ttsUnavailable: "Vorlesen ist auf diesem Gerät nicht verfügbar",
    limitNewCardsLabel: "Neue Karten pro Tag begrenzen",
    limitNewCardsDesc: "Tageslimit für neue Karten aktivieren",
    newCardsPerDayLabel: "Neue Karten pro Tag",
    newCardsPerDayDesc: "Maximale neue Karten pro Tag (0-9999)",
    limitReviewCardsLabel: "Wiederholungskarten pro Tag begrenzen",
    limitReviewCardsDesc: "Tageslimit für Wiederholungskarten aktivieren",
    reviewCardsPerDayLabel: "Wiederholungskarten pro Tag",
    reviewCardsPerDayDesc: "Maximale Wiederholungskarten pro Tag (1-9999)",
    againIntervalNew: "Nochmal-Intervall",
    againIntervalNewDesc:
      "Intervall bei Nochmal auf einer neuen Karte (z. B. 1m)",
    againIntervalReview: "Nochmal-Intervall",
    againIntervalReviewDesc:
      "Intervall bei Nochmal auf einer Wiederholungskarte (z. B. 10m)",
    headerLevelLabel: "Überschriftenebene",
    headerLevelDescParsing: "Überschriftenebene für das Parsen",
    extraHeaderLevelsLabel: "Zusätzliche Überschriftenebenen",
    extraHeaderLevelsDesc: "Diese Überschriftenebenen ebenfalls als Karten parsen (neue und importierte Karten verwenden weiterhin die primäre Ebene oben)",
    clozeDeletionsLabel: "Lückentexte",
    clozeDeletionsDesc: "Lückentexte aus ==hervorgehobenem== Text generieren",
    clozeContextLabel: "Lücken-Kontext",
    clozeContextDesc:
      "Wie nicht abgefragte Lücken bei der Wiederholung erscheinen",
    clozeShowOption: "Andere Lücken anzeigen",
    clozeHideOption: "Alle Lücken verbergen",
    reviewOrderLabel: "Wiederholungsreihenfolge",
    reviewOrderDesc: "Reihenfolge, in der Karten wiederholt werden",
    requestRetentionLabel: "Gewünschte Retention",
    requestRetentionDesc: "Ziel-Retentionsrate (0,5 - 0,995)",
    fsrsProfileLabel: "FSRS-Profil",
    fsrsTrainedDesc:
      "Standard oder Intensiv nutzen mitgelieferte Gewichte. Trainiert wendet deine optimierten Gewichte an (Standard-Intervalle).",
    fsrsUntrainedDesc:
      "Lernintensitäts-Profil. Trainiere Gewichte in der Algorithmus-Feinabstimmung, um die Option Trainiert zu aktivieren.",
    fsrsStandardOption: "Standard",
    fsrsIntensiveOption: "Intensiv",
    fsrsTrainedOption: "Trainiert",
    fsrsTrainedUnavailable: "Trainiert (nicht verfügbar)",
    decksUsingProfile: "Stapel, die das Profil verwenden",
    deckCount: "{count} Stapel",
    tagAssignmentsHeading: "Tag-Zuweisungen",
    saveChanges: "Änderungen speichern",
    savingChanges: "Speichere …",
    deleteProfileButton: "Profil löschen",
    close: "Schließen",
    noticeProfileCreated: "Neues Profil erstellt",
    noticeProfileNameEmpty: "Profilname darf nicht leer sein",
    noticeNewCardsRange:
      "Neue Karten pro Tag müssen zwischen 0 und 9999 liegen",
    noticeReviewCardsRange:
      "Wiederholungskarten pro Tag müssen zwischen 1 und 9999 liegen",
    noticeRequestRetentionRange:
      "Gewünschte Retention muss zwischen 0,5 und 0,995 liegen",
    noticeInvalidAgainInterval: "Ungültiges Nochmal-Intervall",
    noticeReadAloudRange: 'Vorlesegeschwindigkeit muss zwischen {min} und {max} liegen',
    noticeInvalidLapseInterval: 'Ungültiges Intervall für Rückfälle',
    noticeQuestionCountRange: 'Anzahl der Fragen muss 0 oder mehr sein',
    noticeTimeLimitRange: 'Zeitlimit muss 0 oder mehr sein',
    noticePassScoreRange: 'Mindestpunktzahl zum Bestehen muss zwischen 0 und 100 liegen',
    noticeProfileSaved: "Profil gespeichert",
    noticeProfileSaveError: "Fehler beim Speichern des Profils",
    noticeCannotDeleteDefault: "Das STANDARD-Profil kann nicht gelöscht werden",
    noticeProfileDeleted: "Profil gelöscht",
    confirmDeletePrompt:
      'Bist du sicher, dass du das Profil „{name}" löschen möchtest?\n\nAlle Stapel, die dieses Profil verwenden, werden auf das STANDARD-Profil zurückgesetzt.',
    modalTitle: "Profile",
    modalSubtitle: "Konfiguriere ein Profil und wähle, welche Stapel es verwenden – alles an einem Ort.",
    tabSettings: "Einstellungen",
    tabAssignments: "Zuweisungen",
    activeProfileLabel: "Aktives Profil",
    newProfileButton: "Neu",
    newProfileName: "Neues Profil",
    assignmentsHeading: "Stapel & Tags, die {name} verwenden",
    assignmentsExplainer: "Wende dieses Profil auf ein Tag an, und jeder Stapel darunter übernimmt diese Einstellungen. Ein Stapel kann nur ein Profil verwenden.",
    assignTagPlaceholder: "Tag zum Anwenden wählen…",
    applyButton: "Anwenden",
    assignmentsEmpty: "Noch kein Stapel verwendet dieses Profil. Wende es oben auf ein Tag an.",
    assignmentMeta: "{decks} Stapel · {cards} Karten",
    effectiveSettingsHeading: "Effektive Einstellungen",
    removeAssignmentConfirm: "Dieses Profil von „{tag}“ entfernen? Seine Stapel fallen auf das übergeordnete Profil zurück.",
    noticeProfileDuplicated: "Profil dupliziert",
    copySuffix: " Kopie",
    defaultSuffix: "(STANDARD)",
    nameDesc: "Name für dieses Profil.",
    headerLevelDesc: "Überschriftenebene, die als Kartenvorderseite gelesen wird.",
    clozeDesc: "Lückentextkarten aus ==hervorgehobenem== Text erzeugen.",
    examDesc: "Eine Aufgabenliste unter einer Überschrift wird zur Multiple-Choice-Karte.",
    newLimitDesc: "Tageslimit für neue Karten aktivieren.",
    reviewLimitDesc: "Tageslimit für Wiederholungen aktivieren.",
    newLimitMaxDesc: "Höchstzahl neuer Karten pro Tag (0–9999).",
    reviewLimitMaxDesc: "Höchstzahl an Wiederholungen pro Tag (1–9999).",
    retentionDesc: "Ziel-Behaltensrate (0,5–0,995).",
    newAgainInterval: "Nochmal-Intervall für neue Karten",
    newAgainIntervalDesc: "Intervall, wenn bei einer neuen Karte Nochmal gedrückt wird (z. B. 1m).",
    lapseAgainInterval: "Nochmal-Intervall bei Rückfall",
    lapseAgainIntervalDesc: "Intervall, wenn bei einer Wiederholung Nochmal gedrückt wird (z. B. 10m).",
    intervals: "Intervalle",
    clozeMaskDesc: "Wie nicht abgefragte Lücken während der Wiederholung erscheinen.",
    ttsLanguageDesc: "Sprache, in der die Systemstimme liest. Automatisch folgt dem Gerät.",
    ttsPreview: "Aktuelle Stimme und Geschwindigkeit anhören.",
    ttsVoiceMissing: "Die gespeicherte Stimme ist auf diesem Gerät nicht installiert, daher wird die Sprache unten verwendet.",
    automatic: "Automatisch",
    random: "Zufällig",
    sections: "Profilabschnitte",
    loading: "Profile werden geladen…",
    noneYet: "Noch keine Profile.",
    availableWhenDb: "Profile erscheinen, sobald die Datenbank verfügbar ist.",
    tagToApply: "Anzuwendender Tag",
    tagDesc: "Wende dieses Profil auf einen Tag an, und jeder Stapel darunter erbt diese Einstellungen. Ein Stapel kann nur ein Profil nutzen.",
    duplicate: "Duplizieren",
    examsHeading: "Prüfungen",
  },

  statistics: {
    title: "Statistik",
    overall: "Gesamt",
    deck: "Stapel",
    cardCounts: "Kartenanzahl",
    cardCountsNew: "Neu",
    cardCountsLearning: "Lernen",
    cardCountsReview: "Wiederholen",
    cardCountsRelearning: "Nachlernen",
    cardCountsTotal: "Gesamt",
    cardCountsYoung: "Jung",
    cardCountsMature: "Reif",
    cardsAddedSeries: "Hinzugefügte Karten",
    againSeries: "Nochmal",
    hardSeries: "Schwer",
    goodSeries: "Gut",
    easySeries: "Leicht",
    reviewsOnDate: "{count} Wiederholungen am {date}",
    totalReviewsCount: "{count} Wiederholungen",
    cardDifficulty: "Karten-Schwierigkeit",
    cardStability: "Karten-Stabilität",
    cardRetrievability: "Karten-Abrufbarkeit",
    cardsAdded: "Hinzugefügte Karten",
    futureDue: "Zukünftig fällige Karten",
    hourlyBreakdown: "Stündliche Aufschlüsselung",
    maturityProgression: "Reife-Entwicklung",
    reviewHeatmap: "Wiederholungs-Heatmap",
    reviewIntervals: "Wiederholungsintervalle",
    reviewsOverTime: "Wiederholungen im Zeitverlauf",
    trueRetention: "Tatsächliche Retention",
    legendDays: "Tage",
    legendCount: "Anzahl",
    legendCards: "Karten",
    legendReviews: "Wiederholungen",
    legendAccuracy: "Genauigkeit",
    daysAgo: "vor {count} Tagen",
    today: "Heute",
    yesterday: "Gestern",
    tomorrow: "Morgen",
    inDays: "In {count} Tagen",
    rangeWeek: "Woche",
    rangeMonth: "Monat",
    rangeYear: "Jahr",
    rangeAll: "Gesamtzeit",
    matureCards: "Reif ({count}+ Tage)",
    youngCards: "Jung (<{count} Tage)",
    againRate: "Nochmal-Rate",
    hardRate: "Schwer-Rate",
    goodRate: "Gut-Rate",
    easyRate: "Leicht-Rate",
    retention: "Retention",
    retentionYoung: "Retention (jung)",
    retentionMature: "Retention (reif)",
    noData: "Keine Daten verfügbar",
    elapsedDays: "Verstrichene Tage",
    intervalDays: "Intervall (Tage)",
    countLabel: "Anzahl",
    averageDifficulty: "Durchschnittliche Schwierigkeit",
    averageStability: "Durchschnittliche Stabilität",
    averageRetrievability: "Durchschnittliche Abrufbarkeit",
    totalReviews: "Wiederholungen gesamt",
    totalCards: "Karten gesamt",
    studyTime: "Lernzeit",
    streak: "Serie",
    currentStreak: "Aktuelle Serie",
    longestStreak: "Längste Serie",
    daysStudied: "Lerntage",
    minutesPerDay: "Min/Tag",

    // Chart strings — DRAFT translations
    cardDistribution: "Kartenverteilung",
    cardDistributionByState: "Kartenverteilung nach Zustand",
    cardDistributionByStateDoughnut: "Kartenverteilung nach Zustand (Ring)",
    cardCountsTooltip: "{label}: {count} Karten ({percent}%)",
    selectDeckCardDistribution:
      "Wähle einen Stapel, um die Kartenverteilung anzuzeigen.",
    cardDifficultyDistribution: "Verteilung der Kartenschwierigkeit",
    cardDifficultyRange: "Schwierigkeitsbereich",
    cardDifficultySubtitle:
      "FSRS-Schwierigkeitswerte geben an, wie schwer Karten zu merken sind",
    higherDifficultyTip: "Höhere Schwierigkeit = schwerer zu merken",
    numberOfCards: "Anzahl Karten",
    numberOfReviews: "Anzahl Wiederholungen",
    selectDeckCardDifficulty:
      "Wähle einen Stapel, um die Kartenschwierigkeit anzuzeigen.",
    cardStabilityDistribution: "Verteilung der Kartenstabilität",
    cardStabilityRange: "Stabilitätsbereich",
    cardStabilitySubtitle:
      "FSRS-Stabilitätswerte zeigen, wie gut Karten im Gedächtnis verankert sind",
    selectDeckCardStability:
      "Wähle einen Stapel, um die Kartenstabilität anzuzeigen.",
    cardRetrievabilityDistribution: "Verteilung der Karten-Abrufbarkeit",
    retrievabilityRange: "Abrufbarkeitsbereich",
    reviewsLabel: "Wiederholungen",
    cardRetrievabilitySubtitle:
      "FSRS-Abrufbarkeitswerte zeigen die Wahrscheinlichkeit, sich heute zu erinnern (0-100%)",
    higherRetrievabilityTip: "Höhere Abrufbarkeit = leichter abrufbar",
    reviewsTooltip: "{label}: {count} Wiederholungen ({percent}%)",
    selectDeckCardRetrievability:
      "Wähle einen Stapel, um die Karten-Abrufbarkeit anzuzeigen.",
    cardsAddedOverTime: "Hinzugefügte Karten im Zeitverlauf",
    dateAdded: "Hinzufügedatum",
    selectDeckCardsAdded:
      "Wähle einen Stapel, um hinzugefügte Karten im Zeitverlauf anzuzeigen.",
    cardsAddedTooltip: "{label}: {count} {plural}",
    cardSingular: "Karte",
    cardPlural: "Karten",
    basedOnFirstReview: "Basiert auf dem ersten Wiederholungsdatum",
    timeframeLabel: "Zeitraum:",
    timeframe1Month: "1 Monat",
    timeframe3Months: "3 Monate",
    timeframe1Year: "1 Jahr",
    allTime: "Gesamtzeit",
    futureDueTitle: "Zukünftig fällig",
    selectDeckFutureDue:
      "Wähle einen Stapel, um zukünftig fällige Wiederholungen anzuzeigen.",
    dueCards: "Fällige Karten",
    cumulativeBacklog: "Kumulativer Rückstand",
    totalReviewsLabel: "Wiederholungen gesamt:",
    avgPerDay: "Ø/Tag:",
    dueTomorrow: "Fällig morgen:",
    futureDueSubtitle: "Anzahl zukünftig fälliger Wiederholungen.",
    backlogControl: "Rückstand",
    onemonth: "1 Monat",
    threeMonths: "3 Monate",
    oneYear: "1 Jahr",
    all: "alle",
    cumulativeSeries: "Kumulativ",
    dayLabel: "Tag {label}",
    totalBacklogTooltip: "Gesamter Rückstand: {value} Wiederholungen",
    dueTooltip: "Fällig: {value} Karten",
    noUpcomingReviews:
      "Keine bevorstehenden Wiederholungen geplant. Füge deinen Stapeln Karten hinzu, um eine Prognose zu sehen.",
    hourlyBreakdownTitle: "Wiederholungsaktivität nach Stunde",
    hourlyBreakdownChart: "Wiederholungsaktivität nach Tagesstunde",
    reviewCountLabel: "Wiederholungsanzahl",
    hourOfDay: "Tagesstunde",
    successRatePercent: "Erfolgsrate (%)",
    selectDeckHourly:
      "Wähle einen Stapel, um die stündliche Aktivität anzuzeigen.",
    hourlySuccessRateTooltip: "{label}: {value}%",
    hourlyReviewsTooltip: "{label}: {value} Wiederholungen",
    timePeriod: "Tageszeit: {period}",
    morning: "Morgen",
    afternoon: "Nachmittag",
    evening: "Abend",
    night: "Nacht",
    maturityForecast: "Prognose der Reife-Entwicklung",
    cardMaturityProgression: "Karten-Reife-Entwicklung",
    daysFromNow: "Tage ab jetzt",
    newCardsLabel: "Neue Karten",
    matureCardsLabel: "Reife Karten",
    learningCardsLabel: "Karten im Lernen",
    selectDeckMaturity: "Wähle einen Stapel, um die Reife-Prognose anzuzeigen.",
    calculating: "Berechne …",
    maturityRetentionNote:
      "Basierend auf deiner historischen Retentionsrate von {percent}% reifen deine Karten so:",
    maturityAllMatureIn: "Alle Karten sind in etwa {days} reif.",
    maturitySubtitle:
      "Prognose, wie Karten basierend auf dem aktuellen Wiederholungstempo zur Reife kommen.",
    maturityRetentionDetail:
      "Basierend auf deiner historischen Retentionsrate von {retention}% werden ungefähr {maintenance}% der Karten (~{count} Karten) durch natürliche Fehler immer in der Lernphase sein.",
    maturityEquilibrium: "Gleichgewicht in {days} erreicht.",
    maturityAllMatureAtPace:
      "Alle Karten sind in etwa {days} reif beim aktuellen Tempo.",
    maturityTotalCards: "Gesamt: {count} Karten",
    standardMaintenanceLine: "Standard-Erhaltungslinie ({percent}%)",
    yourMaintenanceLine: "Deine Erhaltungslinie ({percent}%)",
    overallTitle: "Gesamtstatistik",
    currentStatus: "Aktueller Stand",
    learningLabel: "Lernen",
    matureLabel: "Reif",
    dueToday: "Heute fällig",
    reviewPace: "Wiederholungstempo",
    averagePerCard: "Durchschnitt je Karte",
    totalTime: "Gesamtzeit",
    activitySummary: "Aktivitätsübersicht",
    thisWeek: "Diese Woche",
    thisMonth: "Dieser Monat",
    thisYear: "Dieses Jahr",
    answerButtons: "Antwortschaltflächen",
    noAnswerButtonData: "Noch keine Daten zu Antwortschaltflächen verfügbar.",
    reviewActivityHeading: "Wiederholungsaktivität",
    legendLess: "Weniger",
    legendMore: "Mehr",
    reviewIntervalDistribution: "Verteilung der Wiederholungsintervalle",
    intervalRange: "Intervallbereich",
    reviewIntervalsTitle: "Wiederholungsintervalle",
    selectDeckIntervals:
      "Wähle einen Stapel, um die Intervallverteilung anzuzeigen.",
    reviewIntervalsSubtitle:
      "Verteilung der aktuellen Wiederholungsintervalle der Karten",
    reviewsOverTimeTitle: "Wiederholungen im Zeitverlauf",
    selectDeckReviewsOverTime:
      "Wähle einen Stapel, um den Wiederholungsverlauf anzuzeigen.",
    dateAxisLabel: "Datum",
    selectDeckLabel: "Stapel auswählen:",
    deckDropdownPrompt: "-- Stapel wählen --",
    allDecks: "Alle Stapel",
    tagPrefix: "Tag: {tag}",
    statisticsTitle: "Gesamtstatistik",
    loadingStatistics: "Statistiken werden geladen …",
    failedToLoad: "Statistiken konnten nicht geladen werden",
    failedToLoadIntro:
      "Beim Laden deiner Statistiken ist ein Fehler aufgetreten. Mögliche Ursachen:",
    failedReasonDb: "Probleme mit der Datenbankverbindung",
    failedReasonData: "Beschädigte Daten",
    failedReasonSize: "Großer Datensatz, dessen Verarbeitung zu lange dauert",
    failedConsoleHint:
      "Prüfe die Browser-Konsole (F12) für detaillierte Fehlerinformationen.",
    retryLoading: "Erneut laden",
    timeframeLast12Months: "Letzte 12 Monate",
    timeframeAllHistory: "Gesamte Historie",
    labelReviews: "Wiederholungen",
    labelTime: "Zeit",
    labelCorrect: "richtig",
    completeReviewsHint:
      "Wiederhole einige Karten, um Statistiken zu den Antwortschaltflächen zu sehen.",
    selectDeckPrompt: "Wähle oben einen Stapel aus, um Statistiken anzuzeigen.",
    reviewHeatmapTitle: "Wiederholungs-Heatmap",
    dailyReviewActivity: "Tägliche Wiederholungsaktivität im Zeitverlauf",
    trueRetentionTitle: "Tatsächliche Retention",
    selectDeckRetention:
      "Wähle einen Stapel, um Retentions-Statistiken anzuzeigen.",
    retentionDescription:
      "Erfolgsquoten für Wiederholungskarten (Bewertung ≥ Gut). Nur Karten mit Intervallen > 1 Tag.",
    noReviewDataYet:
      "Noch keine Wiederholungsdaten vorhanden. Wiederhole einige Karten, um Retentions-Statistiken zu sehen.",
    columnCardType: "Kartentyp",
    columnPassed: "Bestanden",
    columnTotalLabel: "Gesamt",
    columnPassRate: "Erfolgsquote",
    youngWithDays: "Jung (< {count} Tage)",
    matureWithDays: "Reif (≥ {count} Tage)",
    allCards: "Alle Karten",
    loading: "Laden …",
    scope: "Statistikbereich",
    timeframe: "Zeitraum",
    updating: "Wird aktualisiert…",
    waitingForDb: "Warte auf die Kartendatenbank…",
    tryAgain: "Erneut versuchen",
    noneYet: "Noch keine Statistiken.",
    noneYetHint: "Wiederhole ein paar Karten, dann erscheint dein Fortschritt hier.",
    noReviewsInPeriod: "In diesem Zeitraum wurden keine Wiederholungen erfasst.",
    notEnoughHistory: "Noch zu wenig Lernverlauf.",
    cards: "Karten",
    cardStateDistribution: "Verteilung der Kartenzustände",
    dueNext30: "Fällige Karten der nächsten 30 Tage",
    reviewsAllTime: "Wiederholungen gesamt",
    studyTimeAllTime: "Lernzeit gesamt",
    timeInPeriod: "Zeit im Zeitraum",
    passRateDesc: "Erfolgsquote bei Karten mit einem Intervall von mindestens einem Tag.",
  },

  ankiExport: {
    title: "Nach Anki exportieren",
    titleWithName: "Nach Anki exportieren: {name}",
    description: "Ausgewählte Karten als Anki-kompatible CSV exportieren.",
    selectAll: "Alle auswählen",
    deselectAll: "Alle abwählen",
    selectedCount: "{count} ausgewählt",
    deckName: "Stapelname",
    deckNamePlaceholder: "Mein Stapel",
    ankiDeckNameLabel: "Anki-Stapelname",
    ankiDeckNameDesc:
      "Name des Stapels in Anki, in den die Karten importiert werden",
    ankiDeckNamePlaceholder: "Stapelname eingeben...",
    includeTags: "Tags einschließen",
    includeTagsDesc: "Stapel- und Karten-Tags in den Export aufnehmen",
    includeScheduling: "Planung einschließen",
    includeSchedulingDesc: "FSRS-Status im Export beibehalten",
    export: "Exportieren",
    exportButtonLabel: "Nach Anki exportieren",
    cancel: "Abbrechen",
    exporting: "Exportiere…",
    exportSuccess: "{count} Karten nach {filename} exportiert",
    exportFailed: "Export fehlgeschlagen: {message}",
    noSelection: "Mindestens eine Karte zum Exportieren auswählen.",
    sectionInfo: "Export-Informationen",
    sectionHowToImport: "Importieren",
    sectionSettings: "Export-Einstellungen",
    infoSourceDeck: "Quellstapel:",
    infoSourceTag: "Quell-Tag:",
    infoSourceFile: "Quelldatei:",
    importStep1: "Es wird eine mit Anki kompatible Textdatei heruntergeladen",
    importStep2Prefix: "Öffne Anki und gehe zu",
    importStep2Item: "Datei → Importieren",
    importStep3: "Wähle die heruntergeladene Datei aus",
    importStep4: "Wähle deinen Zielstapel und die Importeinstellungen",
    importStep5Prefix: "Klicke",
    importStep5Item: "Importieren",
    importStep5Suffix: ", um die Karten hinzuzufügen",
  },

  filterBuilder: {
    title: "Filter",
    add: "Bedingung hinzufügen",
    addRule: "+ Regel hinzufügen",
    remove: "Bedingung entfernen",
    removeRule: "Regel entfernen",
    clear: "Alle löschen",
    apply: "Anwenden",
    save: "Filter speichern",
    matchLabel: "Erfülle",
    matchAll: "Alle Bedingungen",
    matchAny: "Irgendeine Bedingung",
    matchAllOption: "Alle",
    matchAnyOption: "Beliebige",
    matchSuffix: "der folgenden Regeln",
    fieldDeck: "Stapel",
    fieldCardState: "Kartenstatus",
    fieldDueDate: "Fälligkeitsdatum",
    fieldDeckTag: "Stapel-Tag",
    fieldCardTag: "Karten-Tag",
    fieldSourceFile: "Quelldatei",
    fieldCardType: "Kartentyp",
    fieldState: "Status",
    fieldTag: "Tag",
    fieldDue: "Fällig",
    fieldDifficulty: "Schwierigkeit",
    fieldStability: "Stabilität",
    fieldLapses: "Vergessen",
    fieldFront: "Vordertext",
    fieldBack: "Rücktext",
    fieldFlagged: "Markiert",
    fieldSuspended: "Ausgesetzt",
    fieldCreated: "Erstellt",
    fieldReviewed: "Zuletzt wiederholt",
    fieldIsLeech: "Ist Egel",
    fieldIsDense: "Ist dicht",
    fieldIsSuspended: "Ist ausgesetzt",
    fieldIsBuried: "Ist vergraben",
    operatorIs: "Ist",
    operatorIsNot: "Ist nicht",
    operatorIsNew: "Ist neu",
    operatorIsDue: "Ist fällig",
    operatorHasTag: "Hat Tag",
    operatorEquals: "gleich",
    operatorNotEquals: "Ungleich",
    operatorContains: "enthält",
    operatorNotContains: "enthält nicht",
    operatorBefore: "vor",
    operatorAfter: "nach",
    operatorGreaterThan: "Größer als",
    operatorLessThan: "Kleiner als",
    operatorBetween: "zwischen",
    operatorIsLeech: "ist Egel",
    operatorIsDense: "ist dicht",
    selectTagPlaceholder: "Tag auswählen...",
    stateNew: "Neu",
    stateReview: "Wiederholen",
    typeHeaderParagraph: "Überschrift-Absatz",
    typeTable: "Tabelle",
    typeCloze: "Lückentext",
    typeImageOcclusion: "Bildverdeckung",
    typeSpatial: "Räumlich",
    boolTrue: "Wahr",
    boolFalse: "Falsch",
    valuePlaceholder: "Wert...",
    secondValuePlaceholder: "Bis",
    cardsMatch: "{count} Karten passen",
    cardMatchSingular: "{count} Karte passt",
    fieldBreadcrumb: "Brotkrumen",
    fieldInterval: "Intervall (min)",
    fieldRepetitions: "Wiederholungen",
    operatorEqualsCap: "Gleich",
    operatorIsOneOf: "Ist eines von",
    operatorContainsCap: "Enthält",
    operatorNotContainsCap: "Enthält nicht",
    operatorHasAnyOf: "Hat eines von",
    operatorDoesNotHaveTag: "Hat Tag nicht",
    operatorBeforeCap: "Vor",
    operatorAfterCap: "Nach",
    allRules: "Alle Regeln",
    anyRule: "Beliebige Regel",
    noRulesYet: "Noch keine Regeln — ein Filter ohne Regeln passt auf jede Karte.",
    ruleField: "Regelfeld",
    ruleOperator: "Regeloperator",
    ruleValue: "Regelwert",
    ruleMatching: "Regelverknüpfung",
    selectDeck: "Stapel wählen",
    selectTag: "Tag wählen",
    selectType: "Typ wählen",
    value: "Wert",
    badgeType: "Typ",
    badgeInterval: "Intervall",
    badgeReps: "Wdh.",
    badgeNot: "Kein {label}",
    badgeNotText: "nicht „{value}“",
  },

  modals: {
    confirm: {
      cancel: "Abbrechen",
      confirm: "Bestätigen",
      ok: "OK",
    },
    renameCustomDeck: {
      title: "Benutzerdefinierten Stapel umbenennen",
      label: "Stapelname",
      placeholder: "Neuen Namen eingeben",
      rename: "Umbenennen",
      cancel: "Abbrechen",
    },
    deckReset: {
      title: "Stapel-Fortschritt zurücksetzen",
      intro: 'Dies setzt den gesamten Fortschritt für "{name}" zurück:',
      listCardsReset: "Alle Karten in den neuen Zustand zurückgesetzt",
      listHistoryDeleted: "Wiederholungsverlauf gelöscht",
      listSessionsDeleted: "Wiederholungssitzungen gelöscht",
      cannotUndo: "Diese Aktion kann nicht rückgängig gemacht werden.",
      cancel: "Abbrechen",
      resetButton: "Fortschritt zurücksetzen",
      resetting: "Stapel-Fortschritt wird zurückgesetzt…",
      resetSuccess: "Stapel-Fortschritt erfolgreich zurückgesetzt",
      resetFailed: "Zurücksetzen des Stapel-Fortschritts fehlgeschlagen",
    },
    purgeDatabase: {
      title: "Datenbank leeren",
      warningStrong:
        "Dies löscht endgültig alle Karteikartendaten, einschließlich:",
      listFlashcards: "Aller Karten und ihres Inhalts",
      listReviews: "Vollständigen Wiederholungsverlaufs und Fortschritts",
      listDecks: "Aller Stapelinformationen",
      listStatistics: "Statistischer Daten",
      cannotUndo: "Diese Aktion kann nicht rückgängig gemacht werden!",
      rebuildNote:
        "Die Datenbank wird aus deinen aktuellen Vault-Dateien neu aufgebaut, aber der gesamte Fortschritt geht verloren.",
      confirmPrompt:
        "Um diese Aktion zu bestätigen, gib den im Platzhalter gezeigten Text ein",
      confirmPlaceholder: "ALLE DATEN LÖSCHEN",
      purgeButton: "Datenbank leeren",
      cancel: "Abbrechen",
    },
    releaseNotes: {
      title: "Neuigkeiten",
      close: "Schließen",
      versionLabel: "Version {version}",
    },
    statistics: {
      title: "Statistik",
      close: "Schließen",
    },
    optimizeFsrs: {
      title: "Parameter optimieren",
      loadingHistory: "Wiederholungsverlauf wird geladen…",
      trainingStep: "Trainingsschritt {step} / {total}",
      notEnough:
        "Noch nicht genug Wiederholungsverlauf zum Trainieren. Wiederhole weiter und versuche es später erneut.",
      reviewsAvailable:
        "Verfügbare Wiederholungen auf STANDARD-Profil-Stapeln: {count}.",
      trainedSummary:
        "Trainiert mit {reviews} Wiederholungen über {cards} Karten in {seconds} s.",
      logLossBefore: "Log-Loss vorher",
      logLossAfter: "Log-Loss nachher",
      improvement: "Verbesserung",
      noImprovement:
        "Das Training hat den Log-Loss nicht wesentlich verbessert — die aktuellen Gewichte passen bereits gut. Du kannst sicher verwerfen.",
      discard: "Verwerfen",
      apply: "Anwenden",
      close: "Schließen",
      failedHeader: "Optimierung fehlgeschlagen:",
      applied: "Trainierte Parameter angewendet.",
    },
    flashcardManager: {
      title: "Karteikarten-Manager",
    },
    decksView: {
      title: "Stapel",
    },
    aiBatch: {
      title: "KI-Stapelüberarbeitung",
      intro: "{count} ausgewählte Karten werden überprüft. Akzeptiere oder lehne jedes vorgeschlagene Feld ab.",
      start: "Start",
      cancel: "Abbrechen",
      close: "Schließen",
      retry: "Erneut versuchen",
      accept: "Akzeptieren",
      decline: "Ablehnen",
      acceptAll: "Alle akzeptieren",
      discardAll: "Alle verwerfen",
      stop: "Stopp",
      skipCard: "Karte überspringen",
      processing: "Verarbeite {current} von {total}…",
      waiting: "Warte…",
      done: "Fertig",
      noProposals: "Keine Änderungen vorgeschlagen",
      applied: "Angewendet",
      skipped: "Übersprungen",
      failed: "Fehlgeschlagen",
      summary: "{applied} angewendet · {skipped} übersprungen · {failed} fehlgeschlagen",
      bulkButton: "Mit KI überarbeiten",
      suggestionFor: "Vorschlag für {field}",
      statusReady: "Bereit",
      statusAccepted: "Übernommen",
      apply: "{count} anwenden",
      backToList: "Zurück",
      selectPrompt: "Wähle eine Karte zur Überprüfung",
    },
    aiGenerator: {
      destination: "Ziel",
      deckSearchPlaceholder: "Vorhandene Stapel im Tresor suchen…",
      title: "Karteikarten mit KI generieren",
      intro: "Beschreibe, was generiert werden soll. Karten erscheinen unten — behalte die gewünschten und speichere sie dann.",
      promptPlaceholder: "Beschreibe die zu generierenden Karteikarten…",
      generate: "Generieren",
      generating: "Generiere…",
      continueGenerating: "Weiter erzeugen",
      stop: "Stopp",
      streaming: "{count} bisher generiert…",
      noCards: "Noch keine Karten. Beschreibe unten, was generiert werden soll.",
      backToList: "Zurück",
      selectPrompt: "Wähle eine Karte zur Ansicht",
      fieldFront: "Vorderseite",
      fieldBack: "Rückseite",
      fieldNotes: "Notizen",
      keep: "Behalten",
      discard: "Verwerfen",
      addNote: "Notiz anhängen",
      addImage: "Bild anhängen",
      modeNew: "Neue Datei erstellen",
      modeAppend: "An Stapel anhängen",
      format: "Format",
      formatHeader: "Überschrift + Absatz",
      formatTable: "Tabelle",
      formatCanvas: "Canvas",
      name: "Dateiname",
      namePlaceholder: "Neuer Stapel",
      folder: "Ordner",
      folderPlaceholder: "Tresor-Stammverzeichnis",
      tag: "Tag",
      profile: "Profil",
      deck: "Stapel",
      save: "{count} speichern",
      saving: "Speichere…",
      saved: "{count} Karten gespeichert",
      saveFailed: "Karten konnten nicht gespeichert werden",
      nameRequired: "Gib einen Dateinamen ein",
      deckRequired: "Wähle einen Stapel zum Anhängen",
      noKept: "Behalte mindestens eine Karte zum Speichern",
      close: "Schließen",
      cancel: "Abbrechen",
      includeGenerated: "Generierte Karten als Kontext einbeziehen",
      existingCardsHeading: "In dieser Sitzung bereits generierte Karten:",
      avoidDuplicatesInstruction: "Wiederhole keine der oben genannten Karten.",
      savedBadge: "Gespeichert",
      savedNotice: "Bisher {count} Karten gespeichert",
      generateFailed: "Karten konnten nicht generiert werden",
      debugTitle: "Debug",
      debugSystem: "System",
      debugUser: "Benutzer",
      debugPriorAssistant: "Assistent (bisherige Karten)",
      debugFollowup: "Folgenachricht",
      debugImages: "Bilder",
      debugResponse: "Antwort",
      debugEmpty: "Starte eine Generierung, um Anfrage und Antwort zu sehen.",
      debugToggle: "Debug-Panel umschalten",
      debugOcr: "OCR-Transkriptionen",
      debugOcrPage: "Seite {page}",
      pdfChapters: "Kapitel",
      pdfClosePanel: "Kapitelbereich schließen",
      pdfCloseTab: "Dieses PDF entfernen",
      pdfParseMode: "Auswertungsmodus",
      pdfModeAuto: "Automatisch (Text, OCR bei Scans)",
      pdfModeText: "Nur Text",
      pdfModeOcr: "Alle Seiten per OCR",
      pdfSelectAll: "Alle wählen",
      sourceCovered: "Das Modell hat aus dieser Quelle nichts Weiteres gefunden.",
      clearCards: "Leeren",
      clearConfirmTitle: "Generierte Karten verwerfen?",
      clearConfirmMessage:
        "{count} behaltene Karten sind noch nicht gespeichert und werden verworfen. Bereits gespeicherte Karten bleiben in deinem Vault.",
      pdfClear: "Leeren",
      pdfOcrProgress: "Seiten werden gelesen {done}/{total}…",
      pdfCached: "zwischengespeichert",
      pdfNoChapters: "Keine Kapitel erkannt.",
      addPdf: "PDF anhängen",
      pdfFromVault: "Aus dem Vault",
      pdfFromComputer: "Vom Computer",
      pdfTooLarge: "PDF ist zu groß (max. {max} MB).",
      tooManyNotes: "Du kannst höchstens {max} Notizen anhängen.",
      tooManyImages: "Du kannst höchstens {max} Bilder anhängen.",
    },
    editFlashcard: {
      aiRefactor: "Mit KI überarbeiten",
      aiRefactoring: "Überarbeite…",
      aiSuggestion: "KI-Vorschlag",
      aiAccept: "Akzeptieren",
      aiDecline: "Ablehnen",
      aiNoChanges: "Die KI hat keine Änderungen vorgeschlagen",
      aiDrawerTitle: "KI-Überarbeitung",
      aiIncludeSourceContext: "Quellkontext einbeziehen ({file})",
      aiPresetFixLatex: "LaTeX korrigieren",
      aiPresetSimplify: "Erklärung vereinfachen",
      aiPresetTranslate: "Ins Englische übersetzen",
      aiCustomInstructionsPlaceholder: "Eigene Anweisungen für diese Karte…",
      aiTargetScope: "Zielfelder",
      aiGenerate: "Generieren",
      aiPromptPlaceholder: "Beschreibe, wie diese Karte überarbeitet werden soll…",
      aiAddNote: "Notiz anhängen",
      aiAddImage: "Bild anhängen",
      aiAddPdf: "PDF anhängen",
      aiSend: "Generieren",
      aiModel: "Modell",
      aiAcceptVersion: "Diese Version übernehmen",
      aiDismiss: "Verwerfen",
      aiShow: "KI-Assistent anzeigen",
      aiHide: "KI-Assistent ausblenden",
      aiSplit: "Teilen",
      aiSplitToggle: "In mehrere Karten teilen",
      aiAcceptSplit: "Aufteilung übernehmen",
      aiSplitTitle: "In {count} Karten teilen",
      aiRemoveContext: "Entfernen",
      aiDebugTitle: "Letzte Anfrage / Antwort",
      aiDebugSystem: "System",
      aiDebugPrompt: "Prompt",
      aiDebugResponse: "Antwort",
      cancel: "Abbrechen",
      save: "Speichern",
      saving: "Speichere…",
      title: "Karte bearbeiten",
      fieldHeader: "Überschrift",
      fieldBody: "Inhalt",
      fieldFront: "Vorderseite",
      fieldBack: "Rückseite",
      fieldNotes: "Notizen",
      fieldHint: "Hinweis",
      fieldImage: "Bild",
      fieldQuestion: "Frage",
      fieldOptions: "Optionen",
      fieldSentence: "Satz",
      templateColumnsHint: "Diese Karte nutzt eine Vorlage — bearbeite unten ihre Zeilenspalten.",
      templateColumnFallback: "Spalte {n}",
    },
  },

  settings: {
    ai: {
      heading: "KI-Funktionen",
      enabled: "KI-Funktionen aktivieren",
      enabledDesc: "Schaltet die KI-gestützte Überarbeitung von Karten ein. Erfordert einen API-Schlüssel oder ein lokales Modell.",
      provider: "Anbieter",
      providerDesc: "Welcher KI-Anbieter für die Überarbeitung genutzt wird.",
      model: "Modell",
      modelDesc: "Modellname für den gewählten Anbieter.",
      tier: "Qualitätsstufe",
      tierDesc: "Decks Pro wählt das Modell anhand der Stufe für dich.",
      modelCustom: "Benutzerdefiniert…",
      modelCustomLabel: "Eigenes Modell",
      localBaseUrl: "URL des lokalen Servers",
      localBaseUrlDesc: "Basis-URL für einen OpenAI-kompatiblen lokalen Server wie Ollama oder LM Studio.",
      apiKey: "API-Schlüssel",
      apiKeyDesc: "Wird in einer lokalen Datei gespeichert, die nicht zwischen Geräten synchronisiert wird.",
      apiKeyPlaceholder: "API-Schlüssel einfügen",
      providerGemini: "Google Gemini",
      providerOpenai: "OpenAI",
      providerClaude: "Anthropic Claude",
      providerLocal: "Lokaler oder eigener Endpunkt",
      providerDecksPro: "Decks Pro",
      serverUrl: "Server-URL",
      serverUrlDesc: "Backend-URL von Decks Pro. Leer lassen, um die Standard-URL zu nutzen.",
      siteUrl: "Website-URL",
      siteUrlDesc: "URL der Decks-Pro-Website. Leer lassen für den Standard.",
      accountLoading: "Konto wird geladen\u2026",
      accountError: "Kontodaten konnten nicht geladen werden. Pr\u00fcfe deine Verbindung.",
      accountRefresh: "Aktualisieren",
      trialActive: "Kostenlose Testphase \u2013 etwa 100 Lernkarten oder 10 PDF-Seiten.",
      trialExhausted: "Testphase aufgebraucht. Abonniere, um weiter zu generieren.",
      dailyLimitReached: "Tageslimit erreicht. Es wird morgen zur\u00fcckgesetzt.",
      account: "Decks-Pro-Konto",
      accountSignedOut: "Melde dich an, um Decks Pro zu nutzen.",
      accountSignedInAs: "Angemeldet als {email}",
      accountSignedIn: "Angemeldet",
      signIn: "Bei Decks Pro anmelden",
      signInPending: "Schließe die Anmeldung im Browser ab und komm dann hierher zurück.",
      signInSuccess: "Bei Decks Pro angemeldet.",
      signInFailed: "Anmeldung fehlgeschlagen. Bitte versuche es erneut über die Einstellungen.",
      signOut: "Abmelden",
      signOutDone: "Von Decks Pro abgemeldet.",
      subscribe: "Abonnieren",
      subscribeDesc: "9,99 \u20ac pro Monat \u2013 etwa 20.000 Lernkarten oder 5.000 PDF-Seiten.",
      manageSubscription: "Abo verwalten",
      openAccount: "Kontoseite öffnen",
      subscriptionStatus: "Abonnement",
      subscriptionNone: "Kein aktives Abo. Abonniere, um mit dem Generieren zu starten.",
      subscriptionActive: "Aktiv",
      subscriptionTrial: "Testphase",
      subscriptionPastDue: "Zahlung überfällig",
      subscriptionCancelled: "Gekündigt – aktiv bis zum Ende des Zeitraums",
      subscriptionExpired: "Abgelaufen",
      subscriptionPaused: "Pausiert",
      enableDesc: "Schaltet die Kartenerzeugung und die KI-Aktionen im Karteneditor ein.",
      baseUrl: "Basis-URL",
      baseUrlDesc: "Ein lokaler oder selbst gehosteter Modell-Endpunkt, der von diesem Gerät erreichbar ist.",
      modelId: "Modell-ID",
      modelIdDesc: "Die Modell-ID, die dein Server bereitstellt.",
      proAccount: "DecksMD-Pro-Konto",
      proDesc: "Nutze dein DecksMD-Konto, um Karten ohne eigenen API-Schlüssel zu erzeugen.",
      noKeyNeeded: "DecksMD Pro braucht keinen eigenen Schlüssel. Die anderen Anbieter greifen direkt auf dein Konto zu.",
      email: "E-Mail",
      password: "Passwort",
      noAccountYet: "Noch kein Konto?",
      createOne: "Auf decksmd.app eines erstellen",
      manageAccount: "Konto verwalten",
      billingDesc: "Abrechnung und verknüpfte Geräte findest du auf der Website.",
      seePlans: "Tarife ansehen",
      open: "Öffnen",
      clearKey: "Löschen",
      deviceSignedIn: "Dieses Gerät ist angemeldet, ein Schlüssel ist nicht nötig.",
    },
    language: {
      heading: "Sprache",
      name: "Plugin-Sprache",
      desc: 'Wähle die Anzeigesprache. "Auto" folgt der Spracheinstellung von Obsidian. Eine Änderung erfordert das Neuladen des Plugins.',
      auto: "Auto",
    },

    review: {
      heading: "Wiederholungssitzungen",
      showProgress: "Fortschritt anzeigen",
      showProgressDesc:
        "Fortschrittsbalken während Wiederholungssitzungen anzeigen",
      keyboardShortcuts: "Tastenkürzel",
      enableShortcuts: "Tastenkürzel aktivieren",
      keyboardShortcutsDesc:
        "Tastenkürzel im Wiederholungsfenster aktivieren (1-4 für Schwierigkeit)",
      shortcutsHeading: "Tastenkürzel für die Wiederholung",
      shortcutsHeadingDesc: "Klicke unten auf eine Taste und drücke dann die gewünschte Taste. Esc zum Abbrechen.",
      shortcutReveal: "Taste zum Aufdecken / Weiter",
      shortcutPressKey: "Taste drücken… (Esc zum Abbrechen)",
      shortcutResetDefaults: "Auf Standard zurücksetzen",
      shortcutDuplicate: "Diese Taste wird bereits von einer anderen Aktion verwendet.",
      sessionDuration: "Sitzungsdauer",
      sessionDurationDesc:
        "Maximale Dauer für Wiederholungssitzungen in Minuten (1-60)",
      sessionDurationPlaceholder: "25",
      studyDayStartsAt: "Lerntag beginnt um",
      studyDayStartsAtDesc:
        "Stunde, zu der der Lerntag wechselt (0-23, Standard 4). Wiederholungen vor dieser Stunde zählen zur Statistik und zu den Tageslimits des Vortags.",
      enableGlobalReviewCap: "Globales tägliches Kartenlimit",
      enableGlobalReviewCapDesc: "Begrenzt die Gesamtzahl der Karten (neu + Wiederholung) pro Tag über alle Stapel hinweg (zusätzlich zu den Limits pro Stapel).",
      globalReviewCap: "Karten pro Tag (alle Stapel)",
      globalReviewCapDesc: "Maximale Anzahl an Karten (neu + Wiederholung) pro Tag über alle Stapel hinweg.",
      globalReviewCapPlaceholder: "100",
      leechThreshold: "Egel-Schwelle",
      leechThresholdDesc:
        "Eine Karte wird als Egel markiert, sobald ihre Vergessen-Anzahl diese Zahl erreicht (Standard 8). Egel werden wiederholt vergessen und sollten meist umgeschrieben werden.",
      leechThresholdPlaceholder: "8",
      denseCardCharThreshold: "Schwellenwert dichte Karten (Zeichen)",
      denseCardCharThresholdDesc:
        "Eine Karte wird als dicht markiert, sobald ihr Rücktext diese Zeichenanzahl erreicht (Standard 500). Dichte Karten sind meist zu informationsreich und profitieren vom Aufteilen.",
      denseCardCharThresholdPlaceholder: "500",
    },

    parsing: {
      heading: "Parsen",
      deckTag: "Stapel-Tag",
      deckTagDesc:
        "Basis-Tag zur Identifizierung von Karteikarten-Stapeln. Dateien mit diesem Tag (oder Unter-Tags wie {tag}/math) werden als Stapel behandelt. Das Standard-Tag wurde von #flashcards auf #decks geändert. #flashcards funktioniert weiterhin, bis du es hier änderst.",
      deckTagPlaceholder: "#decks",
      folderSearchPath: "Ordnersuchpfad",
      folderSearchPathDesc:
        'Scan auf einen bestimmten Ordner beschränken. „Ganzen Vault scannen" wählen, um alle Dateien zu scannen.',
      folderSearchPathDefault: "Ganzen Vault scannen (Standard)",
    },

    templates: {
      heading: "Kartenvorlagen",
      folder: "Vorlagenordner",
      folderDesc: "Ordner mit Vorlagendateien. Die decks-tags jeder Datei binden sie an Tabellenzeilen oder Notizen.",
      folderDefault: "Deaktiviert (kein Vorlagenordner)",
      previewHeading: "Vorschau",
      previewHint: "Die Vorschau nutzt Beispieldaten aus einer Zeile.",
      noTemplates: "Im gewählten Ordner wurden noch keine Vorlagen gefunden.",
      notesLabel: "Notizen",
    },

    canvasDecks: {
      heading: "Canvas-Stapel",
      folderPath: "Canvas-Stapel-Ordner",
      folderPathDesc:
        "Ordner, der nach Obsidian-Canvas-Dateien (.canvas) durchsucht wird. Jedes Canvas darin wird zu einem Stapel. Jeder Textknoten wird mit den gleichen vier Karteikarten-Formaten wie Markdown-Dateien analysiert. Leer lassen, um die Canvas-Suche zu deaktivieren.",
      folderPathDefault: "Deaktiviert – keine Canvas-Suche",
      tagName: "Canvas-Stapel-Tag",
      tagNameDesc:
        "Tag, das jedem gefundenen Canvas-Stapel zugewiesen wird. Wird für die Gruppierung in der Tag-Ansicht und für die Profilzuordnung verwendet. Standard: #decks/canvas.",
      tagNamePlaceholder: "#decks/canvas",
    },

    ui: {
      heading: "Benutzeroberfläche",
      backgroundRefreshInterval: "Aktualisierungsintervall im Hintergrund",
      backgroundRefreshIntervalDesc:
        "Wie oft die Stapel-Statistik im Seitenpanel aktualisiert wird (in Sekunden)",
      backgroundRefreshIntervalPlaceholder: "5",
      enableBackgroundRefresh: "Hintergrundaktualisierung aktivieren",
      enableBackgroundRefreshDesc:
        "Stapel-Statistik im Seitenpanel automatisch aktualisieren",
      enableNotices: "Benachrichtigungen aktivieren",
      enableNoticesDesc:
        "Benachrichtigungen für abgeschlossene Wiederholungssitzungen und Sync-Operationen anzeigen",
      hideAnchorTokens: "Karten-ID-Markierungen ausblenden",
      hideAnchorTokensDesc:
        "Blendet die %%dk:…%%-Markierungen, die Decks in deine Notizen schreibt, in der Live-Vorschau aus. In der Zeile mit dem Cursor erscheinen sie wieder; aus der Datei werden sie nie entfernt.",
      reviewDisplayMode: "Anzeigemodus für Wiederholungen",
      reviewDisplayModeDesc:
        "Wo Wiederholungs- und Durchsuchen-Sitzungen geöffnet werden. Tabs können in Seitenleisten, untere Panels oder separate Fenster gezogen werden.",
      flashcardManagerDisplayMode: "Anzeigemodus für Karteikarten-Manager",
      flashcardManagerDisplayModeDesc:
        "Wo der Karteikarten-Manager geöffnet wird. Tabs können in Seitenleisten, untere Panels oder separate Fenster gezogen werden.",
      aiGeneratorDisplayMode: "Anzeigemodus des KI-Generators",
      aiGeneratorDisplayModeDesc:
        "Wo der KI-Karteikartengenerator geöffnet wird. Tabs können in Seitenleisten, untere Panels oder separate Fenster gezogen werden.",
      displayModeModal: "Modal-Overlay",
      displayModeTab: "Neuer Tab",
      minDeckCardCount: "Stapel ausblenden mit weniger als",
      minDeckCardCountDesc:
        "Stapel (und Gruppen) mit weniger als dieser Gesamtkartenanzahl werden aus der Liste ausgeblendet. Angeheftete Stapel sind immer sichtbar. 0 deaktiviert.",
      minDeckCardCountPlaceholder: "0",
    },

    backup: {
      heading: "Sicherung",
      enableAutoBackup: "Automatische Sicherung aktivieren",
      enableAutoBackupDesc:
        "Wiederholungsdaten nach jeder Sitzung automatisch sichern",
      maxBackups: "Maximale Sicherungen",
      maxBackupsDesc: "Maximale Anzahl der zu behaltenden Sicherungen (3-10)",
      createBackupNow: "Jetzt Sicherung erstellen",
      createBackupNowDesc:
        "Manuelle Sicherung der Karteikarten-Datenbank erstellen",
      createBackupButton: "Sicherung erstellen",
      restoreHeading: "Sicherung wiederherstellen",
      availableBackups: "Verfügbare Sicherungen",
      availableBackupsDesc: "Sicherung zum Wiederherstellen auswählen",
      selectBackupOption: "Sicherung auswählen…",
      noBackupsOption: "Keine Sicherungen gefunden",
      refreshButton: "Aktualisieren",
      refreshTooltip: "Sicherungsliste aktualisieren",
      restoreButton: "Wiederherstellen",
      restoreFromFile: "Aus Datei wiederherstellen",
      restoreFromFileTooltip:
        "Eine .db-Sicherung von beliebiger Stelle auf der Festplatte wählen",
      rebuildState: "Kartenplanung neu aufbauen",
      rebuildStateDesc: "Stellt die Planung für Karten wieder her, die als neu erscheinen, aber noch Lernverlauf haben.",
      rebuildStateButton: "Aus Verlauf wiederherstellen",
    },

    fsrs: {
      heading: "Algorithmus-Feinabstimmung",
      optimize: "Parameter optimieren",
      optimizeButton: "Optimieren",
      resetButton: "Auf Standard zurücksetzen",
      descUntrained:
        "Trainiert die 21 Gewichte des Algorithmus auf deinem Standard-Profil-Wiederholungsverlauf. Benötigt mindestens 100 Wiederholungen. Intensive Stapel sind nicht betroffen.",
      descTrained:
        "Verwende derzeit trainierte Gewichte vom {when} ({count} Wiederholungen, Log-Loss {before} → {after}).",
      descTrainedUnknownWhen: "unbekannt",
      descTrainedMissingMetric: "—",
    },

    debug: {
      heading: "Debug",
      paragraph: "Debug-Einstellungen für Fehlersuche und Entwicklung.",
      enableLogging: "Debug-Logging aktivieren",
      enableLoggingDesc:
        "Detailliertes Logging in der Konsole für Sync-Operationen und Karteikartenverarbeitung anzeigen",
      performanceLogs: "Performance-Logs aktivieren",
      performanceLogsDesc:
        "Performance-Timing-Metriken in der Konsole anzeigen (Sync-Zeiten, Parsing-Performance usw.)",
    },

    paths: {
      heading: "Dateispeicherorte",
      paragraph:
        "Alle Pfade sind Vault-relativ. Leer lassen für den Standardpfad. Die meisten Nutzer müssen hier nichts ändern — die Optionen sind vorhanden, um die Datenbank aus dem versteckten Ordner {configDir}/ zu verschieben (iCloud und andere Sync-Anbieter behandeln versteckte Ordner nachrangig).",
      dbFolder: "Datenbankordner",
      dbFolderDesc:
        'Ordner mit flashcards.db. Standard: {pluginFolder}/. Obsidian nach Änderung neu starten. Die DB-Datei wird nicht automatisch verschoben — zuerst sichern, dann manuell verschieben oder mit „Aus Datei wiederherstellen" in den neuen Speicherort importieren.',
      backupFolder: "Sicherungsordner",
      backupFolderDesc:
        "Ordner für automatische und manuelle Sicherungen. Standard: {pluginFolder}/backups/. Änderungen werden sofort wirksam — kein Neustart erforderlich.",
      backupFolderPlaceholder: "{pluginFolder}/backups",
      syncLogFolder: "Sync-Log-Ordner",
      syncLogFolderDesc:
        "Ordner für die geräteweisen .deckssynclog-Dateien für die Multi-Device-Synchronisierung. Standard: Vault-Wurzel (die iCloud am schnellsten synchronisiert). Obsidian nach Änderung neu starten.",
      syncLogFolderPlaceholder: "(Vault-Wurzel)",
      pdfCacheFolder: "Ordner für den PDF-OCR-Zwischenspeicher",
      pdfCacheFolderDesc: "Ordner mit dem zwischengespeicherten Seitentext aus PDF-OCR, damit ein PDF nie zweimal gelesen wird.",
      pdfCacheFolderPlaceholder: "{pluginFolder}/pdf-ocr",
    },

    database: {
      heading: "Datenbankverwaltung",
      paragraph:
        "Verwalte deine Karteikarten-Datenbank. Mit Vorsicht verwenden – diese Aktionen können nicht rückgängig gemacht werden.",
      purgeDatabase: "Datenbank leeren",
      purgeDatabaseDesc:
        "Löscht alle Karten, den Wiederholungsverlauf und die Stapel-Daten endgültig. Erzwingt einen sauberen Neuaufbau aus den Vault-Dateien. Aller Fortschritt geht verloren!",
      purgeDatabaseButton: "Datenbank leeren",
    },
    hideIdentityMarkers: "Karten-Identitätsmarker ausblenden",
    scanFolderDesc: "Die Suche auf einen Ordner beschränken. Leer lassen, um alles zu durchsuchen.",
    foldersRelativeDesc: "Ordner sind relativ zum Vault. Leer lassen für den Standardort.",
    syncLogFolderDesc: "Ordner für die geräteeigenen Sync-Protokolle der Mehrgeräte-Synchronisation. Standard: Vault-Wurzel.",
    syncLogIdDesc: "Kennzeichnet die Sync-Protokolldatei dieses Geräts. Ein Ordnerwechsel gilt ab dem nächsten Öffnen des Vaults.",
    canvasTagRule: "Ein Canvas-Tag muss mit # beginnen und Kleinbuchstaben, Ziffern, Binde-, Unterstriche oder Schrägstriche verwenden.",
    fallbackExamDesc: "Wird von Stapeln ohne eigenes Profil genutzt, etwa eigenen Stapeln. Stapel mit Profil verwenden dessen Prüfungseinstellungen.",
  },
  cardEdit: {
    typeMismatch: "Bearbeitungstyp {edit} passt nicht zum Kartentyp {card}",
    vaultReadOnly: "Dieser Vault ist schreibgeschützt geöffnet.",
    canvasNotHere: "Canvas-Karten lassen sich hier nicht bearbeiten.",
    couldNotRead: "{file} konnte nicht gelesen werden: {reason}",
    cardNotFound: "Diese Karte war in der Quellnotiz nicht auffindbar.",
    noteChanged: "Die Notiz hat sich geändert, seit diese Karte geladen wurde. Neu laden und erneut versuchen.",
    tableRowUnparseable: "Die Tabellenzeile lässt sich nicht mehr auswerten.",
    tableRowChanged: "Die Tabellenzeile hat sich geändert, seit diese Karte geladen wurde.",
    clozeChanged: "Der Lückentext hat sich geändert, seit diese Karte geladen wurde.",
    clozeRowUnparseable: "Die Tabellenzeile des Lückentexts lässt sich nicht mehr auswerten.",
    clozeRowInvalid: "Die Tabellenzeile des Lückentexts ist keine gültige Tabellenzeile.",
    emptyHeaderSegment: "Leerer Überschriftenabschnitt.",
    anchorNotHeader: "Die Ankerzeile ist keine Überschrift.",
    notATableRow: "Keine gültige Tabellenzeile.",
    noNotesColumn: "Diese Tabelle hat keine Notizen-Spalte.",
    separatorNotFound: "Die Trennzeile der Tabelle für eine neue Notizen-Spalte war nicht auffindbar.",
    frontEmpty: "Der Text der Vorderseite darf nicht leer sein.",
    headerEmpty: "Der Text der Überschrift darf nicht leer sein.",
    clozeNeedsSpan: "Ein Lückentext braucht mindestens eine ==Lücke==.",
  },
};
