...
export class StatisticsService {
    ...
    async simulateFutureDueLoadForDeck(
        ...
        let backlog = await this.db.getCurrentBacklog(
            deckId,
            start.toISOString()
        );
    ...
    private async simulateFsrsDemand(
        ...
        while (heap.length > 0) {
            ...
            if (profile === "INTENSIVE") {
                const dayKey = new Date(node.nextDue)
                    .toISOString()
                    .slice(0, 10);
                ...
                if (
                    ...
                    if (deck.config.reviewCardsPerDay === 0) {
                        ...
                        const remainingReview = Math.max(
                            0,
                            deck.config.reviewCardsPerDay -
                                dailyCounts.reviewCount
                        );
