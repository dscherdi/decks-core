...
export class Scheduler {
    ...
    async endReviewSession(sessionId: string): Promise<void> {
        ...
        if (this.settings.backup.enableAutoBackup) {
            try {
                ...
                this.debugLog(
                    "Failed to create backup after session end:",
                    error
                );
            }
        }
    ...
    async rate(
        ...
        this.debugLog(
            `Found card: ${card.front.substring(0, 50)}... in deck: ${
                deck.name
            }`
        );
        this.updateFSRSForDeck(deck);
    ...
    async peekDue(now: Date, deckId: string, limit = 10): Promise<Flashcard[]> {
        this.debugLog(
            `Peeking at due cards for deck: ${deckId}, limit: ${limit}`
        );
        const deck = await this.db.getDeckById(deckId);
        if (!deck) return [];
    ...
    `;
        ...
        const results = await this.db.querySql(query, [
            deckId,
            now.toISOString(),
        ]);
