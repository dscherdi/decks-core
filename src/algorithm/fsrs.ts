...
export class FSRS {
  ...
  private calculateSchedule(card: Flashcard, difficulty: Difficulty, now: Date): SchedulingCard {
    let interval: number;
  ...
  private nextInterval(currentInterval: number, easeFactor: number, retrievability: number, rating: number): number {
